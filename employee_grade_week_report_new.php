...
$today = select_query_json("SELECT SYSDATE AS TODAYS_DATE FROM DUAL");

//$sql_week = select_query_json(" SELECT NEXT_DAY ((SYSDATE-1) - 7, 'SUN') AS PREV_SUNDAY ,(SYSDATE-1) AS TODAYS_DATE FROM DUAL");
//print_r($sql_week);
// Select & display the Current Week Sunday to Saturday
...
function update_detail(){
 ...
 <div class="row">
            ...
            <div class="page-content-wrap">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title">MY TEAM EMPLOYEES GRADE FIX</h3><br>



                        <div class="pull-right">
                            <?  $menu_name = 'EMPLOYEE GRADE MASTER';
                                $inner_submenu = select_query_json("select MNUCODE from srm_menu where SUBMENU = '".$menu_name."' order by MNUCODE Asc", "Centra", 'TEST');
                                if($_SESSION['tcs_empsrno'] != '') {
                                    $inner_menuaccess = select_query_json("select * from srm_menu_access
                                                                                    where MNUCODE = ".$inner_submenu[0]['MNUCODE']." and ENTSRNO = '".$_SESSION['tcs_empsrno']."' 
                                                                                    order by MNUCODE Asc", "Centra", 'TEST');
                                } else {
                                    $inner_menuaccess = select_query_json("select * from srm_menu_access
                                                                                    where MNUCODE = ".$inner_submenu[0]['MNUCODE']." and SUPCODE = '".$_SESSION['tcs_userid']."' 
                                                                                    order by MNUCODE Asc", "Centra", 'TEST');
                                }
                                if($inner_menuaccess[0]['VEWVALU'] == 'Y') { ?><a class="btn btn-danger" target="_blank" style="margin-right: 5px;" href="employee_add.php">(+) Add Employees</a>&nbsp;&nbsp;<? } ?><a class="btn btn-danger" target="_blank" style="margin-right: 50px;" href="employee_grade_fix_week_report.php">Grade Fix Report</a><br><br>
                        </div>
                    </div>
                    ...
                    <table id="customers" class="table datatable" border="1">
						...
						<?	foreach ($sql_week as $key => $week_value) {
                            ...
                            // $dt_val = strtoupper(date("d-M-Y", strtotime($week_value['TODAYS_DATE'])));
							?>		
							<th style="text-align: center;"><? echo $dt_value;?></th>
						<? }?>
                        <?
                        $start=strtoupper(date("d-M-Y", strtotime($sql_week[0]['DT'])));
                        $end=strtoupper(date("d-M-Y", strtotime($today[0]['TODAYS_DATE'])));
                        ?>
                         <th style="text-align: center;">REMARKS <?php//echo $sql_week[0]['PREV_SUNDAY'];?> </th>
                        <th style="text-align: center;background-color: #0d3575"><?php echo $start.' TO '.$end; ?></th>
                        ...
                        <? $cnt_join = count($join); $search_ii = 0;
                           ...
                           //$dt_value = strtoupper(date("d-M-Y", strtotime($week_value['PREV_SUNDAY'])));
                            //  $dt_val = strtoupper(date("d-M-Y", strtotime($week_value['TODAYS_DATE'])));
                            ...
                            <tr class="odd gradeX" style='background-color:<?=$bgclr?>; color:<?=$clr?>;'>
                                ...
                                <td class="center" width="15%"><? echo $join[$search_i]['EMPCODE'];?> - <? echo $join[$search_i]['EMPNAME'];?></td>
								<?	$cnt_join = count($join); $t=0;
								foreach ($sql_week as $key => $week_value) {$t++;
                              $dt_value = strtoupper(date("d-M-Y", strtotime($week_value['DT'])));
                              //echo "select EMPGRAD, EMPRMRK from EMPLOYEE_GRADE_FIX 
                                                                                   // where deleted = 'N' and EMPSRNO = '".$join[$search_i]['EMPSRNO']."' ".$and."";  
                             // $dt_val = strtoupper(date("d-M-Y", strtotime($week_value['TODAYS_DATE']))); 
                              ?>		
						<td class="center" style="text-align: center;">

                                    <select id="txt_employee_grade_<?=$search_i;?>_<?=$dt_value?>" name='txt_employee_grade_<?=$search_i;?>_<?=$dt_value?>_<?=$dt_val?>' rel="<?=$dt_value?>" class="txt_employee_<?=$search_i?>" onchange="task_assign('<?=$join[$search_i]['EMPSRNO']?>', '<?=$search_i?>', '<?=$dt_value?>');" style="min-height: 25px;">
                                        ...
                                        $sql_empgrade = select_query_json("select EMPGRAD, EMPRMRK from EMPLOYEE_GRADE_FIX 
                                                                                    where deleted = 'N' and EMPSRNO = '".$join[$search_i]['EMPSRNO']."' ".$and."", 'Centra', 'TEST'); 
																		echo "select EMPGRAD, EMPRMRK from EMPLOYEE_GRADE_FIX 
                                                                                    where deleted = 'N' and EMPSRNO = '".$join[$search_i]['EMPSRNO']."' ".$and."";			
																					?>
									...
									<!--  <input type="text" name="txt_empgrade_remarks_<?=$search_i?>_<?=$dt_value?>" id='txt_empgrade_remarks_<?=$search_i?>_<?=$dt_value?>' maxlength="100" class="form-control" placeholder="Employees Remarks" title="Employee Remarks" value="<?=$sql_empgrade[0]['EMPRMRK']?>" onblur="task_assign('<?=$join[$search_i]['EMPSRNO']?>', '<?=$search_i?>', '<?=$dt_value?>');" style="width: 100%; text-transform: uppercase; border: 1xp solid #000"> -->
                                </td>
								
								
						<? } ?>
                      
                                    <input type="hidden" id="remark_val_<?=$search_i?>" value="<?=$sql_empgrade[0]['EMPRMRK']?>"/>
						<td class="center" style="text-align: center;"><input type="text" name="txt_empgrade_remarks_<?=$search_i?>" id='txt_empgrade_remarks_<?=$search_i?>' maxlength="100" class="form-control" placeholder="Employees Remarks" title="Employee Remarks" value="<?=$sql_empgrade[0]['EMPRMRK']?>" onblur="setremark(this.value,'<?=$search_i?>','<?=$join[$search_i]['EMPSRNO']?>');" style="width: 100%; text-transform: uppercase; border: 1px solid #000"></td>
						 <td class="center" style="text-align: center;background-color:#00bcd41f"> <select id="txt_employee_grade_week_<?=$search_i?>" name='txt_employee_grade_week' class="txt_employee_grade" rel="<?=$search_i?>" style="min-height: 25px;width:100%" onchange="setvalue(this.value,'<?=$search_i?>','<?=$join[$search_i]['EMPSRNO']?>')">
    ...
    function task_assign(empsrno, iv, fromdate) {
        $('#load_page').show();
        var grade = $("#txt_employee_grade_"+iv+"_"+fromdate).val();
      ...
      //  var txt_empgrade_remarks = $("#txt_empgrade_remarks_"+iv).val();
        ...
        var strURL="ajax/fix_grade_week_report.php?action=fix_grade1&empsrno="+empsrno+"&grade="+grade+"&txt_empgrade_remarks="+txt_empgrade_remarks+"&fromdate="+fromdate+"&status="+status;
	...
	function setvalue(val,id,empsrno){
   
    $('.txt_employee_'+id).each(function(){

        $(this).val(val);
        var dat=$(this).attr('rel');
        console.log(dat);
        task_assign(empsrno, id, dat);
    ...
    function setremark(val,id,empsrno){
 console.log($('#remark_val_'+id).val());
 console.log(val);
    if($('#remark_val_'+id).val()==val)	
    	return false;
    var grade=$('#txt_employee_grade_week_'+id).val();	
...
/*$(function() {
        ...
        $(document).ready(function() {
            ...
            $('.txt_employee_grade').each(function(){

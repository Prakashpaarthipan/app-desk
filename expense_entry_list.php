                ...
                <div class="panel panel-default">
                  <div class="panel-body">
                    <div class="panel-heading" style="margin-bottom: 20px;">
                      ...
                      <? 
					  if($action == 'request'){
					$opensearch = select_query_json("select APPSTAT,EXPVALUE from APPROVAL_REQUEST where APPSTAT = 'E' and APRNUMB = '".$_REQUEST['ap']."'","Centra","TEST");
					  }
					  else{
                        ...
                        <div class='row'>
                           ...
                           <!--  <div class="col-md-4">
                            ...
                            <div class="col-md-4 col-sm-offset-2">
                              ...
                              <button class="btn btn-success pull-left" type="button"   id="appbtn" onclick=" moveToapproval()"><span class="fa fa-finish"></span> Finish</button>
    ...
    function checkNumberCess(val){
                ...
                success: function(data1) {        
                    if(data1 == '') {
                        ...
                        if(ostate == 1)
                        {
                            ...
                            $("#cgst_"+id).attr("readonly",false);
                            $("#sgst_"+id).attr("readonly",true);
                            $("#igst_"+id).attr("readonly",true);
                            $("#igst_"+id).val(0);
                        }
                        else{
                            $("#cgst_"+id).val(0);
                            $("#sgst_"+id).val(0);
                            $("#cgst_"+id).attr("readonly",true);
                            $("#sgst_"+id).attr("readonly",true);
                            $("#igst_"+id).attr("readonly",false);
       ...
       function moveToapproval(){
        var apprnumb = $("#appnumb").text();
         ...
         console.log(tar+""+apprnumb+""+procode);
          ...
          $.ajax({
             url:"prakash/savesupplierquote.php?action=finish&ap="+apprnumb,            
            type: "POST",                    
            processData: false,
            contentType: false,
            async:true,
            ...
            success:function(data){
                 jAlert("Approval is moving to flow users","Alert");
                  ...
                  // $('#appbtn').attr('disabled', 'disabled');
                  //    $("#prebtn").attr('disabled','disabled');
                  //    $("#addadditional").attr('disabled','disabled');
                  //    $("#saveadditional").attr('disabled','disabled');
                  //    $("#btnsave").attr('disabled','disabled');
                     
                 document.location.href='request_list_quote.php';
              },
       ...
       function expensePreview(){
        ...
        // var prd = $("#slt_subproduct").val();
         ...
         {
          console.log("error");
          window.open("expense_supplier_report_viki.php?view=current_req&apprnumb="+apprnumb,'_blank');
...
//run time check
       ...
       ntab++;
        $('#list_content').append("<div id=content"+ntab+" class='row' style='margin-bottom: 10px;''>"+
                                        "<div class='form-group' style='width:100%;margin:0 !important'>"+
                                          ...
                                          "</div><!-- SGST -->"+
                                          "<div style='width:text-align: center;width:5%;font-size: 12px;float:left;padding:5px' >"+
                          ...
                          "<span class='help-block'>NOTE : ALLOWED ONLY PDF / IMAGES</span>"+
                                          "</div><!-- attachemnt -->"+
                                          "<div style='text-align: center;width:10%;font-size: 12px;float:left;padding:5px'>"+
                                               "<input type='text' class='form-control' id='remarks_"+ntab+"' name='remarks[]' style='text-transform: uppercase;width:70%;display:inline-block' maxlength='50'  /><span style='width: 25%;float:right;display: inline-block;' class='input-group-btn'><button id='remove_button' type='button' onclick='remove_row("+ntab+")' class='btn btn-danger' title ='Remove'>-</button></span>"+
                                          "</div><!-- remarks -->"+
      ...
      {
		  var data = $(".ui-autocomplete-input").length;
      var actual_list = $("#count_list").val();
		  if(data !=actual_list){
        $("#content"+tab).remove();
		  }
		  else{
			  jAlert("Minimun 3 Suppliers are Mandatory!");
        return false;
			 // window.location.reload(true);
		  }
      }
      ...
      function add_newrow(){
        ...
        $('#list_content').append("<div id=content"+ntab+" class='row' style='margin-bottom: 10px;''>"+
                                        "<div class='form-group' style='width:100%;margin:0 !important'>"+
                                          ...
                                          "<div style='text-align: center;width:30%;font-size: 12px;float:left;padding:5px' >"+
                                              "<input type='text' class='form-control findsp required rdupe'  id='slp_load_"+ntab+"' name='slp_load[]' style='text-transform: uppercase;' maxlength='100' required  onblur ='javascript:return removeDupeRun(this);' onchange='javascript:fix_state(this.value,"+ntab+");' />"+
                                          ...
                                          "</div><!-- amount -->"+
                                          "<div style='text-align: center;width:5%;font-size: 12px;float:left;padding:5px'>"+
                                             "<input type='text' class='form-control' id='cgst_"+ntab+"' name='cgst[]' onkeypress='javascript:return validateFloatKeyPress(this,event);return isNumber(event);' maxlength='5' onblur='setvalue(this.value,"+ntab+")' oninput ='checkNumber(this)' />"+
                                          "</div><!-- CGST -->"+
                                          "<div style='text-align: center;width:5%;font-size: 12px;float:left;padding:5px' >"+
                                              "<input type='text' class='form-control' id='sgst_"+ntab+"' name='sgst[]' onkeypress='javascript:return validateFloatKeyPress(this,event);return isNumber(event);' maxlength='5' readonly oninput ='checkNumber(this)' />"+
                                          "</div><!-- SGST -->"+
                                          "<div style='width:text-align: center;width:5%;font-size: 12px;float:left;padding:5px' >"+
                                              "<input type='text' class='form-control' id='igst_"+ntab+"' name='igst[]' onkeypress='javascript:return validateFloatKeyPress(this,event);return isNumber(event);' maxlength='5' oninput ='checkNumber(this)' />"+
                                          "</div><!-- IGST -->"+
...
//validate
  function validate(){
    ...
    $("#hid_expense").val(expense); 
	  ...
	  {
		  var data = $(".findspold").length;
      var actual_list = $("#count_list_old").val();
		  if(data !=actual_list){
        $('#load_page').show();
       ...
       function getsuppliertable(subproduct,tarnum){
        ...
        var subcode=subproducts[2];   
          ...
          $.ajax({
            ...
            success:function(data){
              if(data == 1){
                jAlert("Select Product and Sub-Product already exists in previous approval ")
                 $('#load_page').hide();
              }
              if(data == 0)
              {
 ...
 function ValidateSingleInput(oInput, file_ext) {
      ...
      //alert(oInput+" 1\n "+file_ext);
            ...
            var _validFileExtensions = [".jpg",".jpeg",".png",".gif",".pdf",".JPG",".JPEG",".PNG",".GIF"];
      //  alert(oInput+" 2\n "+file_ext);
        }
        if (oInput.type == "file") {
      ...
      //alert(sFileName+" 3\n "+file_ext);
          ...
          jAlert("Kindly Upload Only image file. Other Formats not allowed!!");
          $("#load_page").fadeOut("slow");
                    oInput.value = "";
                    return false;
                }

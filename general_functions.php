...
function add_ivuser($md_user, $alternate_user, $txt_approval_number, $maxarqcode) {
	$ivqry = 0;
	$sql_akiv = select_query_json("select COUNT(*) CNT from APPROVAL_request where aprnumb = '".$txt_approval_number."' and ARQSRNO=".$maxarqcode." AND (REQSTFR = ".$md_user." and REQSTBY <> 168)", "Centra", "TCS");
	if(count($sql_akiv) > 0) {

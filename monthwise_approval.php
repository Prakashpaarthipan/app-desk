...
/*$sql_reqid = select_query_json("select req.*, prj.APRNAME, top.ATCNAME, typ.ATYNAME, apm.APMNAME, regexp_replace(SubStr(nicname,1,4),'[0-9]','')||SubStr(nicname,5,10) branch, emp.EMPCODE, 
                                        emp.EMPNAME, ast.EXPSRNO, ast.DEPNAME, (select ADDUSER from trandata.APPROVAL_REQUEST@tcscentr where ARQCODE = req.ARQCODE and ARQYEAR = req.ARQYEAR and ARQSRNO = 1 
                                        and ATCCODE = req.ATCCODE and ATYCODE = req.ATYCODE and deleted = 'N' and rownum <= 1) addeduser, (select ARQYEAR from trandata.APPROVAL_REQUEST@tcscentr 
                                        ...
                                        rownum <= 1) ARYR, (select EMPCODE||' - '||EMPNAME from trandata.APPROVAL_REQUEST@tcscentr r1, trandata.employee_office@tcscentr e1 where e1.empsrno = r1.ADDUSER and 
                                        ...
                                        and rownum <= 1) addedempuser, (select DELUSER from trandata.APPROVAL_REQUEST@tcscentr where ARQCODE = req.ARQCODE and ARQYEAR = req.ARQYEAR and ARQSRNO = 1 and 
                                        ATCCODE = req.ATCCODE and ATYCODE = req.ATYCODE and deleted = 'N' and rownum <= 1) deltuser, (select EMPCODE||' - '||EMPNAME from trandata.APPROVAL_REQUEST@tcscentr r2, 
                                        trandata.employee_office@tcscentr e2 where e2.empsrno = r2.DELUSER and r2.ARQCODE = req.ARQCODE and r2.ARQYEAR = req.ARQYEAR and r2.ARQSRNO = 1 and 
                                        ...
                                        (select ATMNAME from trandata.approval_type_mode@tcscentr where atmcode = req.atmcode) ATMNAME   
from trandata.APPROVAL_REQUEST@tcscentr req, trandata.approval_type@tcscentr typ, trandata.approval_project@tcscentr prj, trandata.approval_topcore@tcscentr top, trandata.branch@tcscentr brn, trandata.approval_master@tcscentr apm, trandata.department_asset@tcscentr ast, 
                                        trandata.employee_office@tcscentr emp
                                    where req.ATYCODE = typ.ATYCODE and req.APRCODE = prj.APRCODE and req.ATCCODE = top.ATCCODE and req.APMCODE = apm.APMCODE and req.deleted = 'N' and 
                                        ...
                                        req.DEPCODE = ast.DEPCODE  and rownum<2
                                    order by req.ARQCODE, req.ARQSRNO, req.ATYCODE", "Centra", 'LIVE');*/
...
function budget_planner_summary($field_values,$tbl_name){
    ...
    if($insert_response == 1){
        ...
        print_r($field_values['BPLYEAR']);
        print_r($field_values['BPLNUMB']);
	}
	return 0;
}
...
if(isset($_POST['sbmt_request'])){
	...
	foreach($_POST['txt_prdcode'] as $key=>$rowdata){
		...
		$aprnumbData = select_query_json("select aprnumb,depcode,tarnumb,PURHEAD,REQSTBY,RESPUSR from approval_request where aprnumb like '%ADMIN / INFO TECH 4008116 / 19-03-2018 / 8116 / 11:25 AM%' and arqsrno=1","Centra", 'TEST');
		...
		$field_values['BPLREMA'] = strtoupper($_REQUEST['txt_prdspec'][$key]);
		$field_values['BPLPURP'] = strtoupper($_REQUEST['txt_prdspec'][$key]);
		...
		if(is_array($bpl_sum)){
			...
			$field_values['BPLDESC'] =  strtoupper($_REQUEST['txt_prdspec'][$key]);
    ...
    <div class="page-container page-navigation-top-fixed">
        ...
        <div class="page-content">
            ...
            <div class="page-content-wrap">
				<?php
					$aprnumbData = select_query_json("select aprnumb,depcode,tarnumb,PURHEAD,REQSTBY,RESPUSR,subcore from approval_request where aprnumb like '%ADMIN / INFO TECH 4008108 / 08-03-2018 / 8108 / 03:37 PM%' and arqsrno=1","Centra",'TEST');
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input type="hidden" class="form-control" name='function' id='function' tabindex="1" value='request_entry_1' />
                         ...
                         <input type="hidden" name="slt_subcore" id="slt_subcore" value="<?=$aprnumbData[0]['SUBCORE'];?>">
                                                ...
                                                '<div class="row" style="margin-right: -10px; display: flex;">'+
                                                    '<div class="col-sm-1 colheight" style="padding: 1px 0px;">'+
                                                        '<div class="fg-line">'+
                                                            ...
                                                            '<button class="btn btn-success btn-add3" id="addbtn_'+id+'" type="button" title="Add Suppliers" onclick="call_innergrid('+id+')" style="padding: 2px 5px;margin-right: 4px;"><span class="glyphicon glyphicon-plus"></span></button>'+
                                                            '<button id="removebtn_'+id+'" class="btn btn-remove btn-danger" type="button" title="Delete Suppliers" style="padding: 2px 5px; margin-right: 0px !important;" onclick="call_innergrid_remove('+id+')"><span class="glyphicon glyphicon-minus"></span></button>&nbsp;<input type="radio" checked="checked" name="txt_sltsupplier['+id+'][]" id="txt_sltsupplier_'+id+'_1" value="1" onclick="getrequestvalue('+id+', 1)" data-toggle="tooltip" data-placement="top" title="Select This Supplier" placeholder="Select This Supplier" class="calc">&nbsp;1'+
                                                        '</div>'+
                                                    '</div>'+

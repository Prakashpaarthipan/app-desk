...
if($_REQUEST['finstat'] == 'c') {
	...
	$sql_reqid = select_query_json("select ar.*, to_char(APPRSFR,'hh:mi:ss AM') APPRSFR_crt_Time, to_char(APPRSFR,'dd-MON-yyyy hh:mi:ss AM') APPRSFR_Time,
										...
										order by ARQCODE, ARQSRNO, ATYCODE", "Centra", 'TCS'); //  and APPSTAT = 'A' and FINSTAT = 'C'
}
	...
	<?php
		...
		$sql_hir = select_query_json("with t as (select apphead from approval_mdhierarchy
											where aprnumb in ('".$sql_approve_leads[0]['APRNUMB']."') and apphead in (1,2,3)) 
												select * from t pivot(count(APPHEAD)for(apphead) in (1 as KS,2 as PS,3 as AK))", 'Centra', 'TCS');
	?>
	<div class="page">
					...
					<td colspan=3 style='width:100%; height:20px; text-align:right;'>
						<label>Date : <?=$sql_reqid[0]['ADDDATE_DATE']?><br>
							Process Priority : <? if($sql_approve_leads[0]['PRICODE'] == '') {
									...
									<span class="badge <?=$clrcod?>" style="font-size:20px; font-weight:bold;"><? echo "AP-".$sql_process_priority[0]['PRICODE']; ?></span>
								<? } else { ?>
		...
		<td colspan=2 style='width:100%; vertical-align:top; text-align:left;'>
			<table border=0 style='width:100%; max-width: 773px; '>
				...
				<tr style='min-height:20px; line-height:20px;'>
					...
					<td style='width:80%; text-align:left;'>
						<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? echo $sql_reqid[0]['APMASTER']; /* if(trim($sql_approve_leads[0]['DYNSUBJ']) != '-') { echo rtrim($sql_approve_leads[0]['DYNSUBJ'], " - "); } /* if(trim($sql_approve_leads[0]['TXTSUBJ']) != '-') { echo $sql_approve_leads[0]['TXTSUBJ']; } */ ?></span></label> <!-- Approval Listings -->
					</td>
				</tr>
									...
									<table style="width: 100%; line-height: 15px;">
										<? $sql_prdlist = select_query_json("select * from APPROVAL_PRODUCTLIST
																					where PBDCODE = '".$sql_reqid[0]['IMUSRIP']."' and PBDYEAR = '".$sql_reqid[0]['ARQYEAR']."'
																						and REJUSER is null", 'Centra', 'TCS');
											if(count($sql_prdlist) > 0) {
												$inc = 0;
			...
			$sql_ap = select_query_json("select distinct aprnumb from approval_request
												where ARQCODE = '".$_REQUEST['reqid']."' and ARQYEAR = '".$_REQUEST['year']."' and ATCCODE = '".$_REQUEST['creid']."' and
													ATYCODE = '".$_REQUEST['typeid']."' and  deleted='N'", 'Centra', 'TCS');

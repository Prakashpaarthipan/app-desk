...
$_POST['function'] = 'signin';
$fn_selected_section_group="APPROVAL DESK";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' and $_POST['function'] == 'signin') {
	...
	if($valid_user != 1) {
		...
		if($fn_password !='')
		{
			...
			// echo "**".$resp."**";
			if($resp == 1) { // echo "::";
			echo "select * from userid where USRCODE = ".$_REQUEST['uname']."";
			...
			echo $_SESSION['websiteurl']		= "http://".$_SERVER['HTTP_HOST']."/";
			$redirect_url = '';
			if($hid_action == '') {
			...
			echo "select PRTCODE, SUPCODE, EMPSRNO, USRNAME, PASALOW, PARSECT, PARTSUP, USRSTAT, USRMOBL, ALLSECTION, BRNCODE
													from srm_userid where empsrno = '".$empcode[0]['EMPSRNO']."'";
			$response = '';
			...
			echo "select brn.brncode from branch brn
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' and $_POST['function'] == 'signin_test') {
	...
	//Sanitize input data using PHP filter_var().
	$uname = filter_var(trim($REQUEST["uname"]), FILTER_SANITIZE_STRING);
	//$password = filter_var(trim($_POST["password"]), FILTER_SANITIZE_STRING);
	// $password = filter_var(trim($_POST["password"]), FILTER_SANITIZE_EMAIL);
	...
	if($valid_user != 1) {
		...
		if($fn_password !='')
		{
			...
			$branch = rtrim($branch, ",");
			echo $_SESSION['tcs_all_allowed_branch'] = $branch;
			// print_r($response); exit;
			echo "select brn.BRNCODE from srm_userid brn
														where brn.DELETED = 'N' and empsrno=".$_SESSION['tcs_empsrno']." order by brn.BRNCODE";
			$sql_brnch1 = select_query_json("select brn.BRNCODE from srm_userid brn
														where brn.DELETED = 'N' and empsrno=".$_SESSION['tcs_empsrno']." order by brn.BRNCODE", "Centra", 'TCS');

	...
	for($keybrn = 0; $keybrn < count($slt_brnch); $keybrn++) { // branch for loop
		...
		//	echo "insert_appreq = ".$insert_appreq;
			...
			if($insert_appreq == 1 and $slt_approval_listings == 954)
			{
				...
				$lpdyear = $current_year[0]['PORYEAR'];
				$txt_srcfilename = "apd_".$lpdyear."_".$maxtagsrno[0]['MAXARQCODE']."_1.txt";
				...
				$creator = explode(" - ", $txtdynamic_creator);
				$assistby = explode(" - ", $txtcoordinate_assistby);
				$assist = explode(" - ", $txtdynamic_assist);
				$approvedby = explode(" - ", $txtdynamic_approvedby);

				$g_table = "approval_policy_form";
				$g_fld = array();//       								hid_apprsub

				$g_fld['APLCYCD'] = $maxtagsrno[0]['MAXARQCODE']; // srno auto generate
				$g_fld['APRNUMB'] = $apprno;
				$g_fld['PLCSUBJ'] = $txtdynamic_subject; // subject
				$g_fld['EFCTDAT'] = $txtdynamic_efct_date;
				$g_fld['VALDUPT'] = $txtdynamic_valid_upto;
				$g_fld['APPRDAT'] = $txtdynamic_valid_upto;
				$g_fld['PLCYTYP']  = $txtdynamic_policy_type;
				$g_fld['CRTECNO']  = $creator[0];
				$g_fld['CRTUSNM']  = $creator[1];
				$g_fld['CRDECNO']  = $assistby[0];
				$g_fld['CRDUSNM']  = $assistby[1];
				$g_fld['ASTECNO'] = $assist[0];
				$g_fld['ASTUSNM'] = $assist[1];
				$g_fld['USERLST'] = $txtdynamic_userlist;
				$g_fld['APRVDBY'] = $approvedby[0];
				$g_fld['APRVDUS'] = 'test'; //  error

				$g_fld['DESKPRO'] = $upload_img1; // doc
				$g_fld['PLCDATA'] = $MAINTENANCE_OF_POLICY; //
				$g_fld['PLCATTC'] = $txt_srcfilename;

	...
	for($keybrn = 0; $keybrn < count($slt_brnch); $keybrn++) { // branch for loop
		...
		// if(count($sql_reqexist) <= 0) {
			...
			if($insert_appreq == 1 and $slt_submission == 9 ){
				...
				$renewal_field['AGFRDAT'] = 1;
				$renewal_field['AGTODAT'] = 1;
		...
		$f_fld = array();
				...
				$f_fld['POLSUBJ'] = $txtdynamic_subject;				
				$f_fld['EFFDATE'] = $tar_date;
				$f_fld['POLTYPE'] = $txtdynamic_policy_type;
				$f_fld['VALUPTO'] = $due_date;
				$f_fld['CRENAME'] = $txtdynamic_creator;
                $f_fld['APPDATE'] = $app_date;
				$f_fld['COONAME'] = $txtdynamic_coordinator;
				$f_fld['USELIST'] = $txtdynamic_userlist;
				$f_fld['ASISTBY'] = $txtdynamic_asistby;
				$f_fld['DESKPRO'] = '0';
				$f_fld['POLIDOC'] = $txtdynamic_policy_docs;
				$f_fld['FILEPOL'] = '0';
				//print_r($g_fld);
		$f_insert = insert_dbquery($f_fld,$f_table);
		//print_r($g_fld);
			}
			//echo "<br>||";

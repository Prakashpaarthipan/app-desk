...
try{
	...
	$sql_apmaster = select_query_json("select SUBCORE, TOPCORE from APPROVAL_master 
												where apmcode > 965 and apmcode in (".$slt_approval_listings.") and SUBCORE = '".$slt_subcore."' and TOPCORE = '".$slt_topcore."'", "Centra", 'TEST');
	...
	for($keybrn = 0; $keybrn < count($slt_brnch); $keybrn++) { // branch for loop
		...
		// if(count($sql_reqexist) <= 0) {
			...
			if($insert_budmode) {
				...
				//PRAKASH PHP APPSTAT
				if($slt_approval_listings == 1906 or $slt_approval_listings == 1905 or $slt_approval_listings == 1907){
				...
				// if($slt_approval_listings == 1906 or $slt_approval_listings == 1905 or $slt_approval_listings == 1907){
				// 	$field_appreq['INTPFRD'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$product_rate_date_frm;
				// 	$field_appreq['INTPTOD'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$product_rate_date_to;
				// }
				...
				$field_appreq['EXPVALUE'] = $slt_expense_approval_listings;
				// Attachments
			...
			if($insert_appreq == 1){
				...
				$approval_timer_field['STRTIME'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;
					$hrs_add = strtotime($currentdate)+3600;
				...
				$approval_timer_field['EXTNDUR'] = 1;
				$approval_timer_field['SKIPAPR'] = 'Y';
				$t_insert = insert_test_dbquery($approval_timer_field,$approval_timer_table);


			}
			//Quotation renewal Date
			//if($insert_appreq == 1 and $slt_submission == 10 or $slt_submission == 11 ) and ($slt_approval_listings == 1906 or $slt_approval_listings == 1905 or $slt_approval_listings == 1907) )
			...
			{		
					...
					$t_insert = insert_test_dbquery($field_qutotin_fix,$quotatin_fix_table);
				}
			
			// Renewal Type Process
			if($insert_appreq == 1 and $slt_submission == 9 and $slt_approval_listings == 1491 ){
				...
				$renewal_insert = insert_test_dbquery($renewal_field,$renewal_table);
				echo '</br>';
			...
			if($insert_appreq == 1 and ($slt_approval_listings==909 or $slt_approval_listings==1368))
			{
				...
				if($txt_cmp_name !='')
				{
					...
					for($cmny = 0; $cmny < sizeof($txt_cmp_name);$cmny++)
					{
						...
						$ENTSRNO = select_query_json("select nvl(Max(ENTSRNO),0)+1 MAXARQCODE from approval_contractor_night_duty where  APRYEAR = '".$current_year[0]['PORYEAR']."' and APRNUMB = '".$apprno."' ","Centra","TEST");
						$sqlCompany = select_query_json("select  cmpname from approval_contractors where cmpcode = '".$txt_cmp_name[$cmny]."'", 'Centra', 'TEST');
						

							 if($_FILES['contractor_photo']['name'][$cmny] != null)
					      	{ 
					      	
					      		/*$fldimli = find_indicator( $_FILES['contractor_photo']['type'][$cmny] );
								$imgfile1 = $_FILES['contractor_photo']['tmp_name'][$cmny];
								if($fldimli == 'i')
								{
									...
									elseif ($info['mime'] == 'image/jpg') $image = imagecreatefrompng($imgfile1);
									//save it
								...
								$expl = explode(".", $_FILES['contractor_photo']['name'][$cmny]);
								...
								$source = $imgfile1;
								$complogos1 = preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $nameforserver); //str_replace(" ", "_", $upload_img1));
								$complogos1 = str_replace(" ", "-", $nameforserver);
								...
								{
									// echo "%%";
									$nameforserver_tmp = $current_year[0]['PORYEAR'].'_'.$ENTSRNO[0]['MAXARQCODE'].'_'.$txt_cmp_name[$cmny].'_'.$srno.'.'.strtolower($path_parts['extension']);
									$source_tmp = $imgfile1;
									$complogos1_tmp = preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $nameforserver_tmp); //str_replace(" ", "_", $upload_img1));
									$complogos1_tmp = str_replace(" ", "-", $nameforserver_tmp);
									...
									$resized_file = "../uploads/admin_projects_local/attachments/resize/". $complogos1_tmp;
									$dest_thumbfile = "../uploads/admin_projects_local/attachments/". $complogos1_tmp;
									//$dest_thumbfile = "approval_desk/contractor_night_duty/".$current_year[0]['PORYEAR']."/thumb_images/".$complogos1_tmp;
									imagejpeg($tmp1, $resized_file, 50);
									imagedestroy($image1);
									imagedestroy($tmp1);
									 
									move_uploaded_file($resized_file, $dest_thumbfile);
									 
									// exit;
									$local_file = "../uploads/admin_projects_local/attachments/".$complogos1_tmp;
									$server_file = 'approval_desk/contractor_night_duty/'.$current_year[0]['PORYEAR'].'/'.$complogos1_tmp;
									echo $server_file;
									if ((!$conn_id) || (!$login_result)) {
										$upload = ftp_put($ftp_conn, $server_file, $resized_file, FTP_BINARY);
													//echo "tmp Succes";
										echo "file uploaded";
										unlink($$resized_file);
							...
							$g_insert = insert_test_dbquery($g_fld_con,$g_table_contractor); 
							print_r($g_fld_con);
	...
	// echo "==".$insert_appreq."==";
	// exit();
	// PRAKASH PHP PRODUCT QUOTE REDIRECT
	if($insert_appreq == 1 and ($slt_approval_listings == 1906 or $slt_approval_listings == 1907 or $slt_approval_listings == 1905)){
		
		
		$redirect_url1 = "expense_entry_list.php?action=request&ap=".$apprno."&brn=".$slt_branch."";
								//$json = json_encode(array('type' => 'success', "info" => $redirect_url, "msg" => "Page will redirect now"));
								
									//echo $redirect_url1;
			echo '<script type="text/javascript">document.location.href="'.$redirect_url1.'";</script>';
									//echo '<META HTTP-EQUIV="refresh" content="0" URL='.$redirect_url1.'>';

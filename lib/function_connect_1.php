...
// class db_connect{
		...
		if($valid_user != 1) { 
			...
			if($fn_password !='')
			{
				...
				if($resp == 1) {
					...
					$json = json_encode(array('type' => 'error', "info" => '', "msg" => $response1));
				}
	...
	try{
		// echo "!=".$hidrequest_value."!=".$txtrequest_value."!=CA1ME";
		...
		for($keybrn = 0; $keybrn < count($slt_brnch); $keybrn++) { // branch for loop
			...
			// if(count($sql_reqexist) <= 0) {
				...
				// exit;
				// $insert_budmode = 1;
				// echo "****".$insert_budmode."+++".$hid_newentry."---".$slt_submission."***"; print_r($hid_appuser);
				$amrsrno = 0;
				if($insert_budmode) {
					...
					/* echo "<br>***";
					print_r($advarray);
					echo "<br>***".$slt_approval_listings."***".count($advance_array)."****"; */
				...
				if($insert_appreq == 1 and ($slt_approval_listings == 669 or  $slt_approval_listings == 6 or $slt_approval_listings == 829 or $slt_approval_listings == 668 or $slt_approval_listings == 725 or $slt_approval_listings == 712 or $slt_approval_listings == 710 or $slt_approval_listings == 711 or $slt_approval_listings == 843 ) )
				{
				...
				if($insert_appreq == 1 and ($slt_approval_listings == 8 or $slt_approval_listings == 671 or $slt_approval_listings == 715 or $slt_approval_listings == 714 or $slt_approval_listings == 670 or $slt_approval_listings == 716))
				{
					...
					for($pdi = 0; $pdi < count($txt_prdcode); $pdi++) {
						if($txt_prdcode[$pdi] != '') {
							...
							$maxprlstno = select_query_json("Select nvl(Max(PRLSTNO),0)+1 MAXPRLSTNO 
																	...
																	WHERE PBDYEAR = '".$current_year[0]['PORYEAR']."' and PBDCODE = ".$srno." and PRLSTYR = '".$current_year[0]['PORYEAR']."' and PRLSTNO = '".$maxprlstno[0]['MAXPRLSTNO']."'", "Centra", 'TCS'); 
							...
							// echo "<br>IN".$pdi."IN"; print_r($field_appdet); echo "<br>"; ****0+++1---7***2676~~14413~~1118~~1657~~17108~~2546~~1986~~1~~====
							...
							for($qdi = 0; $qdi < count($txt_sltsupcode[$pdii]); $qdi++) { // echo "***";
								...
								$fldimli = '-'; $complogos1 = '-'; $uploadimg = $current_year[0]['PORYEAR'];

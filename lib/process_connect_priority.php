...
try{
	...
	for($keybrn = 0; $keybrn < count($slt_brnch); $keybrn++) { // branch for loop
		...
		// if(count($sql_reqexist) <= 0) {
			...
			if($insert_appreq == 1)
			{
				/* // Approval Timer approval_timer insert for Monitoring and skip approvals
				...
				// Approval Timer approval_timer insert for Monitoring and skip approvals
				$tbl_tags = "approval_timer";
				$field_tags = array();
			 	$field_tags['APRNUMB'] = $apprno;
			 	$field_tags['ARQSRNO'] = 1;
			 	$field_tags['STRTIME'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;
			 	$field_tags['ENDTIME'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$endtime;
			 	$field_tags['EXTNTIM'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$endtime;
			 	$field_tags['EXTNDUR'] = $sql_duration[0]['RGFLWTM'];
			 	$field_tags['SKIPAPR'] = $sql_duration[0]['SKIPAPR'];
				$field_tags['TIMSTAT'] = 'N';
				$field_tags['EDTUSER'] = '';
				$field_tags['EDTDATE'] = '';
				$field_tags['REMARKS'] = '';
				// print_r($field_tags);

...
if ($function == 'signin') {
	...
	$uname = filter_var(trim($_REQUEST['uname']), FILTER_SANITIZE_STRING);
	//$password = filter_var(trim($_REQUEST['password']), FILTER_SANITIZE_STRING);
	// $password = filter_var(trim($_REQUEST['password']), FILTER_SANITIZE_EMAIL);
	...
	$fn_username = isset($_REQUEST['uname']) ? ($_REQUEST['uname']) : "";
	//$fn_password = isset($_REQUEST['password']) ? ($_REQUEST['password']) : "";
	//$fn_rememberme = isset($_POST['rememberme']) ? ($_POST['rememberme']) : "";
	$fn_selected_section_group = isset($_REQUEST['section']) ? ($_REQUEST['section']) : "APPROVAL DESK";
    //echo $fn_selected_section_group;
	...
	if($valid_user != 1) {
		...
		$_SESSION['websiteurl']		= "http://".$_SERVER['HTTP_HOST']."/";
			...
			if($hid_action == '') {
				...
				{
					case 'SYSTEM':
							$redirect_url = $_SESSION['websiteurl'].'/home.php?sltmdul=SYSTEM';
							break;
					case 'ADMIN':
							$redirect_url = $_SESSION['websiteurl'].'/home.php?sltmdul=ADMIN';
							break;
					case 'PURCHASE':
							$redirect_url = $_SESSION['websiteurl'].'/home.php?sltmdul=PURCHASE';
							break;
					case 'CRM':
							$redirect_url = $_SESSION['websiteurl'].'/home.php?sltmdul=CRM';
							break;
					case 'JEWELLERY':
							$redirect_url = $_SESSION['websiteurl'].'ktmportal/index.php';
							break;
					case 'APPROVAL DESK':
			...
			if($response[0]['SUPCODE'] > 0 and $resvalue == '') {
				...
				echo "select distinct ma.mnucode, mainmenu, submenu from srm_menu_access ma, srm_menu mu
															where mu.mnucode = ma.mnucode and (ma.SUPCODE = '".$resvalue."' or ma.ENTSRNO = '".$resvalue."')
															order by ma.mnucode Asc";
				for($ij = 0; $ij < count($sql_menuaccess); $ij++)
				{
					$mainmenu_access[] = $sql_menuaccess[$ij][1];
					$submenu_access[] = $sql_menuaccess[$ij][2];
				}
				echo $_SESSION['tcs_mainmenu_access'] = $mainmenu_access;
				echo $_SESSION['tcs_submenu_access'] = $submenu_access;
				echo $rights = find_user_rights(strtoupper($fn_selected_section_group), $empcode[0]['EMPSRNO']);
...
if($jsondecode[type]=='success'){
	echo $jsondecode[info];
	if($fn_selected_section_group=='APPROVAL DESK')
	header('Location:../'.$jsondecode[info]);
    else
    	header('Location:'.$jsondecode[info]);
}
/* Output header */

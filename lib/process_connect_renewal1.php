...
try{
	...
	for($keybrn = 0; $keybrn < count($slt_brnch); $keybrn++) { // branch for loop
		...
		if($_FILES['txtdynamic_deskprocedure']['name'][0] != '') {
			$assign=$_FILES['txtdynamic_deskprocedure']['name'];
			$noofattachment += count($_FILES['txtdynamic_deskprocedure']['name']);
		}
		...
		// if(count($sql_reqexist) <= 0) {
			...
			if($insert_budmode) {
				...
				if($txt_dynamic_subject21 != '') {
					$field_appreq['DYNSUBJ'] = " ".strtoupper($txt_dynamic_subject11)." - ".strtoupper($txt_dynamic_subject21)." - ";
				} else {
					$field_appreq['DYNSUBJ'] = " ".strtoupper($txt_dynamic_subject11)." - ";
				}
				//$field_appreq['DYNSUBJ'] = " ".strtoupper($txt_dynamic_subject21); //renewal date check
				...
				$field_appreq['RMBDAPR'] = strtoupper($txtdynamic_deskprocedure_remarks);
		...
		if($insert_appreq == 1 and ($slt_approval_listings==955 or $slt_approval_listings==954 ))
			{
				...
				// Product Image
				if($_FILES['attachments']['type'][$pdi] == "image/jpeg" or $_FILES['attachments']['type'][$pdi] == "image/gif" or $_FILES['attachments']['type'][$pdi] == "image/png" or $_FILES['attachments']['type'][$pdi] == "application/pdf") {
						$fldimli = find_indicator( $_FILES['attachments']['type'][$pdi] );
						$imgfile1 = $_FILES['attachments']['tmp_name'][$pdi];
						if($fldimli == 'i')
						{
						...
						switch($_FILES['attachments']['type'][$pdi]) {
							case 'image/jpeg':
							case 'image/jpg':
							case 'image/gif':
							case 'image/png':
						...
						$expl = explode(".", $_FILES['attachments']['name'][$pdi]);
			...
			$f_fld['DESKPRO'] =  $complogos1;
			$f_fld['PLCATTC'] =  $txtdynamic_policy_docs;
			
				$description1 = $_REQUEST['FCKeditor2'];
			$lpdyear = $current_year[0]['PORYEAR'];
			$txt_srcfilename1 = "apd_".$lpdyear."_".$srno."_1.txt";
			// $txt_srcfilename = "apd_2018-19_4008105_1.txt";

			$local_file = "uploads/text_approval_policy/".$txt_srcfilename1;
			$myfile = fopen($local_file, "w");
			fwrite($myfile, $description1);
			fclose($myfile);

			$server_file = 'approval_desk/text_approval_policy/'.$lpdyear.'/'.$txt_srcfilename1;
			if ((!$conn_id) || (!$login_result)) {
				$upload = ftp_put($ftp_conn, $server_file, $local_file, FTP_BINARY);
				//unlink($local_file);
			}
			...
			$f_fld['PLCDATA'] = str_replace("'", "", $lpdyear.'/'.$txt_srcfilename1);
		...
		//	echo "insert_appreq = ".$insert_appreq;
			...
			if($insert_appreq == 1 and ($slt_approval_listings == 669 or  $slt_approval_listings == 6 or $slt_approval_listings == 829 or $slt_approval_listings == 668 or $slt_approval_listings == 725 or $slt_approval_listings == 712 or $slt_approval_listings == 710 or $slt_approval_listings == 711 or $slt_approval_listings == 1274 or $slt_approval_listings == 1275 or $slt_approval_listings == 1276 or $slt_approval_listings == 1280 or $slt_approval_listings == 1281 ) )
			{
				...
				for($emp = 0; $emp < sizeof($EMPNAME);$emp++)
				{
					...
					print_r($g_fld);
					echo "+++++++++".$g_insert = insert_dbquery($g_fld,$g_table);
				}
			}
			exit;
			...
			// print_r($_FILES['txtdynamic_deskprocedure']);
			...
			if($insert_appreq == 1) {
				...
				for($i=0; $i<count($assign); $i++)
				{
					...
					{
						$fldimli = '-'; $complogos1 = '-';
						if($_FILES['txtdynamic_deskprocedure']['type'][$i] == "image/jpeg" or $_FILES['txtdynamic_deskprocedure']['type'][$i] == "image/gif" or $_FILES['txtdynamic_deskprocedure']['type'][$i] == "image/png" or $_FILES['txtdynamic_deskprocedure']['type'][$i] == "application/pdf") {
							$fldimli = find_indicator( $_FILES['txtdynamic_deskprocedure']['type'][$i] );

							$imgfile1 = $_FILES['txtdynamic_deskprocedure']['tmp_name'][$i];
							if($fldimli == 'i')
							{
								$info = getimagesize($imgfile1);
								$image1 = imagecreatefromjpeg($imgfile1);
							...
							switch($_FILES['txtdynamic_deskprocedure']['type'][$i]) {
								case 'image/jpeg':
								case 'image/jpg':
								case 'image/gif':
								case 'image/png':
							...
							// $upload_img1 = $_FILES['txtdynamic_deskprocedure']['name'];
							$expl = explode(".", $_FILES['txtdynamic_deskprocedure']['name'][$i]);
			  ...
			  /*
				for($i3=0; $i3<count($assign3); $i3++)
				{
					...
					{
						$lstapri = '-'; $complogos8 = '-';
						if($_FILES['txtdynamic_deskprocedure']['type'][$i3] == "image/jpeg" or $_FILES['txtdynamic_deskprocedure']['type'][$i3] == "image/gif" or $_FILES['txtdynamic_deskprocedure']['type'][$i3] == "image/png" or $_FILES['txtdynamic_deskprocedure']['type'][$i3] == "application/pdf") {
							$lstapri = find_indicator( $_FILES['txtdynamic_deskprocedure']['type'][$i3] );

							$imgfile8 = $_FILES['txtdynamic_deskprocedure']['tmp_name'][$i3];
							if($lstapri == 'i')
							{
							...
							switch($_FILES['txtdynamic_deskprocedure']['type'][$i3]) {
								case 'image/jpeg':
								case 'image/jpg':
								case 'image/gif':
								case 'image/png':
							...
							$expl = explode(".", $_FILES['txtdynamic_deskprocedure']['name'][$i3]);

...
<form class="form-horizontal" role="form" id="frm_requirement_entry" name="frm_requirement_entry" action="call_center_summary1.php" method="post" enctype="multipart/form-data">
                            ...
                            <div class="panel panel-default tabs">                            
                                    ...
                                    <div style="height: 700px;overflow-x: scroll;overflow-y: scroll;">
                                        <table  class="table  table-striped" name="copy_pre" >
                                            ...
                                            <tbody>
                                            <?  
                                            $sql_search = select_query_json("select ATYNAME, TARNUMB, apmcode, APMNAME, decode(SUBCORE, -1, 'TEXTILE', -2, 'READY MADE', ESENAME) DEPT, atc.atcname from trandata.approval_master@tcscentr mas, trandata.approval_type@tcscentr typ, trandata.empsection@tcscentr sub, trandata.approval_topcore@tcscentr atc where ROWNUM<=10 AND atc.atccode = mas.topcore and mas.atycode = typ.atycode and sub.esecode(+) = mas.SUBCORE and sub.deleted(+) = 'N' and mas.deleted = 'N' and typ.deleted = 'N' and ( apmcode = 954 or apmcode = 955 or apmcode = 888 or apmcode = 856 or apmcode > 965 ) and tarnumb>9000 order by ATYNAME, TARNUMB, DEPT, APMNAME, apmcode", "Centra", 'TCS');
                                           ...
                                           // echo("</pre>");
                                            ...
                                            {   $k++;
                                                ...
                                                {
                                                     $sql_brn = select_query_json("select APR.BRNCODE,apr.EMPCODE||'-'||EMPNAME NAME from trandata.approval_branch_head apr, trandata.branch brn where  apr.brncode = brn.brncode and apr.deleted = 'N' and brn.deleted = 'N' AND apr.tarnumb ='".$value[0]['TARNUMB']."'  and apr.APRVALU > 100000 GROUP BY apr.BRNCODE, apr.TARNUMB, apr.EMPSRNO, apr.EMPCODE, apr.EMPNAME, apr.brncode,apr.brnhdsr Order by apr.brncode,apr.brnhdsr", "Centra", 'TEST');
                                                }
                                                else{
                                                ...
                                                <tr class="odd gradeX" style='background-color:<?=$bgclr?>; color:<?=$clr?>;'>
                                                    ...
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="EKM">
                                                        <span id="td_10_<?=$value[0]['TARNUMB']?>" onclick="loaduser('10','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[10]);$i++)
                                                            {    $temp=explode('-',$arr_brn[10][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[10][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[10][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_10_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <!-- ///////////////////// -->
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="MDU">
                                                        <span id="td_14_<?=$value[0]['TARNUMB']?>" onclick="loaduser('14','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[14]);$i++)
                                                            {    $temp=explode('-',$arr_brn[14][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[14][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[14][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_14_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="DGL">
                                                        <span id="td_23_<?=$value[0]['TARNUMB']?>" onclick="loaduser('23','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[23]);$i++)
                                                            {    $temp=explode('-',$arr_brn[23][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[23][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[23][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_23_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="AIR-HYD">
                                                        <span id="td_107_<?=$value[0]['TARNUMB']?>" onclick="loaduser('107','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[107]);$i++)
                                                            {    $temp=explode('-',$arr_brn[107][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[107][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[107][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_107_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="AIT-MDU">
                                                        <span id="td_112_<?=$value[0]['TARNUMB']?>" onclick="loaduser('112','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[112]);$i++)
                                                            {    $temp=explode('-',$arr_brn[112][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[112][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[112][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_112_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="TAILYOU">
                                                        <span id="td_114_<?=$value[0]['TARNUMB']?>" onclick="loaduser('114','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[114]);$i++)
                                                            {    $temp=explode('-',$arr_brn[114][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[114][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[114][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_114_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="KKV">
                                                        <span id="td_115_<?=$value[0]['TARNUMB']?>" onclick="loaduser('115','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[115]);$i++)
                                                            {    $temp=explode('-',$arr_brn[115][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[115][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[115][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_115_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="AIR-CHN">
                                                        <span id="td_116_<?=$value[0]['TARNUMB']?>" onclick="loaduser('116','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[116]);$i++)
                                                            {    $temp=explode('-',$arr_brn[116][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[116][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[116][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_116_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="SCM">
                                                        <span id="td_117_<?=$value[0]['TARNUMB']?>" onclick="loaduser('117','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[117]);$i++)
                                                            {    $temp=explode('-',$arr_brn[117][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[117][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[117][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_117_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="CLT">
                                                        <span id="td_120_<?=$value[0]['TARNUMB']?>" onclick="loaduser('120','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[120]);$i++)
                                                            {    $temp=explode('-',$arr_brn[120][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[120][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[120][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_120_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="TUP-KTM">
                                                        <span id="td_201_<?=$value[0]['TARNUMB']?>" onclick="loaduser('201','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[201]);$i++)
                                                            {    $temp=explode('-',$arr_brn[201][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[201][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[201][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_201_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="MDU-TJ">
                                                        <span id="td_203_<?=$value[0]['TARNUMB']?>" onclick="loaduser('203','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[203]);$i++)
                                                            {    $temp=explode('-',$arr_brn[203][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[203][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[203][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_203_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="TUP-TJ">
                                                        <span id="td_204_<?=$value[0]['TARNUMB']?>" onclick="loaduser('204','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[204]);$i++)
                                                            {    $temp=explode('-',$arr_brn[204][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[204][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[204][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_204_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="DGL-TJ">
                                                        <span id="td_206_<?=$value[0]['TARNUMB']?>" onclick="loaduser('206','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[206]);$i++)
                                                            {    $temp=explode('-',$arr_brn[206][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[206][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[206][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_206_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                    <td style='text-align:left;width: 10%;' data-toggle="tooltip" data-placement="top" title="COR">
                                                        <span id="td_888_<?=$value[0]['TARNUMB']?>" onclick="loaduser('888','<?=$value[0]['TARNUMB']?>');">
                                                            <?$exist=array();$user='';
                                                            for($i=0;$i<count($arr_brn[888]);$i++)
                                                            {    $temp=explode('-',$arr_brn[888][$i]['NAME']);
                                                                if(!in_array($temp[0], $exist)){?>
                                                                <div class="row" style="padding-top: 4px;">
                                                                    <? $user.=$arr_brn[888][$i]['NAME'].'~~';
                                                                        
                                                                        $exist[$i]=$temp[0];
                                                                    echo $arr_brn[888][$i]['NAME']; ?>
                                                                </div>
                                                                <?}}?>
                                                            <input type="hidden" id="users_888_<?=$value[0]['TARNUMB']?>" value="<?=$user;?>">
                                                        </span>
                                                    </td>
                                                </tr>
   ...
   <div id="myModal1" class="modal fade">
            <div class="modal-dialog" style='width:85%'>
                <div class="modal-content">
                    <div class="modal-body" id="modal-body1">some text</div>
                </div>
            </div>
        </div>
    <!-- Collect Document -->

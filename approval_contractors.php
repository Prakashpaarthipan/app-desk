...
if($_SERVER['REQUEST_METHOD']=="POST" and $_REQUEST['sbmt_update'] != '') 
{
   ...
   //$sql_empcode = select_query_json("select EMPCODE,EMPSRNO from employee_office where EMPCODE='".$expl[0]."'");
	...
	$tbl_appreq = "approval_contractor";
	$field_appreq = array();
...
//exit
	if($insert_appreq == 1) { ?>
		<script>window.location='approval_contractors.php?status=update_success';</script>
		<?php
		exit();
	} else { ?>
		<script>window.location='approval_contractors.php?action=edit&reqid=<?=$hid_reqid?>&status=failure';</script>
		<?php
		exit();
	}
}
...
//echo ("$sql_empcode");
	...
	// Insert in APPROVAL_email_master Table
	$max_val = select_query_json("select count(*) max from approval_contractor", "Centra", 'TEST');
	$tbl_appreq = "approval_contractor";
	$field_appreq = array();
...
//exit;
	...
	if($insert_appreq == 1) { ?>
		<script>window.location='approval_contractors.php?status=success';</script>
		<?php
		exit();
	} else { ?>
		<script>window.location='approval_contractors.php?action=add&status=failure';</script>
		<?php
		exit();
	}
}
...
if($_REQUEST['action'] == 'deleted')
{
	...
	// Update in APPROVAL_email_master Table
if($insert_appreq == 1) { ?>
		<script>window.location='approval_contractors.php?status=delete_success';</script>
		<?php
		//exit();
	} else { ?>
		<script>window.location='approval_contractors.php?status=failure';</script>
		<?php
		//exit();
	}
}
...
<head>
    <!-- META SECTION -->
    <title><?=$title_tag?> Requirement Entry :: Night Approval :: <?php echo $site_title; ?></title>
...
#text
{
    ...
    background-color:#EFEFEF;
			...
			$sql_reqid = select_query_json("select * from approval_contractor where CMPSRNO='".$_REQUEST['reqid']."'","Centra",'TEST');
...
<!-- contact person -->
                    ...
                    <div class="col-md-9 col-xs-12">
                    <? if($_REQUEST['action'] == 'view') { ?>
						      :<b><input id="text" name='cmp_contact' id='cmp_contact' value='<?=$sql_reqid[0]['CONTPER']?>' size="70" disabled></b><br><br>
                    <? } else { ?>
							 <input class="form-control" placeholder="ENTER THE PERSON NAME" tabindex='4' required autocomplete="on" maxlength='40' name='cmp_contact' id='cmp_contact' value='<?=$sql_reqid[0]['CONTPER']?>' data-toggle="tooltip" data-placement="top" title="Contact Person"><br><br>
				 <? } ?>
                    </div>
                    </div>
<!-- contact person -->
...
<!-- address -->
                    ...
                    <div class="col-md-9 col-xs-12">
                    <? if($_REQUEST['action'] == 'view') { ?>
							 :<b><input id="text" name='cmp_addr' id='cmp_addr' value='<?=$sql_reqid[0]['CMPADDR']?>' size="70" disabled></b><br><br>
				   <? } else { ?>
                              <input class="form-control" placeholder="ENTER THE ADDRESS" tabindex='4' required autocomplete="on" maxlength='50' name='cmp_addr' id='cmp_addr' value='<?=$sql_reqid[0]['CMPADDR']?>' data-toggle="tooltip" data-placement="top" title="Address"><br><br>
				  <? } ?>
				  </div>
                  </div>
<!-- address -->
...
<!-- mobile  -->
                    ...
                    <div class="col-md-9 col-xs-12">
                    <? if($_REQUEST['action'] == 'view') { ?>
                              :<b><input id="text" name='cmp_mobile' id='cmp_mobile' value='<?=$sql_reqid[0]['MOBILE']?>' size="70" disabled></b><br><br>

					<? } else { ?>
							   <input class="form-control" placeholder="ENTER THE MOBILE NUMBER" tabindex='4' required autocomplete="on" maxlength='10' name='cmp_mobile' id='cmp_mobile' value='<?=$sql_reqid[0]['MOBILE']?>' data-toggle="tooltip" data-placement="top" title="Mobile"><br><br>
				   <? } ?>
				   </div>
                   </div>
<!--mobile-->
...
<!--phone-->
                   ...
                   <div class="col-md-9 col-xs-12">
                   <? if($_REQUEST['action'] == 'view') { ?>
                      :<b><input id="text" name='cmp_phone' id='cmp_phone' value='<?=$sql_reqid[0]['PHONE']?>' size="70" disabled></b><br><br>

				   <? } else { ?>
                             <input class="form-control" placeholder="ENTER THE PHONE NUMBER" tabindex='4' required autocomplete="on" maxlength='15' name='cmp_phone' id='cmp_phone' value='<?=$sql_reqid[0]['PHONE']?>' data-toggle="tooltip" data-placement="top" title="Phone"><br><br>
                   <? } ?>                 
                   </div>
                   </div>
<!--phone-->
...
<!--email-->
                    ...
                    <div class="col-md-9 col-xs-12">
                    <? if($_REQUEST['action'] == 'view') { ?>
                              :<b><input id="text" name='cmp_email' id='cmp_email' value='<?=$sql_reqid[0]['EMAIL']?>' size="70" disabled></b>

					<? } else { ?>
							  <input class="form-control" placeholder="ENTER THE EMAIL ID" tabindex='4' required autocomplete="on" maxlength='40' name='cmp_email' id='cmp_email' value='<?=$sql_reqid[0]['EMAIL']?>' data-toggle="tooltip" data-placement="top" title="Email"><br><br>
				    <? } ?>                 
                    </div>
                    </div>
				    </div>
...
<!-- Email ID -->
				...
				<div class="form-group trbg" style='min-height:40px; padding-top:10px'>
					<div class="col-lg-12 col-md-12" style=' text-align:center; padding-right:10px;'>
						<? if($_REQUEST['action'] == 'view') { ?><a href='approval_contractors.php'><i class="fa fa-refresh"></i> Back</a><? } elseif($_REQUEST['action'] == 'edit') { ?>
		...
		<!-- /.row -->
			...
			<div class="panel panel-default">
					<div class="panel-heading">
				<h2>	<i class="fa fa-check-circle-o fa-fw"></i>Approval Contractors</h2>
						<div class="pull-right">
							<div class="btn-group">
								...
								<ul class="dropdown-menu pull-right" role="menu">
									<li><a href="approval_contractors.php?action=add">Add</a></li>
								</ul>
							</div>
						</div>
					</div>
	...
	<tr>
						<? 	$sql_project = select_query_json("select * from approval_contractor"); 
					...
					<td style='text-align:center'><?echo $sql_project[$project_i]['CMPSRNO'];?></td>
						...
						<td><?=$sql_project[$project_i]['CMPADDR']?></td>
						<td><?=$sql_project[$project_i]['CONTPER']?></td>
						<td style='text-align:center'><?=$sql_project[$project_i]['MOBILE']?></td>
						<td><?=$sql_project[$project_i]['PHONE']?></td>
						<td><?=$sql_project[$project_i]['EMAIL']?></td>
						<td style='text-align:center'><a href='approval_contractors.php?action=edit&reqid=<? echo $sql_project[$project_i]['CMPSRNO']; ?>' title='Edit' alt='Edit'><i class="fa fa-edit"></i> Edit</a> / <a href='approval_contractors.php?action=view&reqid=<? echo $sql_project[$project_i]['CMPSRNO']; ?>' title='View' alt='View'><i class="fa fa-eye"></i> View</a> / <a href='javascript:void(0)' id="delete_confirm" onclick="call_confirm(<?=($project_i+1)?>, 'approval_contractors.php?action=deleted&reqid=<? echo $sql_project[$project_i]['CMPSRNO']; ?>')" title='Delete' alt='Delete'><i class="fa fa-trash-o"></i> Delete</a></td>

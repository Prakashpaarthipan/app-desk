...
if($_SERVER['REQUEST_METHOD']=="POST" and $_REQUEST['sbmt_update'] != '') 
{
    ...
    $sql_empcode = select_query_json("select EMPCODE,EMPSRNO from employee_office where EMPCODE='".$txt_empcode."'");
	...
	// Update in APPROVAL_email_master Table
	if($insert_appreq == 1) { ?>
		<script>window.location='am.php?';</script>
		<?php
		exit();
	} else { ?>
		<script>window.location='am.php?action=edit&reqid=<?=$hid_reqid?>';</script>
		<?php
		exit();
	}
}
if($_SERVER['REQUEST_METHOD']=="POST" and $_REQUEST['sbmt_request'] != '') 
{
  ...
  //  print_r($field_appreq);
    ...
    if($insert_appreq == 1) { ?>
        ...
        <script>window.location='am.php?action=add';</script>
        <?php
        exit();
    }
}
...
$insert_appreq = update_test_dbquery($field_appreq, $tbl_appreq, $where_appreq);
	...
	if($insert_appreq == 1) { ?>
		<script>window.location='am.php?status=delete_success';</script>
		<?php
		//exit();
	} else { ?>
		<script>window.location='am.php?status=failure';</script>
		<?php
		//exit();
	}
}
...
<body>
    ...
    <div class="page-container page-navigation-toggled page-container-wide page-navigation-top-fixed">
        ...
        <div class="page-content">
           ...
           <? {  ?>
            ...
            <div class="row">
               ...
               <center><h1 class="page-header">TailYou::Employee Management</h1></center>
	    ...
	    $sql_reqid2 = select_query_json("select PRSCODE from tlu_employee_rights  where EMPCODE = '".$_REQUEST['reqid']."' order by EMPCODE");
...
<!-- process -->
	              ...
	              <input class="form-control" tabindex='6' style="text-transform: uppercase;" name='txt_empcode' id='txt_empcode' data-toggle="tooltip" onblur="find_tags();" data-placement="top" data-original-title="Assign member" value='<?=$sql_reqid[0]['EMPCODE']." - ".$sql_reqid1[0]['EMPNAME']?>'>
					...
					<div class="form-group trbg">
						...
						<input type='hidden' name='txt_prscode' id='txt_prscode' value='<?=$sql_reqid[0]['PRSCODE']?>'>
							...
							<? } else { ?>
						<select class="form-control" tabindex='1' required name='txt_prscode' id='txt_prscode' autofocus required data-toggle="tooltip" data-placement="top" title="Process Code" value='<?=$sql_reqid[0]['PRSCODE']?>'>
				...
				<div class="form-group trbg" style='min-height:40px; padding-top:10px'>
					<div class="col-lg-12 col-md-12" style=' text-align:center; padding-right:10px;'>
						<? if($_REQUEST['action'] == 'view') { ?><a href='am.php'><i class="fa fa-refresh"></i> Back</a><? } elseif($_REQUEST['action'] == 'edit') { ?>
						...
						<div class="pull-right">
							<div class="btn-group">
								...
								<ul class="dropdown-menu pull-right" role="menu">
									<li><a href="am.php?action=add">Add</a></li>
								</ul>
							</div>
						</div>
					</div>

...
$sql_search = select_query_json("select distinct ar.APRNUMB, ar.APPSTAT, ar.RQFRDES, ar.APPRSUB, ar.APPRFOR, ar.APPFVAL AVAL, ar.APRTITL, max(ar.ARQSRNO) ARQSRNO, ar.ARQCODE, ar.ATYCODE, ar.ATCCODE,
                                            ...
                                            regexp_replace(SubStr(brn.nicname,1,4),'[0-9]','')||SubStr(nicname,5,10) branch, AR.INTPEMP, ar.pricode||' - '||pri.priname priority, ar.AUTFRWD
                                        ...
                                        group by ar.APRNUMB, ar.APPSTAT, ar.APPRSUB, ar.RQFRDES, ar.APPRFOR, ar.APPFVAL, ar.APRTITL, ar.ARQCODE, ar.ATYCODE, ar.ATCCODE, ar.ARQYEAR, ar.ADDUSER, 
                                            ...
                                            ar.REQSTFR, AR.INTPEMP, ar.AUTFRWD 
                                    union
                                        select distinct ar.APRNUMB, ar.APPSTAT, ar.RQFRDES, ar.APPRSUB, ar.APPRFOR, ar.APPFVAL AVAL, ar.APRTITL, max(ar.ARQSRNO) ARQSRNO, ar.ARQCODE, ar.ATYCODE, 
                                            ...
                                            ar.pricode||' - '||pri.priname priority, ar.AUTFRWD 
                                        from APPROVAL_REQUEST ar, branch brn, approval_priority pri
                                        where brn.brncode = ar.brncode and pri.pricode(+) = ar.pricode and pri.deleted(+) = 'N' and ar.DELETED = 'N' 
...
/*echo("select distinct ar.APRNUMB, ar.APPSTAT, ar.RQFRDES, ar.APPRSUB, ar.APPRFOR, ar.APPFVAL AVAL, ar.APRTITL, max(ar.ARQSRNO) ARQSRNO, ar.ARQCODE, ar.ATYCODE, ar.ATCCODE,
                                            ar.ARQYEAR, ar.ADDUSER, ar.RQESTTO, decode(ar.APPSTAT, 'N', 'NEW', 'F', 'FORWARD', 'A', 'APPROVED', 'P', 'PENDING', 'Q', 'QUERY', 'S', 'RESPONSE',
                                            'C', 'COMPLETED', 'R', 'REJECTED', 'NEW') APPSTATUS, decode(ar.APPSTAT, 'N', '1', 'F', '2', 'A', '3', 'P', '4', 'Q', '5', 'S', '6', 'C', '7', 
                                            'R', '8', '9') APPORDER, (select APPSTAT from APPROVAL_REQUEST where ARQSRNO = 1 and DELETED = 'N' and ARQCODE = ar.ARQCODE and ARQYEAR = ar.ARQYEAR and 
                                            ATCCODE = ar.ATCCODE and ATYCODE = ar.ATYCODE and ATMCODE = ar.ATMCODE and APMCODE = ar.APMCODE and APPSTAT = 'A') as APSTAT, ar.ADDDATE, (select ADDDATE
                                    ...
                                    union
                                        ...
                                        group by ar.APRNUMB, ar.APPSTAT, ar.APPRSUB, ar.RQFRDES, ar.APPRFOR, ar.APPFVAL, ar.APRTITL, ar.ARQCODE, ar.ATYCODE, ar.ATCCODE, ar.ARQYEAR, ar.ADDUSER, 
                                            ar.ADDDATE, ar.RQESTTO, ar.ATMCODE, ar.TARDESC, ar.APMCODE, ar.APPRDET, ar.PRICODE, brn.brncode, brn.nicname, ar.RQBYDES, pri.priname, ar.APPFRWD, 
                                            ar.REQSTFR, AR.INTPEMP, ar.AUTFRWD 
                                        order by branch asc, AVAL Desc, APPORDER Asc");*/
                        ...
                        <table id="customers2" class="table datatable">
                            ...
                            <?  $ij = 0;
                                ...
                                <tr class="odd gradeX" style='background-color:<?=$bgclr?>; color:<?=$clr?>;'>
                                    ...
                                    <td>
                                        <a href='view_pending_approval_quote_viki.php?action=view&reqid=<? echo $sql_search[$search_i]['ARQCODE']; ?>&year=<? echo $sql_search[$search_i]['ARQYEAR']; ?>&rsrid=1&creid=<? echo $sql_search[$search_i]['ATCCODE']; ?>&typeid=<? echo $sql_search[$search_i]['ATYCODE']; ?>' title='View' alt='View' style='color:<?=$clr?>; font-weight:normal; font-size:10px;'><? echo $sql_search[$search_i]['APRNUMB']; ?></a>
                                        <?  $userid = $_SESSION['tcs_empsrno'];
                                            ...
                                            <!-- Timer for Skip -->
                                            <? $sql_skip = select_query_json("select to_char(EXTNTIM, 'dd-MON-yyyy hh:mi:ss AM') EXTNTIM, to_char(STRTIME, 'dd-MON-yyyy hh:mi:ss AM') STRTIME, 
                                                                                        EDTUSER, EXTNDUR 
                                                                                    from approval_timer 
                                                                                    where aprnumb = '".$sql_search[$search_i]['APRNUMB']."' and skipapr = 'Y' and 
                                                                                        arqsrno = '".$sql_search[$search_i]['ARQSRNO']."' and timstat = 'N'", "Centra", "TEST"); 
                                                if(count($sql_skip) > 0) { ?>
                                                    <br><span class="label label-primary" id="id_process_duration_<?=$ij?>" style="font-size: 12px; line-height: 25px;">Process Duration : <?=$sql_skip[0]['EXTNTIM']?></span><br>
                                                    <? if($sql_skip[0]['EDTUSER'] == '') { ?>
                                                        <span id="id_slt_duration_<?=$ij?>"><select class="form-control" required name='txt_duration[]' id='txt_duration_<?=$ij?>' onchange="change_duration('<?=$sql_search[$search_i]['APRNUMB']?>', '<?=$sql_skip[0]['STRTIME']?>', '<?=$sql_search[$search_i]['ARQSRNO']?>', '<?=$ij?>')" data-toggle="tooltip" data-placement="top" data-original-title="Change Duration">
                                                            <?  $sql_duration = select_query_json("select * from approval_duration 
                                                                                                        where DELETED='N' and pricode = '".$sql_search[$search_i]['PRICODE']."' and EXTTIME > 0 
                                                                                                        order by adrcode", "Centra", 'TEST');
                                                                // update approval_timer set EXTNTIM = '', EXTNDUR = '', EDTUSER = '1444288', EDTDATE = SYSDATE WHERE aprnumb = 'ADMIN / OFFICE 4000032 / 23-10-2018 / 0032 / 11:38 AM' AND ARQSRNO = 3 AND TIMSTAT = 'N';
                                                                // $duration_ii = $sql_duration[0]['RGFLWTM'];
                                                                $duration_ii = $sql_skip[0]['EXTNDUR'];
                                                                
                                                                for($duration_i = $duration_ii; $duration_i <= $sql_duration[0]['EXTTIME']; $duration_i++) { ?>
                                                                    <option value='<?=$duration_i?>' <? if($duration_i == $duration_ii) { ?> selected <? } ?>><?=$duration_i?> HOURS</option>
                                                            <? } ?>
                                                        </select></span>
                                                    <? } ?>
                                            <? } ?>
                                    ...
                                    <td class="center show_moreless" style="max-width: 800px; overflow-x: auto;">
                                        <?  if($sql_search[$search_i]['APPRFOR'] == '1' or $sql_search[$search_i]['APPRFOR'] == '2' or $sql_search[$search_i]['APPRFOR'] == '3' or $sql_search[$search_i]['APPRFOR'] == '4') {
                                            ...
                                            // if($_SESSION['tcs_empsrno'] == 43878) { echo list_summary(str_replace("&nbsp;", " ", $contents), $limit=1000, $strip = false); echo "<br><br>"; }
                                    ...
                                    <td class="center" style='text-align:center;'>
                                        <? if($sql_search[$search_i]['APPSTAT'] == 'N' or $sql_search[$search_i]['APPSTAT'] == 'P' or $sql_search[$search_i]['APPSTAT'] == 'R' or $sql_search[$search_i]['APPSTAT'] == 'Z') { ?><a target="_blank" href='waiting_approvals_quote_viki.php?action=view&reqid=<? echo $sql_search[$search_i]['ARQCODE']; ?>&year=<? echo $sql_search[$search_i]['ARQYEAR']; ?>&rsrid=<? echo $sql_search[$search_i]['ARQSRNO']; ?>&creid=<? echo $sql_search[$search_i]['ATCCODE']; ?>&typeid=<? echo $sql_search[$search_i]['ATYCODE']; ?>' title='View' alt='View'><i class="fa fa-eye"></i> View</a><? }
...
<!-- START SCRIPTS -->
    ...
    function change_duration(aprnumb, start_time, arqsrno, ivalue)
    {
        var txt_duration = $('#txt_duration_'+ivalue).val();
        var sendurl = "ajax/ajax_employee_details.php?action=change_duration&aprnumb="+aprnumb+"&arqsrno="+arqsrno+"&start_time="+start_time+"&txt_duration="+txt_duration;
        $.ajax({
            url:sendurl,
            success:function(data){
                // alert("==="+data+"===");
                if(data == '0') { alert("CAME");
                    var ALERT_TITLE = "Message";
                    var ALERTMSG = "Timer Duration Change Failure. Kindly try again!!";
                    createCustomAlert(ALERTMSG, ALERT_TITLE);
                } else { alert("NOT");
                    $('#id_process_duration_'+ivalue).html(data);
                    $('#id_slt_duration_'+ivalue).html('');
                }
            }
        });
    }

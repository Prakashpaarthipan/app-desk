...
include("lib/public_functions.php");
extract($_REQUEST);
...
if($action == 'supplier'){
		$result = select_query("select SUPCODE, SUPNAME from supplier 
											where DELETED = 'N' and ( SUPCODE like '%".strtoupper($_GET['name_startsWith'])."%' or 
												SUPNAME like '%".strtoupper($_GET['name_startsWith'])."%' ) and SUPCODE >=7000 and rownum<10  
	...
	if(empty($data)){
		array_push($data, "No Records - Found");
	}

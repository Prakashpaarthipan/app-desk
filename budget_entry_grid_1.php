...
<!-- START SCRIPTS -->
  ...
  <script src="js/jquery-ui-1.10.3.custom.min.js"></script>
    ...
    <script type="text/javascript">
      ...
      function resserve_value_lock(clsname)
      { 
         // console.log(tab);
         var res_cls=clsname.split("req_value_calculation");
         var resverve_budget=Number($("#resval_"+res_cls[1]).html())*100000;
         var sum_req=0;
         $("."+clsname).each(function(){
            sum_req=sum_req+Number($(this).val());
         });
         if(sum_req>resverve_budget)
         {
         ...
         else
        {
         ...
         // console.log("#resval_"+tab+" => "+$("#resval_"+tab).html());
      }
      var ntab=0;
      ...
      { 
        ...
        var rsv_chk=resserve_value_lock(clsname);
        if(rsv_chk==1)
        {
          alert("Request Budget must be < Reserve Budget !");
          return;
        }
      ...
      { 
        ...
        if(Number($('#txt_tar_sal').val())<=0)
        {
          alert("Warning : Target Sale must be > 0 !");
          return;
        }
        var rsv_chk=resserve_value_lock("req_value_calculation"+tab);
        if(rsv_chk==1)
        {
          alert("Request Budget must be < Reserve Budget !");
          return;
        ...
        var subtab=0;
        var subflag=0;
        //console.log( '#reqval_'+tab+'_'+subtab+" => "+$("#reqreason_"+tab+"_"+subtab).length);
        while($('#reqreason_'+tab+'_'+subtab).length)
        { //console.log(Number($('#reqval_'+tab+'_'+subtab).val())+"=>"+$('#reqreason_'+tab+'_'+subtab).val());
          if(Number($('#reqval_'+tab+'_'+subtab).val()) >0 && $('#reqreason_'+tab+'_'+subtab).val() == '')
          {
            //alert("Warning : Resaon required for value Field !");
            $('#reqreason_'+tab+'_'+subtab).css("border","2px solid rgba(224, 75, 74, 1)");
            subflag=1;
          }
          subtab++;
        }

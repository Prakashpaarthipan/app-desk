...
if($_SERVER['REQUEST_METHOD'] == "POST" and $_POST['function'] != 'save_more_suppliers')
{
	...
	if($update_apprq == 1 and $cntlive > 0) {
		...
		for ($atchi=0; $atchi < count($hidchk_attachment); $atchi++) { 
			...
			// echo('</pre>');
			$insert_appcmt = insert_test_dbquery($field_appcmt, $tbl_appcmt);
		}
		// Archive Approvals - IMAGES
		...
		for ($atchi=0; $atchi < count($hidchk_attachment1); $atchi++) { 
			if($chk_attachment1[$hidchk_attachment1[$atchi]] == 'on') {
				...
				$update_apprq1 = update_test_dbquery($field_apprq1, $tbl_apprq1, $where_apprq1);
			} else {
				$tbl_apprq1 = "APPROVAL_REQUEST_DOCS";
				$field_apprq1 = array();
				$field_apprq1['DOCSTAT'] = 'N';
		...
		for ($aai = 0; $aai < count($hidchk_reject_reason); $aai++) {
			...
			$succ = 0;
			$insert_response = insert_test_dbquery($field_values, $tbl_name);
			// exit;
			//*********** Mail Send Function to User ********************/
		}
		// exit;
	...
	if($hid_samearqsrno == 0) {
		...
		if($alternate_available == 1 and $apmcode_alternate_available == 1) {
			$sql_altuser = select_query_json("select alt.*, (select emp.empname from APPROVAL_ALTERNATE_daily al, employee_office emp where al.ALTSRNO = emp.empsrno and 
														al.ALTSRNO = alt.ALTSRNO and al.apdsrno = alt.apdsrno and trunc(al.adddate) = trunc(sysdate)) ALTERNATE_USER, (select emp.empname 
														from APPROVAL_ALTERNATE_daily al, employee_office emp where al.RPTSRNO = emp.empsrno and al.RPTSRNO = alt.RPTSRNO and 
														al.apdsrno = alt.apdsrno and trunc(al.adddate) = trunc(sysdate)) reporting_user, (select emp.empname from APPROVAL_ALTERNATE_daily al, 
														employee_office emp where al.ELGUSER = emp.empsrno and al.ELGUSER = alt.ELGUSER and al.apdsrno = alt.apdsrno 
		...
		if($reqstfr == 34593) { // Verify / Approval User // Project Saravanakumar
            $sql_altuser = select_query_json("select ALTSRNO from APPROVAL_alternate_user where deleted = 'N' and EMPSRNO = '".$reqstfr."'", "Centra", "TEST");
		...
		if($alternate_available == 1 and $apmcode_alternate_available == 1) {
			$sql_altuser = select_query_json("select alt.*, (select emp.empname from APPROVAL_ALTERNATE_daily al, employee_office emp where al.ALTSRNO = emp.empsrno and 
														...
														from APPROVAL_ALTERNATE_daily al, employee_office emp where al.RPTSRNO = emp.empsrno and al.RPTSRNO = alt.RPTSRNO and 
														al.apdsrno = alt.apdsrno and trunc(al.adddate) = trunc(sysdate)) reporting_user, (select emp.empname from APPROVAL_ALTERNATE_daily al, 
														employee_office emp where al.ELGUSER = emp.empsrno and al.ELGUSER = alt.ELGUSER and al.apdsrno = alt.apdsrno and 
	...
	if($insert_appreq == 1) { // exit();
		$sql_finapr = select_query_json("select appstat from approval_request where aprnumb = '".$txt_approval_number."' and arqsrno = 1", "Centra", "TEST");
		// echo "select appstat from approval_request where aprnumb = '".$txt_approval_number."' and arqsrno = 1";
		// echo "==".$update_apprq."==".$insert_appreq."==".$fnupdt."==".$sql_finapr[0]['APPSTAT']."=="; // exit;
		if($fnupdt == 1 and $sql_finapr[0]['APPSTAT'] == 'A') {

			////////////////////////////////////////////////////
			$sql_list = select_query_json("sELECT prd.old_prdcode,apqf.*,req.RQESTTO,req.adddate REQDATE FROM  approval_product_quotation_fix APQF,approval_request req,product_asset prd WHERE prd.prdcode=apqf.prdcode and req.APRNUMB=apqf.aprnumb and apqf.aprnumb='".$txt_approval_number."' and apqf.sltsupp=1 and req.arqsrno = 1 and req.deleted = 'N'", "Centra", 'TEST');
			/* echo("sELECT prd.old_prdcode,apqf.*,req.RQESTTO,req.adddate REQDATE FROM  approval_product_quotation_fix APQF,approval_request req,product_asset prd WHERE prd.prdcode=apqf.prdcode and req.APRNUMB=apqf.aprnumb and apqf.aprnumb='".$txt_approval_number."' and apqf.sltsupp=1"); */
				$ki=count($sql_list);
				...
				{	
						...
						$adddate = strtoupper(date('d-M-Y h:i:s A', strtotime($sql_list[$i]['REQDATE'])));
						...
						$g_fldi['SUP_ACTIVE']='Y';
						$g_fldi['APRNUMB']=$txt_approval_number;
						// print_r($g_fldi);
						//echo("-------------------------------------------------------------------------\n");
...
/*echo("select req.*, prj.APRNAME, top.ATCNAME, typ.ATYNAME, apm.APMNAME, regexp_replace(SubStr(nicname,1,4),'[0-9]','')||SubStr(nicname,5,10) branch, emp.EMPCODE,
										emp.EMPNAME, ast.EXPSRNO, ast.DEPNAME, ast.EXPNAME EXPNNAME, (select
										ADDUSER||'!'||REQSTBY||'!'||RQBYDES||'!'||REQDESC||'!'||REQESEC||'!'||REQDESN||'!'||REQESEN from APPROVAL_REQUEST where ARQCODE = req.ARQCODE and
										ARQYEAR = req.ARQYEAR and ARQSRNO = 1 and ATCCODE = req.ATCCODE and ATYCODE = req.ATYCODE and deleted = 'N') addeduser, (select ARQYEAR
										from APPROVAL_REQUEST where ARQCODE = req.ARQCODE and ARQSRNO = 1 and ATCCODE = req.ATCCODE and ATYCODE = req.ATYCODE and deleted = 'N') ARYR,
									...
									where req.ATYCODE = typ.ATYCODE and req.APRCODE = prj.APRCODE and req.ATCCODE = top.ATCCODE and req.APMCODE = apm.APMCODE and req.BRNCODE = brn.BRNCODE
										...
										and req.deleted = 'N' and brn.DELETED = 'N' and ast.DELETED = 'N' and emp.empsrno = req.ADDUSER and brn.BRNMODE in ('B', 'K', 'T') and
										(req.REQSTFR = '".$usrid."' or req.INTPEMP = '".$usrid."') and req.appstat in ('W', 'N', 'Z') ".$usr." ".$mr_ak_date."
									order by req.ARQCODE, req.ARQSRNO, req.ATYCODE");
*/
			...
			<table border=0 style='width:100%; max-width: 773px; '>
				...
				<tr style='min-height:20px; line-height:20px;'>
					...
					<td style='width:80%; padding-top: 5px; text-align:left;'>
						<span style="font-size:12px; font-weight: normal; line-height:20px;">:</span> <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? echo $sql_reqid[0]['APMASTER']; ?>( <?=$sql_period[0]['ACCYEAR'];?> )</span></label> <!-- Approval Listings -->
					</td>
				</tr>
				...
				<!-- expense head -->
					...
					<tr style='min-height:20px; line-height:20px;'>
						<td style='font-size:16px; padding-top: 5px; font-weight:bold; width:20%; text-align:left;'>
							<strong><label>Exp. Head </label></strong> <!-- Exp. Head / Department -->
						</td>
						<td style='width:80%; text-align:left;'>
							<table style="width: 100%">
							<tr style='min-height:20px; line-height:20px;'>
								<td >
									:<label style='font-size:16px; padding-top: 5px; font-weight:bold; width:20%; text-align:left;'> <? echo $sql_expname[0]['EXPNAME'];?> ( <? echo $sql_expname[0]['TARNUMB'];?>  - <? echo $sql_expname[0]['TARNAME'];?> ) </label> <!-- Exp. Head / Department -->
								</td>
							</tr>
							</table>
						</td>
									...
									<tr style='min-height:30px; width: 100%; vertical-align:top; line-height:10px; font-size: 11px;'>
										...
										<td style='width:60%; text-align:right; line-height:10px;'>
											<? if($sql_approve_leads[0]['APPRFOR'] == '1' or $sql_approve_leads[0]['APPRFOR'] == '2' or $sql_approve_leads[0]['APPRFOR'] == '3' or $sql_approve_leads[0]['APPRFOR'] == '4' or $sql_approve_leads[0]['APPRFOR'] == '5') { ?>
												<label><a href="javascript:void(0)" onclick="popup_original_details('<? echo $sql_reqid[0]['ARQCODE']; ?>', '<? echo $sql_reqid[0]['ARQYEAR']; ?>', '<? echo $sql_reqid[0]['APRNUMB']; ?>')" style="text-transform: uppercase; font-weight: bold;" title="Original Details" class="blue_highlight"><i class="fa fa-paperclip"></i> Original5 Details</a></label> <!-- Original Details -->
											<? } ?>
										</td>
									</tr>
									</table>
								...
								<td style='width:100%; text-align:left; max-width: 760px !important; overflow-x: auto !important;'>
									<label class="print_table"><?
										if($sql_approve_leads[0]['APPRFOR'] == '1' or $sql_approve_leads[0]['APPRFOR'] == '2' or $sql_approve_leads[0]['APPRFOR'] == '3' or $sql_approve_leads[0]['APPRFOR'] == '4' or $sql_approve_leads[0]['APPRFOR'] == '5') {
	                                        ...
	                                        $filename = "ftp://".$ftp_user_name_apdsk.":".$ftp_user_pass_apdsk."@".$ftp_server_apdsk.$ftp_srvport_apdsk."/approval_desk/text_approval_source_test/".$filepathname;
														...
														<?
															...
															foreach($sql_descode as $sectionrow_emp) {
																if ($sectionrow_emp['EMPCODE'] == '0') {
																	// echo "- NILL -";
																	$sql_descode1=select_query_json("select * from APPROVAL_branch_head 
					...
					<div class="row" style="border:1px solid #1c94c4;margin-top: 1%;border-radius: 3px;">
						...
						<!-- <pre> -->
                                ...
                                //  echo("select spa.subname,sup.supname,pa.prdname,apqf.* from approval_product_quotation_fix apqf,supplier_asset sup,SUBPRODUCT_ASSET SPA,PRODUCT_ASSET PA where pa.prdcode=apqf.prdcode AND spa.prdcode=apqf.prdcode and spa.subcode=apqf.subcode and sup.supcode=apqf.supcode and APRNUMB='".$sql_reqid[0]['APRNUMB']."'");
                                    $sql_quote=select_query_json("select spa.subname,unt.untname,sup.supname,pa.prdname,apqf.* from approval_product_quotation_fix apqf,supplier_asset sup,SUBPRODUCT_ASSET SPA,PRODUCT_ASSET PA,unit unt where pa.prdcode=apqf.prdcode AND spa.prdcode=apqf.prdcode and spa.subcode=apqf.subcode and sup.supcode=apqf.supcode and SPA.untcode=unt.untcode and APRNUMB='".$sql_reqid[0]['APRNUMB']."' order by apqf.entnumb", "Centra", "TEST");
                                    $arr_prd=array();
				...
				<td colspan=2 style='width:100%; font-size:11px; text-align:left;'>
					...
					<tr>
						<td style='width:35%; text-align:left;'>
							...
							<b>Work Initiate Person : <? $sql_usr = select_query_json("select EOF.EMPCODE,EOF.EMPNAME,DES.DESNAME,EMP.ESENAME from employee_office EOF,DESIGNATION DES,empsection emp where empcode='".$sql_reqid[0]['WRKINUSR']."' and eof.descode=des.descode and emp.esecode=eof.esecode", "Centra", 'TCS');
								echo $sql_usr[0]['EMPCODE']." - ".$sql_usr[0]['EMPNAME']; ?><br>
							<b>Responsible Person : <? $sql_usr = select_query_json("select EOF.EMPCODE,EOF.EMPNAME,DES.DESNAME,EMP.ESENAME from employee_office EOF,DESIGNATION DES,empsection emp where empcode='".$sql_reqid[0]['RESPUSR']."' and eof.descode=des.descode and emp.esecode=eof.esecode", "Centra", 'TCS');
			...
			<div class="col-lg-12 col-md-12" style=' text-align:center; padding-right:0px;'>
				...
				<? if($_REQUEST['action'] == 'view') {
					...
					if(($sql_requsr[0]['REQSTFR'] == 61579 or $sql_requsr[0]['REQSTFR'] == 83815 or $sql_requsr[0]['REQSTFR'] == 96252) && $sql_reqid[0]['APPSTAT'] =='N' )
					{
						// vikig
						$flagbid=0;
						if($_SESSION['tcs_usrcode']=='1925688' && $sql_reqid[0]['APPSTAT'] !='Z' && $sql_reqid[0]['PURHEAD']!=2) { ?>
							<button type="button" name='sbmt_bid' id='sbmt_bid' tabindex='28' class="btn" title="BID" style="background-color:#d42c65;color: white;" onclick="get_bid_new('<?=$sql_reqid[0]['ARQYEAR']?>','<?=$sql_reqid[0]['IMUSRIP']?>','<?=$sql_reqid[0]['ATCCODE']?>','CREATE_QUOTE','<?=$sql_reqid[0]['APRNUMB']?>');"> <i class="fa fa-gavel"></i> BID</button>
						 <?$flagbid=1;}if($sql_reqid[0]['APPSTAT'] =='Z'){$flagbid=1;}

						 if($final_approval == 1 and $mdfin == 0 and $sql_reqid[0]['APPFRWD'] != 'I' && $flagbid==0) { $cc_stage++; // Final Approval.. ?>
							<button type="submit" name='sbmt_approve' id='sbmt_approve' tabindex='28' value='Final Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Final Approve"><i class="fa fa-check-square-o"></i> Final Approve</button>&nbsp;
						...
						<? }
						 ...
						 if($final_approval == 0 && $flagbid==0) { $isstage = 1; $cc_stage++; // Not Final Approval..?>
							<button type="submit" style="float: right;" name='sbmt_forward' id='sbmt_forward' tabindex='28' value='Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve"><i class="fa fa-fast-forward"></i> Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve</button>&nbsp;
						<? } // Not Final Approval..



					} elseif($_SESSION['tcs_usrcode']=='1925688' && $sql_reqid[0]['APPSTAT'] =='Z' ) {
						 ?>
							<button type="button" name='' id='' class="btn" title="REVERSE BID" tabindex='28' style="background-color:#d42c65;color: white;" onclick="get_bid_new('<?=$sql_reqid[0]['ARQYEAR']?>','<?=$sql_reqid[0]['IMUSRIP']?>','<?=$sql_reqid[0]['ATCCODE']?>','REVERSE_QUOTE','<?=$sql_reqid[0]['APRNUMB']?>');"> <i class="fa fa-gavel"></i> REVERSE BID</button>
							<div id="mybody" class="red_highlight"></div>
						<? 
					} 


					elseif($sql_requsr[0]['REQSTFR'] != 61579 and $sql_requsr[0]['REQSTFR'] != 83815 and $sql_requsr[0]['REQSTFR'] != 96252 && $flagbid==0) {
						...
						// vikisub
						if($final_approval == 0 && $flagbid==0) { $isstage = 1; $cc_stage++; // Not Final Approval..?>
							<button onclick="post();" style="float: right;" name='sbmt_forward' id='sbmt_forward' tabindex='28' value='Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve"><i class="fa fa-fast-forward"></i> Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve</button>&nbsp;
						<? }
					} // Not Final Approval..
				} else {
				...
				if(($sql_requsr[0]['REQSTFR'] == 61579 or $sql_requsr[0]['REQSTFR'] == 83815 or $sql_requsr[0]['REQSTFR'] == 96252) && $sql_reqid[0]['APPSTAT'] =='N')
				{
					if($finish_here == 1 and $sql_reqid[0]['APPFRWD'] != 'I' && $flagbid==0) { $cc_stage++; 
						if($sql_reqid[0]['PURHEAD'] != 2) { ?>
					...
					if($cc_stage == 0 && $flagbid==0){ ?>
						<button type="submit" style="display: block; float: right;" name='sbmt_forward' id='sbmt_forward' tabindex='28' value='Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve"><i class="fa fa-fast-forward"></i> Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve</button>&nbsp;
					<? }
				} elseif(($sql_requsr[0]['REQSTFR'] == 61579 or $sql_requsr[0]['REQSTFR'] == 83815 or $sql_requsr[0]['REQSTFR'] == 96252) && $sql_reqid[0]['APPSTAT'] =='Z' && $flagbid==0 ) {

				} elseif($sql_requsr[0]['REQSTFR'] != 61579 and $sql_requsr[0]['REQSTFR'] != 83815 and $sql_requsr[0]['REQSTFR'] != 96252) {
					if($finish_here == 1 and $sql_reqid[0]['APPFRWD'] != 'I' && $flagbid==0) { ?>
						<button type="submit" name='sbmt_mdapprove' id='sbmt_mdapprove' tabindex='28' value='Final Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Use this Final Approve if value is low and no need for MD Approval." style='margin-left:1px;'><i class="fa fa-check-square-o"></i> Final Approve</button>&nbsp;
					<? } elseif($isstage == 0 ) { ?>
						<button type="submit" style="float: right;" name='sbmt_forward' id='sbmt_forward' tabindex='28' value='Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve"><i class="fa fa-fast-forward"></i> Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve</button>&nbsp;
					<? }
...
<? } ?>
	...
	function post(){
		var form_data = new FormData(document.getElementById('frm_print_request'));
	...
	function get_bid_new(year,code,head,stat,aprnumb)
	{
		var bid_expiry_date = $("#datepicker_example4").val();
		// alert("**"+bid_expiry_date+"**"); exit;
		$(".loader").show();
		$.ajax({
			url:"ajax_bid_new.php?action="+stat+"&aprnumb="+aprnumb+"&year="+year+"&code="+code+"&head="+head+"&bid_expiry_date="+bid_expiry_date,
			success:function(data)
			{
				if(stat == "CREATE_QUOTE")
				{
					
					$("#modal-body5").css('display', 'none');
					$('#load_page').show();

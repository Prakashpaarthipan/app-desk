                                      ...
                                      <!-- ///////////////////////// -->
                                       ...
                                       <div class="panel-body">
                                        ...
                                        <table  class="table datatable">
                                            ...
                                            <tbody>
                                            <?  $sql_search1 = select_query_json("select TEMPYR,TEMPNO,PRCSNO,PRCSYR,LANGCOD,TEMPCMNT from SUPMAIL_PROCESS_ENTRY where DELETED='N' order by TEMPNO desc", "Centra", 'TEST');
                                            if(count($sql_search1)>0){
                                            $ki = 0;
                                            for($k=0;$k<sizeof($sql_search1);$k++){//echo $sql_search[$k]['ENTSTAT'];
                                                 $ki++; ?>
                                                  <tr class="odd gradeX" style='background-color:<?=$bgclr?>; color:<?=$clr?>;'>
                                                    <td class="center" style='text-align:center;'>
                                                        <? echo $ki; // SERIAL NUMBER OF THE RECORD ?>
                                                    ...
                                                    <td class="center" style='text-align:center'><!-- for entryno-->
                                                        <? echo $sql_search1[$k]['TEMPYR'].' _ '.$sql_search1[$k]['TEMPNO']; // SERIAL NUMBER OF THE RECORD ?>
                                                    </td>
                                                    <td class="center" style='text-align:center'><!-- for top core-->
                                                        <? 
                                                        $pro_name = select_query_json("select PRCDSC from SUPMAIL_PROCESS where PRCSNO='".$sql_search1[$k]['PRCSNO']."' and PRCSYR='".$sql_search1[$k]['PRCSYR']."' and DELETED='N'", "Centra", 'TEST');

                                                        echo $pro_name[0]['PRCDSC']; ?> 
                                                    </td>
                                                    <td class="center" style='text-align:center; '><!-- for priority-->
                                                        <?
                                                            $lang_name = select_query_json("select LANGNAM from SUPMAIL_PROCESS_LANGUAGE where PRCSNO='".$sql_search1[$k]['PRCSNO']."' and LANGCOD='".$sql_search1[$k]['LANGCOD']."' and DELETED='N'", "Centra", 'TEST'); 
                                                        echo $lang_name[0]['LANGNAM']; ?>
                                                    </td>
                                                    
                                                    <td class="center" style='text-align:center'><!-- for attachment count-->
                                                        <? echo $sql_search1[$k]['TEMPCMNT']; ?>
                                                    </td>
                                                   
                                                    <td class="center" style='text-align:center'><!-- for USER DETAIL-->
                                                        <a onclick="printpage('<? echo $sql_search1[$k]['TEMPNO'];?>','<?echo $sql_search1[$k]['TEMPYR'];?>');" data-toggle="tooltip" title="print" class="btn btn-warning btn-sm"><span class="fa fa-print"></span></a>
                                                        <a onclick="delprocess('<? echo $sql_search1[$k]['TEMPNO'];?>','<?echo $sql_search1[$k]['TEMPYR'];?>');" data-toggle="tooltip" title="delete" class="btn btn-danger btn-sm"><span class="fa fa-trash-o"></span></a>

...
function approved_create_new_list($txt_approval_number, $slt_topcore) {
	...
	foreach($sql_descode as $sectionrow) { $ai++;
		...
		if($ai == 1) {
			$ivqry = delete_query_json("INSERT INTO approval_budget_planner_temp select '".$apprno."' , '".$budget_planner_temp[0]['MXAPRSRNO']."', APRPRID, APRMNTH, '".$sectionrow['APRAMNT']."', APPMNTH, APPYEAR, TARNUMB, '".$RESVALU."', '".$EXTVALU."', BUDMODE, APRYEAR, ADDUSER, ADDDATE, EDTUSER, EDTDATE, DELETED, DELUSER, DELDATE, '".$sectionrow['BRNCODE']."', APPMODE, EXPSRNO, EXISTVL, USEDVAL, ACCVRFY, TMTARNO, ATYCODE, PHDCODE, PGRCODE, ESECODE, DEPCODE from approval_budget_planner_temp where APRNUMB = '".$txt_approval_number."'", "Centra", "TEST");

			$ivqry = delete_query_json("INSERT INTO approval_budget_planner select '".$apprno."' , '".$budget_planner_temp[0]['MXAPRSRNO']."', APRPRID, APRMNTH, '".$sectionrow['APRAMNT']."', APPMNTH, APPYEAR, TARNUMB, '".$RESVALU."', '".$EXTVALU."', BUDMODE, APRYEAR, ADDUSER, ADDDATE, EDTUSER, EDTDATE, DELETED, DELUSER, DELDATE, '".$sectionrow['BRNCODE']."', 'APPMODE', EXPSRNO, EXISTVL, USEDVAL, DEPCODE from approval_budget_planner_temp where APRNUMB = '".$txt_approval_number."'", "Centra", "TEST");

			$ivqry = delete_query_json("update approval_budget_planner set APPMODE = 'Y' where APRNUMB = '".$apprno."'", "Centra", "TEST");
			...
			*/
			...
			foreach($sql_descode_approval_mdhierarchy as $sectionrow) {
				$amrsrno++;
				$ivqry = delete_query_json("INSERT INTO approval_mdhierarchy select APMCODE, AMHSRNO, APPHEAD, APPDESG, APPDAYS, APPRIOR, APPTITL, VRFYREQ, '".$apprno."', PBDAPPR from approval_mdhierarchy where APRNUMB = '".$apprno."'", "Centra", "TEST");
			}
		}
	}
}

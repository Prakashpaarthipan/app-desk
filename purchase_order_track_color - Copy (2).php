                   ...
                   <?php 
                     ...
                     <div style="height:auto;cursor:pointer" id="order<?=$tme.$t?>" onclick="openprocess('<?=$tme.$t?>','<?=$ord_confirm_value["ZNECODE"]?>','<?=$sql_val["ZNPSRNO"]?>','<?=$sql_val["PORYEAR"]?>','<?=$sql_val["PORNUMB"]?>','<?=$sql_val['PORDATE']?>','<?=$sql_val["SUPCODE"]?>','<?=$sql_val['PORQTY']?>','<?=$sql_val['PORVAL']?>','<?=$sql_val['PROSTAT']?>')">
						...
						<div style="padding-bottom:5px; background-color: #d8d8d8; min-height: 45px;">
						 ...
						 <input type='radio' name='process<?=$tme.$t?>' id='process1<?=$tme.$t?>' <?if($sql_val['PROSTAT']==1){?> checked  <?}?> value="1"/> <label class="<?=$blink1?> label label-danger" style="margin: 2px;">UPDATE</label>
						  <input type='radio' name='process<?=$tme.$t?>' id='process2<?=$tme.$t?>' <?if($sql_val['PROSTAT']==2){?> checked  <?}?> value='2'/> <label class="<?=$blink2?> label label-warning" style="margin: 2px;">PROCESS</label>
						 <input type='radio' name='process<?=$tme.$t?>' id='process3<?=$tme.$t?>' <?if($sql_val['PROSTAT']==3){?> checked  <?}?> value='3'/>  <label class="<?=$blink3?> label label-success" style="margin: 2px;">COMPLETED</label>
						  </div>

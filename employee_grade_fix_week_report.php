...
$emp=array();$empname=array();$empcode=array();
foreach($join as $key=>$val){
$emp[]=$val['EMPSRNO'];
$empname[]=$val['EMPNAME'];
$empcode[]=$val['EMPCODE'];
}
$unique_emp=array_values(array_unique($emp));
$uni_name=array_values(array_unique($empname));	
$uni_code=array_values(array_unique($empcode));			
?>
<!DOCTYPE html>
...
<div class="row">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <div class="pull-right">
                            <a class="btn btn-danger" target="_blank" style="margin-right: 5px;" href="employee_add.php">(+) Add Employees</a>&nbsp;&nbsp;<a class="btn btn-danger" target="_blank" style="margin-right: 50px;" href="employee_grade_fix_week_report.php">Grade Fix</a><br><br>
                        </div>
                    </div>
								...
								<?	foreach ($sql_week as $key => $week_value) {
                                      $dt_value = strtoupper(date("d-M-Y", strtotime($week_value['DT'])));
                                        ...
                                        $sql_empgrade = select_query_json("select EMPGRAD, EMPRMRK from EMPLOYEE_GRADE_FIX 
                                                                                    where deleted = 'N' and EMPSRNO = '".$unique_emp[$search_i]."'".$and."", 'Centra', 'TEST'); 
																				
	
							 ?>
                               <!-- <td class="center" style="text-align: center;"><?=$sql_empgrade[0]['EMPGRAD'];?> </td> -->
                                <td class="center" style="text-align: center;"><?=$sql_empgrade[0]['EMPGRAD'];?> <br><br><?=$sql_empgrade[0]['EMPRMRK'];?></td> 
								<? } ?>
    ...
    function task_assign(empsrno, iv) {
        ...
        var strURL="ajax/fix_grade_week_report.php?action=fix_grade&empsrno="+empsrno+"&grade="+grade;
        if(grade != '') {
            $.ajax({
                type: "POST",
                url: strURL,

...
function addnewrow(){
	$('#process_section').css('display','block');
}
              ...
              '<div class="form-group input-group">'+
				'<div style="width:100%">'+
					...
					'<div style="width:100%;float:left">'+
						'<input type="text" name = "txt_value[]" id= "txt_values" class="form-control" pattern="^[0-9]*$" placeholder= "PROCESS NAME" data-toggle="tooltip" title ="values" required  autocomplete="off">'+
					'</div>'+
				'</div>'+
				'<span class="input-group-btn"><button id="add_ledger_button" type="button" class="btn btn-danger btn-remove" data-toggle="" title ="remove">-</button></span>'+
			  '</div>');
...
function selectproc(frm_name){
  ...
  $('#load_page').show();
   ...
   form_data.append('action','selectprocess');
            $.ajax({
                url:"ajax/ajax_purchase_order2.php",
                type: "POST",
                data: form_data,
                processData: false,
                contentType: false,

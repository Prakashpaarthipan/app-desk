...
if($_SERVER['REQUEST_METHOD']=="POST" and $_REQUEST['sbmt_update'] != '') 
{
   ...
   //$sql_empcode = select_query_json("select EMPCODE,EMPSRNO from employee_office where EMPCODE='".$expl[0]."'");
	...
	$field_appreq['NAME1'] =$stu_name;
    $field_appreq['ADDRESS1'] = $stu_addr;
	$field_appreq['MOBILE1'] = $stu_phone;
	$field_appreq['EMAIL1'] = $stu_email;
	...
	$where_appreq = "SRNO = '".$hid_reqid."'";
	$insert_appreq = update_test_dbquery($field_appreq, $tbl_appreq, $where_appreq);
	// Update in APPROVAL_email_master Table
//exit
	if($insert_appreq == 1) { ?>
		<script>window.location='model.php?status=update_success';</script>
		<?php
		exit();
	} else { ?>
		<script>window.location='model.php?action=edit&reqid=<?=$hid_reqid?>&status=failure';</script>
		<?php
		exit();
	}
}
...
//echo ("$sql_empcode");
	...
	// Insert in APPROVAL_email_master Table
	$max_val = select_query_json("select count(*) max from STUDENTS", "Centra", 'TEST');
	$tbl_appreq = "STUDENTS";
	$field_appreq = array();
	$field_appreq['SRNO']=$max_val[0]['MAX'];
	$field_appreq['NAME1'] = $stu_name;
	//$field_appreq['ADDRESS1'] = $stu_addr;
	$field_appreq['MOBILE1'] = $stu_phone;
	$field_appreq['EMAIL1'] = $stu_email;
...
//exit;
	...
	if($insert_appreq == 1) { ?>
		<script>window.location='model.php?status=success';</script>
		<?php
		exit();
	} else { ?>
		<script>window.location='model.php?action=add&status=failure';</script>
		<?php
		exit();
	}
}
...
if($_REQUEST['action'] == 'deleted')
{
	...
	// Update in APPROVAL_email_master Table
if($insert_appreq == 1) { ?>
		<script>window.location='model.php?status=delete_success';</script>
		<?php
		//exit();
	} else { ?>
		<script>window.location='model.php?status=failure';</script>
		<?php
		//exit();
	}
}
	...
	.dataTables_filter {
		...
		font-size: 15px;
           ...
           <div class="panel-body" style='display:none;'>
			...
			$sql_reqid = select_query_json("select * from STUDENTS where SRNO='".$_REQUEST['reqid']."'","Centra",'TEST');
...
<label for="field1"><span> students Name <span class="required">*</span></span>
<input type="text" class="input-field" name="stu_name" id="stu_name" maxlength='50' value='<?=$sql_reqid[0]['NAME1'];?>' /></label><br><br>
<label for="field2"><span>Address <span class="required">*</span></span>
<input type="text" class="input-field" name="stu_addr" id="stu_addr" maxlength='100' value='<?=$sql_reqid[0]['ADDRESS1'];?>'/></label><br><br>
<label for="field2"><span>Mobile <span class="required">*</span></span>
<input type="number" class="input-field" name="stu_phone" id="stu_phone" maxlength='15' value='<?=$sql_reqid[0]['MOBILE1'];?>'/></label><br><br>
<label for="field2"><span>Email<span class="required">*</span></span>
<input type="text" class="input-field" name="stu_email" id="stu_email" maxlength='50' value='<?=$sql_reqid[0]['EMAIL1'];?>'/></label>
				...
				<div class="form-group trbg" style='min-height:40px; padding-top:10px'>
					<div class="col-lg-12 col-md-12" style=' text-align:center; padding-right:10px;'>
						<? if($_REQUEST['action'] == 'view') { ?><a href='model.php'><i class="fa fa-refresh"></i> Back</a><? } elseif($_REQUEST['action'] == 'edit') { ?>
		...
		<!-- /.row -->
			...
			<div class="panel panel-default">
					<div class="panel-heading">
						<center><i class="fa fa-check-circle-o fa-fw"></i>NightDuty Approval</center>
						<div class="pull-right">
							<div class="btn-group">
								...
								<ul class="dropdown-menu pull-right" role="menu">
									<li><a href="model.php?action=add">Add</a></li>
								</ul>
							</div>
						</div>
					</div>
	...
	<tr>
						<? 	$sql_project = select_query_json("select * from STUDENTS"); 
					for($project_i = 0; $project_i < count($sql_project); $project_i++) { ?>
					
					<!--	<td style='text-align:center'><?=($project_i+1)?></td> -->
					<td style='text-align:center'><?echo $sql_project[$project_i]['SRNO'];?></td>
						<td style='text-align:center'><?=$sql_project[$project_i]['NAME1']?></td>
					<!--	<td style='text-align:center'><?=$sql_project[$project_i]['ADDRESS1']?></td>-->
						<td style='text-align:center'><?=$sql_project[$project_i]['MOBILE1']?></td>
						<td style='text-align:center'><?=$sql_project[$project_i]['EMAIL1']?></td>
			<!--			<td style='text-align:center'><a href='model.php?action=edit&reqid=<? echo $sql_project[$project_i]['CMPSRNO']; ?>' title='Edit' alt='Edit'><i class="fa fa-edit"></i> Edit</a> / <a href='model.php?action=view&reqid=<? echo $sql_project[$project_i]['CMPSRNO']; ?>' title='View' alt='View'><i class="fa fa-eye"></i> View</a> / <a href='javascript:void(0)' id="delete_confirm" onclick="call_confirm(<?=($project_i+1)?>, 'model.php?action=deleted&reqid=<? echo $sql_project[$project_i]['CMPSRNO']; ?>')" title='Delete' alt='Delete'><i class="fa fa-trash-o"></i> Delete</a></td>-->

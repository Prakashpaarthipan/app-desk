...
// }
$sql= select_query_json("select MAX(ZNECODE) ZNECODE from order_tracking_master", "Centra", 'TEST'); 
foreach($sql as $key=> $master){
			$g_table="ORDER_TRACKING_DETAIL";
			$g_fld['ZNECODE']=$master['PORYEAR'];
			$g_fld['ZNENAME']=$master['PORNUMB'];
			$g_fld['ZNEPCDE']=$master['PORQTY'];
			$g_fld['ZNEPNME']=$master['PORVAL'];
			$g_fld['ADDUSER']='dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;
			$g_fld['ADDDATE']='dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;
			$g_fld['DELETED']=$master['SUPCODE'];
			$g_fld['ZNEDAYS']=$master['ZNECODE'];
			$g_fld['ZNEMODE']=$i;
			$g_fld['ZNEDATE']='dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;
			$g_fld['ZNEFIND']=$master['ZNEFIND'];
			$g_fld['ZNESTAT']='T';

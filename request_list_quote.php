                            ...
                            <table id="customers2" class="table datatable" style="overflow-x: scroll !important;">
                                ...
                                <?  $and = "";
                                    ...
                                    if($search_subject != '') {
                                        ...
                                        echo ftp_nlist($ftp_conn, "ftp://".$ftp_user_name_apdsk.":".$ftp_user_pass_apdsk."@".$ftp_server_apdsk.$ftp_srvport_apdsk."/approval_desk/text_approval_source_test/")."++";
                                    ...
                                    if($and != '') {
                                        ...
                                        $rqusr1 = " and ar.ADDUSER = '".$_SESSION['tcs_empsrno']."' ";
                                        $sql_brn = select_query_json("select BRNCODE, esecode from employee_office where empsrno = '".$_SESSION['tcs_userid']."'", "Centra", 'TEST');
                                        ...
                                        $sql_search = select_query_json("select to_char(ar.ADDDATE,'dd-MON-yyyy hh:mi:ss AM') ADDDATE, ar.FINDATE, ar.APRNUMB, ar.APPSTAT, ar.APPRSUB, ar.APPRFOR,ar.BRNCODE
                                                                                ...
                                                                                order by APPORDER asc, ADDDATE desc, APRNUMB desc", "Centra", 'TEST');
                                        if(count($sql_search) <= 0) {
                                            $sql_search = select_query_json("select to_char(ar.ADDDATE,'dd-MON-yyyy hh:mi:ss AM') ADDDATE, ar.FINDATE, ar.APRNUMB, ar.APPSTAT, ar.APPRSUB, ar.APPRFOR,ar.BRNCODE
                                                                                        ...
                                                                                        ar.APPRDET, ar.IMSTATS, ar.IMFNIMG, pr.pricode, pr.priname, pr.pricode||' - '||pr.priname priority
                                                                                    from approval_request ar, approval_priority pr
                                                                                    where ar.PRICODE = pr.pricode(+) and pr.deleted(+) = 'N' and ar.ARQSRNO = 1 ".$rqusr1." ".$and." and ar.deleted = 'N'
                                                                                    order by APPORDER asc, ADDDATE desc, APRNUMB desc", "Centra", 'TEST');
                                        }
                                    } else {
                                        ...
                                        $sql_search = select_query_json("select to_char(ar.ADDDATE,'dd-MON-yyyy hh:mi:ss AM') ADDDATE, ar.FINDATE, ar.APRNUMB, ar.APPSTAT, ar.BRNCODE, ar.APPRSUB, ar.APPRFOR,
                                                                                    ar.APPFVAL, ar.APRTITL, ar.ARQSRNO, ar.ARQCODE, ar.ATYCODE, ar.ATCCODE, ar.ARQYEAR, ar.ADDUSER, ar.RQESTTO,
                                                                                    decode(ar.APPSTAT, 'N', 'NEW', 'F', 'FORWARD', 'A', 'APPROVED', 'P', 'PENDING', 'Q', 'QUERY', 'S', 'RESPONSE',
                                                                                    'C', 'COMPLETED', 'R', 'REJECTED', 'NEW','E','EXPENSE_ENTRY') APPSTATUS, decode(ar.APPSTAT, 'N', '1', 'F', '2', 'A', '3', 'P', '4',
                                                                                    'Q', '5', 'S', '6', 'C', '7', 'R', '8', '9','E') APPORDER, (select APPSTAT from APPROVAL_REQUEST where ARQSRNO = 1 and
                                                                                    DELETED = 'N' and ARQCODE = ar.ARQCODE and ARQYEAR = ar.ARQYEAR and ATCCODE = ar.ATCCODE and ATYCODE = ar.ATYCODE and
                                                                                    ATMCODE = ar.ATMCODE and APMCODE = ar.APMCODE and APPSTAT = 'A') as APSTAT, (select EMPNAME from employee_office
                                                                                    where empsrno = ar.RQESTTO) as reqto, (select EMPNAME from employee_office where empsrno in (select REQSTBY
                                                                                    from APPROVAL_REQUEST where ARQCODE = ar.ARQCODE and ARQYEAR = ar.ARQYEAR and ARQSRNO = 1 and ATCCODE = ar.ATCCODE and
                                    ...
                                    for($search_i = 0; $search_i < count($sql_search); $search_i++) { $ij++;
                                        ...
                                        if($sql_search[$search_i]['APPSTAT'] == 'E') { $appstatus = "10 - RATE FIX"; $bgclr = '#B0E8FB'; $clr = '#000000'; }
                                        ...
                                        <tr class="odd gradeX" style='background-color:<?=$bgclr?>; color:<?=$clr?>;'>
                                            ...
                                            <?  $sectionrow=select_query_json("select distinct substr(bpl.appnumb,1,100) aprnumb, bpl.bplyear, bpl.bplnumb, nrq.nrqyear, nrq.nrqnumb, nto.ntoyear, nto.ntonumb, 
                                                                                ...
                                                                                union 
                                                                                    ...
                                                                                    where bpl.bplyear=chqs.bplyear(+) and bpl.bplnumb=chqs.bplnumb(+) and bpl.appnumb in (select apr.aprnumb 
                                                                                        ...
                                                                                        and bpl.appnumb = '".$sql_search[$search_i]['APRNUMB']."'", "Centra", "TEST");
                                            ...
                                            <td class="center" style='text-align:center; white-space:nowrap;'>
                                                ...
                                                <? if($sql_search[$search_i]['APPSTAT'] == 'E') {

                                                ?><input type="hidden" id="hid_ap_number" value= "<? echo $sql_search[$search_i]['APRNUMB'];?>" name="apprnumber" >
                                                <a target="_blank" href='expense_entry_list.php?action=view&reqid=<? echo $sql_search[$search_i]['ARQCODE']; ?>&year=<? echo $sql_search[$search_i]['ARQYEAR']; ?>&rsrid=1&creid=<? echo $sql_search[$search_i]['ATCCODE']; ?>&typeid=<? echo $sql_search[$search_i]['ATYCODE']; ?>&ap=<? echo urlencode($sql_search[$search_i]['APRNUMB']);?>&brn=<? echo $sql_search[$search_i]['BRNCODE'];?>' title='Quotation Fix' alt='Quotation Fix' style='color:<?=$clr?>; font-weight:normal; font-size:12px;'><i class="fa fa-trophy"></i> Quotation Fix</a>

                                                <?}?>
                                                <a target="_blank" href='view_pending_approval_quote_viki.php?action=view&reqid=<? echo $sql_search[$search_i]['ARQCODE']; ?>&year=<? echo $sql_search[$search_i]['ARQYEAR']; ?>&rsrid=<? echo $sql_search[$search_i]['ARQSRNO']; ?>&creid=<? echo $sql_search[$search_i]['ATCCODE']; ?>&typeid=<? echo $sql_search[$search_i]['ATYCODE']; ?>' title='View' alt='View' style='color:<?=$clr?>;'><i class="fa fa-eye"></i> View</a>
                                                ...
                                                <? ////// HIDE FINISH APPRVOAL //////
                                                    ...
                                                    if($sql_search[$search_i]['APPSTAT'] != 'A' and $sql_search[$search_i]['APPSTAT'] != 'R') { ?>
                                                         / <a target="_blank" href='waiting_approvals_quote_viki.php?action=print&reqid=<? echo $sql_search[$search_i]['ARQCODE']; ?>&year=<? echo $sql_search[$search_i]['ARQYEAR']; ?>&rsrid=<? echo $sql_search[$search_i]['ARQSRNO']; ?>&creid=<? echo $sql_search[$search_i]['ATCCODE']; ?>&typeid=<? echo $sql_search[$search_i]['ATYCODE']; ?>' title='View' alt='View'><i class="fa fa-print"></i> Print</a>
                                                    <? } elseif($sql_search[$search_i]['APPSTAT'] != 'R') { ?>
                                                         / <a href='javascript:void(0)' onclick="PrintDiv('print_request_quote_viki.php?action=print&reqid=<? echo $sql_search[$search_i]['ARQCODE']; ?>&year=<? echo $sql_search[$search_i]['ARQYEAR']; ?>&rsrid=<? echo $sql_search[$search_i]['ARQSRNO']; ?>&creid=<? echo $sql_search[$search_i]['ATCCODE']; ?>&typeid=<? echo $sql_search[$search_i]['ATYCODE']; ?>');" title='Print' alt='Print' style='color:<?=$clr?>;'><i class="fa fa-print"></i> Print</a>

				  ...
				  <div class="page-content-wrap">
					...
					<form role="form" id='frm_project_list_edit' name='frm_project_list_edit' method='post' enctype="multipart/form-data">
						...
						<thead >
							...
							$sql_project = select_query_json("select to_char(ap.ADDDATE,'dd-MM-yyyy HH:mi:ss AM') ADDDATE,ap.* from approval_project_master ap where ap.ADDUSER = '".$_SESSION['tcs_usrcode']."' and ap.DELETED = 'N' ORDER BY ap.ADDDATE DESC ","Centra","TEST");
								$c_sql_project=count($sql_project);
									...
									<td class="center" style="text-align:left"><? echo $sql_project[$project_i]['PRJNAME'];?></td>
								...
								<a onclick=" javascript:var pjid = <? echo $sql_project[$project_i]['PRMSCOD'];?>; 
								var pjyr = '<? echo $sql_project[$project_i]['PRMSYER'];?>';getProjectDetails(pjid,pjyr)" data-toggle="modal" type = "" data-target="#modal_large" data-id="<? echo $sql_project[$project_i]['PRMSCOD']; ?>" style='color:black; font-size:12px;cursor:pointer' id="getProject"><i class="fa fa-refresh"></i> Check</a>&nbsp;&nbsp;&nbsp;
								<a target="_blank" href='/approval-desk-test/print_request_id.php?c_year=<?echo $sql_project[$project_i]['PRMSYER'];?>&id=<?echo $sql_project[$project_i]['PRMSCOD'];?>' title='View' alt='View' style='color:black; font-size:12px'><i class="fa fa-print"></i> Print</a>
							 ...
							 <td class="center" style="text-align:center">
							 <a target="_blank" href='/approval-desk-test/admin_modify.php?c_year=<?echo $sql_project[$project_i]['PRMSYER'];?>&id=<?echo $sql_project[$project_i]['PRMSCOD'];?>' title='View' alt='View' style='color:black; font-size:12px'><i class="fa fa-edit"></i> Edit</a>&nbsp;&nbsp;&nbsp;
							 <a target="_blank" href='/approval-desk-test/print_request_id.php?c_year=<?echo $sql_project[$project_i]['PRMSYER'];?>&id=<?echo $sql_project[$project_i]['PRMSCOD'];?>' title='View' alt='View' style='color:black; font-size:12px'><i class="fa fa-print"></i> Print</a>
						 ...
						 href="/approval-desk-test/admin_project_modify_prakash.php?c_year=<?//echo $sql_project[$project_i]['PRMSYER'];?>&id=<?//echo $sql_project[$project_i]['PRMSCOD'];?>">
							...
							<?} ?>
							 ...
							 $sql_project1 = select_query_json("select to_char(ap.ADDDATE,'dd-MM-yyyy HH:mi:ss AM') ADDDATE,ap.* from approval_project_master ap  where ap.DELETED = 'N' and ap.PRJSTAT = 'N' and ap.CAPPUSR = '".$_SESSION['tcs_empsrno']."' ORDER BY ap.ADDDATE desc","Centra","TEST");
								$c_sql_project1=count($sql_project1);
      							for($project_j = 0; $project_j < $c_sql_project1; $project_j++) {
								...
								<td class="center" style="text-align:left"><?echo $sql_project1[$project_j]['PRJNAME'];?></td>
								...
								<a target="_blank"
								 href="/approval-desk-test/admin_modify.php?c_year=<?echo $sql_project1[$project_j]['PRMSYER'];?>&id=<?echo $sql_project1[$project_j]['PRMSCOD'];?>" title='View' alt='View' style='color:black; font-size:12px'><i class="fa fa-edit"></i> Edit</a>&nbsp;&nbsp;&nbsp;
								</td>
					...
					<div class="non-printable" style='clear:both; border-bottom:1px solid #000; margin-bottom:10px; margin-top: 10px;'></div>
						<table id="tbl_approved_list" class="table datatable" style="font-size: 12px;"  >
							...
							<tbody>
								<?
								 //condition to check first stage of approval process
							$sql_project = select_query_json("select to_char(ah.APPDATE,'dd-MM-yyyy HH:mi:ss AM') APPDATE,am.* from approval_project_master am ,approval_project_hierarchy ah  where ah.prmscod = am.prmscod and ah.EMPSRNO = '".$_SESSION['tcs_empsrno']."' and ah.APPSTAT = 'Y' ORDER BY ah.APPDATE,ah.PRMSCOD desc","Centra","TEST");
									$c_sql_project=count($sql_project);
      								for($project_i = 0; $project_i < $c_sql_project; $project_i++) {
										$row = 1;
									?>
			   ...
			   <div class="page-content-wrap">
					...
					<div class="non-printable" style='clear:both; border-bottom:1px solid #000; margin-bottom:10px; margin-top: 10px;'></div>
						<table id="frm_rejected_list" class="table datatable" style="font-size: 12px;"  >
							...
							<tbody>
								<?
								$sql_project_reject = select_query_json("select to_char(ap.DELDATE,'dd-MM-yyyy HH:mi:ss AM') DELDATE,ap.PRMSCOD,ap.PRJNAME,ap.BRNNAME from approval_project_master ap where ap.ADDUSER = '".$_SESSION['tcs_usrcode']."' and ap.DELETED = 'Y' ORDER BY PRMSCOD","Centra","TEST");
								$c_sql_project_reject=count($sql_project_reject);
      								for($project_ij = 0; $project_ij <$c_sql_project_reject; $project_ij++) {
										
									?>
								
									<tr>
									<td><? echo ($project_ij+1);?></td>
									<td><? echo $sql_project_reject[$project_ij]['PRMSCOD'];?></td>
									<td><? echo $sql_project_reject[$project_ij]['PRJNAME'];?></td>
									<td><? echo $sql_project_reject[$project_ij]['BRNNAME'];?></td>
									<td><? if ($sql_project_reject[$project_ij]['BRN_PRJ'] == 'P') {
								  echo "NEW PROJECT";
								}else {
								  echo "BRANCH";
								}?></td>
									<td><? echo $sql_project_reject[$project_ij]['DELDATE'];?></td>

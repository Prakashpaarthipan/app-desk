...
if($action == 'fix_grade') { 
	...
	$where_apprq = "EMPSRNO = '".$empsrno."' and trunc(ADDDATE) = TO_DATE('".$fromdate."', 'DD-MON-YY') ";
	// print_r($field_apprq); echo "<br>";
	$update_apprq = update_test_dbquery($field_apprq, $tbl_apprq, $where_apprq);
	// Update in EMPLOYEE_GRADE_FIX Table

	$yyyymmdd_date = "TO_DATE('".date("Ymd", strtotime($fromdate))."', 'YYYYMMDD')";
	$grade_fixno = select_query_json("Select to_char(NVL(Max(GRDFXNO), TO_CHAR(".$yyyymmdd_date."+1, 'yyyymmdd')||'0000000')+1) MAXGRDFXNO 
												From EMPLOYEE_GRADE_FIX WHERE trunc(ADDDATE) = TO_DATE('".$fromdate."', 'DD-MON-YY')", 'Centra', 'TEST');
	...
	$field_docs['ADDDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$fromdate;

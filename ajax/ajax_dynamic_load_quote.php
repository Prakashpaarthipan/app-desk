...
// common staff increment 
if($apmcode == 1278){$apmcode = 5;}
...
switch ($apmcode) {
	...
	case 1368: ?>
		...
		if(count($sql_night)>0){?>
			<div class='' id="branch" name="branchstaff" style="text-align: left" ><h4>BRANCH EMPLOYEES</h4></div> 
        ...
        foreach ($sql_night as $gift) {$g++;?>
     	<div class="row" style="margin-right: -5px; display: flex; text-transform: uppercase;">
    		...
    		<div class="col-sm-1 colheight" style="padding: 1px 0px; text-align: center; padding-left: 2px;">	
    			  <input type="text"  name="TOTWORK[]" id="totwork_1" placeholder="WORKING HOURS" data-toggle="tooltip" data-placement="top" title="WORKING HOURS" class="form-control"  readonly="readonly"  style=" text-transform: uppercase;height: 25px;" maxlength="10" value="<?=$gift['MOBILNO']?>" >
    		</div>
        </div>
		<?}}?>
		<!-- staff night duty end -->
		<div class='clear clear_both'></div>
		<div id='' style="padding-bottom: 30px; text-align: center;margin-top: 20px">
			<div class="parts4 fair_border">
		<?
		 $sql_night_contractor = select_query_json("select * from approval_contractor_night_duty 
													where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_approval[0]['APRNUMB']."%' 
													order by entsrno", "Centra", 'TEST');
		 if(count($sql_night_contractor)>0){?>
		 	<div class='' id="contractor" name="tablecontractor" style="text-align: left"><h4>CONTRATCTOR DETAILS</h4></div> 
		<div class="row" style="margin-right: -5px; text-transform: uppercase; background-color: #666666; color:#FFFFFF; border-top-left-radius:5px; border-top-right-radius: 5px; display: flex; font-weight: bold;">
		<div class="col-sm-1 colheight"  style="padding: 0px; border-top-left-radius:5px;">
			 &nbsp;#
		</div>
        ...
        <div class="col-sm-2 colheight"  style="padding: 0px;overflow-wrap: break-word;">TOTAL WORKING HOURS</div>
		<div class="col-sm-2 colheight"  style="padding: 0px;">MOBILE </div>
		<!-- <div class="col-sm-1 colheight"  style="padding: 0px;">WORKING HOURS</div> -->
		<!-- <div class="col-sm-1 colheight"  style="padding: 0px;">MOBILE NO</div> -->
        </div>
        <? $g=0;
        foreach ($sql_night_contractor as $gift) {$g++;?>
     		<div class="row" style="margin-right: -5px; display: flex; text-transform: uppercase;">
					<div class="col-sm-1 colheight" style="padding: 1px 0px;">
						<div class="fg-line" style="padding-top: 5px;">&nbsp;<?=$g?></div>
					</div>

					<div class="col-sm-2 colheight" style="padding: 1px 0px;text-align: left;">	
					<label type="text"  data-toggle="tooltip" data-placement="top" title="COMPANY NAME"  style=" text-transform: uppercase;padding-top: 5px;"><?=$gift['CMPNAME']?></label>
												
		
					</div>
					
		            <div class="col-sm-2 colheight" style="padding: 1px 0px; text-align: left; padding-left: 2px;">	
		    			 <label type="text"   data-toggle="tooltip" data-placement="top" title="EMPLOYEE NAME"  style=" text-transform: uppercase;padding-top: 5px;"><?=$gift['EMPNAME']?></label>
		    		</div>
		    		<div class="col-sm-1 colheight" style="padding: 1px 0px;">
						<div id="photo_1">
							<?$folder_path1 = "approval_desk/contractor_night_duty/".$_REQUEST['year']."/";?>
						<img class="img" style="width:100px;height: 100px; " align="center" src="ftp_image_view.php?pic=<?=$gift['PROLINK']?>&path=<?=$folder_path1?>"  title="<? echo $gift['EMPNAME']; ?>"> 	
						
						</div>	
					</div>

		    		<div class="col-sm-2 colheight" style="padding: 1px 0px; text-align: left; padding-left: 2px;">	
		    			 <label type="text"  data-toggle="tooltip" data-placement="top" title="NATURE OF WORK"  style=" text-transform: uppercase;padding-top: 5px;" ><?=$gift['EMPWORK']?></label>
		    		</div>
		    		 
		    		<div class="col-sm-2 colheight" style="padding: 1px 0px; text-align: center; padding-left: 2px;">	
		    			  <label type="text"   data-toggle="tooltip" data-placement="top" title="TOTAL WORKING HOURS"    style=" text-transform: uppercase;padding-top: 5px;" ><?=$gift['WORKHRS']?></label>
		    		</div>
					
		    		<div class="col-sm-2 colheight" style="padding: 1px 0px; text-align: center; padding-left: 2px;">	
		    			  <label type="text"  data-toggle="tooltip" data-placement="top" title="MOBILE"   style=" text-transform: uppercase;padding-top: 5px;" ><?=$gift['MOBILE']?></label>
		    		</div>
	...
	case 623 :
		...
		case 6 :
			$sql_des = select_query_json("select * from approval_staff_designation where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_approval[0]['APRNUMB']."%'", "Centra", 'TCS');
			if(count($sql_des)>0){
			?>
			<div id='' style="padding-left: 10px; text-align: center;">
			<div class="parts3 fair_border">
	...
	case 967 : // ESI Multiple Branch format 
		$sql_des = select_query_json("SELECT * from approval_branch_detail bd, approval_branch_list bl where bd.BRNCODE = bl.BRNCODE and bd.APRNUMB = '".$apprno."'", "Centra", 'TCS');
		if(count($sql_des)>0){ ?>
			<div id='' style="padding-left: 10px; text-align: center;">
			<div class="parts3 fair_border">
				<div class="row" style="margin-right: -5px; text-transform: uppercase; background-color: #666666; color:#FFFFFF; border-top-left-radius:5px; border-top-right-radius: 5px; display: flex; font-weight: bold;">
		     	...
		     	<div class="row" style="margin-right: -5px; display: flex; text-transform: uppercase;">
		            ...
		            <? if($hid_aprnumb == 'S-TEAM / ATTENDANCE 1000068 / 26-04-2018 / 0068 / 02:23 PM') { 
		            		$sql_des = select_query_json("select * from approval_staff_department 
		            											where apryear='".$_REQUEST['year']."' and aprnumb like '%".$sql_approval[0]['APRNUMB']."%'", "Centra", "TEST"); ?>
		            	<div class="col-sm-1 colheight" style="padding: 1px 0px; text-align: center; padding-left: 2px;">	
...
$sql_gen_det = select_query_json("select * from approval_general_detail where aprnumb='".$apprno."' order by rowsrno,colsrno", "Centra", 'TCS');

...
if($action == 'product') { 
	$result = select_query_json("select distinct prd.PRDCODE, prd.PRDNAME, prd.HSNCODE from approval_budget_product bud, product_asset prd, product_asset_gst_per per 
										where bud.PRDCODE = per.PRDCODE and prd.prdcode = bud.prdcode and bud.depcode = '".$depcode."' and PRD.deleted = 'N' and 
											(PRD.PRDCODE like '%".strtoupper($_GET['name_startsWith'])."%' or PRD.PRDNAME like '%".strtoupper($_GET['name_startsWith'])."%') 
										Order by prd.PRDCODE, prd.PRDNAME", "Centra", 'TCS'); // and HSNCODE is not null
	$data = array();
	// print_r($result);
	if(count($result) > 0) {
	...
	if($_GET['product'] != '') {
	    ...
	    $result = select_query_json("select distinct sub.PRDCODE, prd.prdname, sub.SUBCODE, sub.SUBNAME, sub.HSNCODE,sub.prdtax 
	    									from approval_budget_product bud, subproduct_asset sub, product_asset prd 
											where bud.prdcode = sub.prdcode and sub.PRDCODE = prd.PRDCODE and bud.depcode = '".$depcode."' and sub.deleted = 'N' and prd.deleted = 'N' and 
												(sub.SUBCODE like '%".strtoupper($_GET['name_startsWith'])."%' or sub.SUBNAME like '%".strtoupper($_GET['name_startsWith'])."%') 
												and (prd.prdcode = '".strtoupper($expl[0])."') and sub.HSNCODE is not null
									union 
		...
		$result = select_query_json("select distinct sub.PRDCODE, prd.prdname, sub.SUBCODE, sub.SUBNAME, sub.HSNCODE,sub.prdtax 
											from approval_budget_product bud, subproduct_asset sub, product_asset prd 
											where bud.prdcode = sub.prdcode and sub.PRDCODE = prd.PRDCODE and bud.depcode = '".$depcode."' and sub.deleted = 'N' and prd.deleted = 'N' and 
												sub.HSNCODE is not null and (sub.SUBCODE like '%".strtoupper($_GET['name_startsWith'])."%' or 
												sub.SUBNAME like '%".strtoupper($_GET['name_startsWith'])."%') and  (prd.prdcode like '%".strtoupper($_GET['name_startsWith'])."%' or 
												prd.prdname like '%".strtoupper($_GET['name_startsWith'])."%')
									Union
										select distinct sub.PRDCODE, prd.prdname, sub.SUBCODE, sub.SUBNAME, sub.HSNCODE,sub.prdtax
											from approval_budget_product bud, subproduct_asset sub, product_asset prd 
											where bud.prdcode = sub.prdcode and sub.PRDCODE = prd.PRDCODE and bud.depcode = '".$depcode."' and sub.deleted = 'N' and prd.deleted = 'N' and 
												sub.prdtax='N' and (sub.SUBCODE like '%".strtoupper($_GET['name_startsWith'])."%' or sub.SUBNAME like '%".strtoupper($_GET['name_startsWith'])."%') 
												and (prd.prdcode like '%".strtoupper($_GET['name_startsWith'])."%' or prd.prdname like '%".strtoupper($_GET['name_startsWith'])."%')
											order by SUBCODE, SUBNAME, PRDCODE", "Centra", 'TCS');
	}
...
elseif($action == 'sub_prd'){
	$sql_subprd = select_query_json("select distinct prd.PRDCODE, prd.PRDNAME, sub.subcode, sub.subname, sub.HSNCODE, sub.prdtax, decode(sub.prdtax,'Y','YES','N','NO','NO') TAX 
											from approval_budget_product bud, product_asset prd, subproduct_asset sub, product_asset_gst_per per 
											where bud.prdcode = sub.prdcode and prd.prdcode = sub.prdcode and bud.PRDCODE = per.PRDCODE and prd.prdcode = bud.prdcode and 
												bud.depcode = ".$_REQUEST['depcode']." and sub.deleted = 'N' and prd.deleted = 'N' 
 ...
 <?	} 
	...
	$sql_tabltitl = select_query_json("SELECT table_name, column_name FROM USER_TAB_COLUMNS WHERE table_name = '".$tblname."' ".$exp_mastable1." order by COLUMN_ID asc", "Centra", 'TCS');
	$dlt_available = 0;
	...
	if($dlt_available == 0) {
		$sql_tablrecr = select_query_json("SELECT ".$sql_mastable[0]['FLDNAME']." FROM ".$tblname." order by ".$sql_tabltitl[0]['COLUMN_NAME']." asc", "Centra", 'TCS');
	} else {
		$sql_tablrecr = select_query_json("SELECT ".$sql_mastable[0]['FLDNAME']." FROM ".$tblname." WHERE deleted = 'N' order by ".$sql_tabltitl[0]['COLUMN_NAME']." asc", "Centra", 'TCS');
	}

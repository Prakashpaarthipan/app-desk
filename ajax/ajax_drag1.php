...
{	print_r($_REQUEST);
	...
	$sql_cor = select_query_json("select BRNHDSR,BRNHDCD,EMPCODE from approval_branch_head where apmcode = '".$_REQUEST['apmcode']."' and BRNHDCD='1' and deleted='N' order by BRNHDCD,BRNHDSR", "Centra", "TEST");
	print_r($sql_cor);
	$i=-1;
	foreach($sql_cor as $key=>$value)
	{	$i++;
	...
	print_r($newbrnhdsr);
				...
				$g_table = "APPROVAL_BRANCH_HEAD";
				$g_fld = array();
				$g_fld['BRNHDSR'] = $newbrnhdsr;
				$where_appplan="EMPCODE = '".$newempcode."'";
				print_r($g_fld);
				print_r($where_appplan);

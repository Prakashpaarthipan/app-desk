...
function emp_remove_company(gridid) {
   if ($('.parts4 .part3').length == 0) {
      alert("No more row to remove.");
   }
   var id = ($('.parts4 .part4').length - 1).toString();
   $('#partint3').val(id);
   $(".parts4 .part3:last").remove();
...
function emp_night_add_company(gridid) {
	if( ($('.parts4 .part3').length+1) > 99) {
		alert("Maximum 100 Employee allowed.");
    } else {
    	$('[data-toggle="tooltip"]').tooltip();
		var id = ($('.parts4 .part3').length + 2).toString();
        $('#partint3').val(id);
        $('.parts4').append(
	'<div class="part3" style="margin-right: -5px; text-transform: uppercase;">'+
	  '<div class="row" style="margin-right: -5px; display: flex; text-transform: uppercase;">'+
	     ...
	     '</div>'+
	  	  ...
	  	  '<div class="col-sm-2 colheight" style="padding: 1px 0px;">	'+
				'<div>'+
					'<select type="text" name="txt_cmp_name[]" id="txt_cmp_name'+id+'" required="required" maxlength="50" placeholder="Company Name" title="Company Name" class="form-control find_comcode" data-toggle="tooltip"   data-placement="top"  style=" text-transform: uppercase; padding: 2px; height: 25px;"></select>'+
				'</div>'+
				'<div style="clear: both;"></div>'+
			'</div>'+			
            '<div class="col-sm-2 colheight" style="padding: 1px 0px; text-align: center; padding-left: 2px;">'+
    			 '<input type="text"  name="txt_emp_name[]" id="txt_emp_name_'+id+'" placeholder="EMPLOYEE NAME"  maxlength="30" required="required" data-toggle="tooltip" data-placement="top" title="Employee Name" class="form-control" style=" text-transform: uppercase;height: 25px;" >'+
    		'</div>'+
    		'<div class="col-sm-1 colheight" style="padding: 1px 0px; text-align: center; padding-left: 1px;">'+
    		'<input type="file"  name="contractor_photo[]" id="contractor_photo_'+id+'" placeholder="Choose Photo"  accept="image/*"  data-toggle="tooltip" data-placement="top"  title="" class="form-control" style="text-transform: uppercase;">'+
		    		 	'<div id="cphoto_'+id+'"><img id="design'+id+'" class="img" style="height: 100px;width: 100px" align="center" src="" title="Image" ></div>'+
		    		 	'</div>'+
    		'<div class="col-sm-2 colheight" style="padding: 1px 0px; text-align: center; padding-left: 1px;">	'+
    			 '<input type="text"  name="txt_nature_wrk[]" id="txt_nature_wrk_'+id+'" placeholder="NATURE OF WORK" maxlength="40" required="required" data-toggle="tooltip" data-placement="top" title="Nature of Work" class="form-control"  style=" text-transform: uppercase;height: 25px;" >'+
    		'</div>'+

    		'<div class="col-sm-2 colheight" style="padding: 1px 0px; text-align: center; padding-left:1px;">	'+
    			  '<input type="text"  name="txt_total_wrk_hrs[]" id="txt_total_wrk_hrs_'+id+'" placeholder="TOTAL WORKING HOURS" maxlength="10" required="required" data-toggle="tooltip" data-placement="top" title="Total Working Hours" class="form-control"   style=" text-transform: uppercase;height: 25px;" >'+
    		'</div>'+
    		'<div class="col-sm-2 colheight" style="padding: 1px 0px; text-align: center; padding-left: 1px;">	'+
    		'	  <input type="text"  onkeypress="return validateFloatKeyPress(this,event,1);" maxlength="10" required="required" name="txt_mobile[]" id="txt_mobile_'+id+'" placeholder="MOBILE" data-toggle="tooltip" data-placement="top" title="Mobile" class="form-control"   style=" text-transform: uppercase;height: 25px;" >'+
    		'</div>'+
	...
	'</div>');
			...
			$.ajax({
                ...
                success:function(response){
                 ...
                 $("#txt_cmp_name"+id).append("<option value='' selected hidden>CHOOSE THE COMPANY HERE</option>");
	
                 for( var i = 0; i<len; i++){
                  var id1 = response[i]['id'];
                  var name = response[i]['name'];
                  // $("#txt_ledger_code").append("<option value='"+id+"'>"+id+"</option>");
                  $("#txt_cmp_name"+id).append("<option value='"+id1+"'>"+name+"</option>");

                 }
                }
            }); 

            $("#contractor_photo_"+id).on('change', function () {

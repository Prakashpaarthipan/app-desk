...
if($_GET['type'] == 'employee'){
	...
	$sql_timer = select_query_json("select * from approval_timer where aprnumb = '".$aprnumb."' AND ARQSRNO = ".$arqsrno." AND TIMSTAT = 'N' and EDTUSER is null", "Centra", "TEST");
	if(count($sql_timer) > 0) {
		$currentdate = strtoupper(date('d-M-Y h:i:s A'));
		$endtime = find_end_date($start_time, $txt_duration);
		$tbl_dlapprq = "approval_timer";
		$field_dlapprq['EXTNTIM'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$endtime;
		$field_dlapprq['EXTNDUR'] = $txt_duration;
		$field_dlapprq['EDTUSER'] = $_SESSION['tcs_usrcode'];
		$field_dlapprq['EDTDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;
		// print_r($field_dlapprq);
		$where_dlapprq = "aprnumb = '".$aprnumb."' AND ARQSRNO = ".$arqsrno." AND TIMSTAT = 'N' and EDTUSER is null";
		$update_dlapprq = update_dbquery($field_dlapprq, $tbl_dlapprq, $where_dlapprq);
		// echo "**".$update_dlapprq."**";

		if($update_dlapprq == 1) {
			echo "Process Duration : ".$endtime;
		} else {
			echo "0";
		}
	} else {

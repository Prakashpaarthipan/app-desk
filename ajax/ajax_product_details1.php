...
if($action == 'product') { 
	...
	/*$result = select_query_json("select  distinct aprd.PRDCODE, prd.PRDNAME, prd.HSNCODE FROM  TRANDATA.APPROVAL_BUDGET_PRODUCT aprd,trandata.product_asset prd  where aprd.prdcode=prd.prdcode and rownum<5  ","Centra",'TEST');*/
	$data = array();
	// print_r($result);
	if(count($result) > 0) {
	...
	$result = select_testquery("select distinct APSPCCD, APSPCNM from approval_product_specification 

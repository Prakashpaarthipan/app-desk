...
<body>
	...
	<?php
		...
		$sql_approve_leads = select_query_json("select * from APPROVAL_REQUEST ar,  approval_process_type pt, APPROVAL_BUDGET_MODE bm, approval_priority ap
													where  pt.PRJPRCS(+) = ar.PRJPRCS and bm.BUDCODE(+) = ar.BUDCODE and ap.PRICODE(+) = ar.PRICODE and
														ap.deleted(+) = 'N' and bm.deleted(+) = 'N' and pt.DELETED(+) = 'N' and ar.DELETED = 'N' and ar.DELETED in ('N', 'W') and
	...
	<?
		...
		if(count($sql_gift)>0 && $sql_ap[0]['APRNUMB'] != ""){
			...
			<?php
			$sql_descode=select_query_json("Select p.APLCYNM,emp.EMPCODE,emp.EMPNAME,des.DESNAME,sec.ESENAME, to_char(pf.EFCTDAT, 'dd-MM-yyyy') EFCTDAT, to_char(pf.VALDUPT, 'dd-MM-yyyy') VALDUPT, to_char(pf.APPRDAT, 'dd-MM-yyyy') APPRDAT, pf.APLCYCD, pf.PLCYTYP, pf.CRTECNO, pf.CRTUSNM, pf.CRDECNO, pf.CRDUSNM, pf.ASTECNO, pf.ASTUSNM, pf.USERLST, pf.APRVDBY, pf.APRVDUS, pf.DESKPRO, pf.PLCDATA, pf.PLCATTC From approval_policy_form pf, approval_policy_master p,
			employee_office emp,designation des,empsection sec where pf.APLCYCD = p.APLCYCD and emp.EMPCODE=pf.USERLST and emp.ESECODE = sec.ESECODE and emp.DESCODE = des.DESCODE and pf.APRNUMB = '".$sql_reqid[0]['APRNUMB']."'","Centra","TEST");
				foreach($sql_descode as $sectionrow) {
			?>
			<tr>
				<td colspan="3">
						<table style="width:100%; max-width: 773px; min-height:60px; height:auto; border:1px solid #0088CC;padding:2px;">
							...
							<tbody style="text-align:center"> 
										<tr>
								<td class="highlight_column1" style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:50%"> EFFECTIVE DATE : <span class="blue_highlight"><b><?echo $sectionrow['EFCTDAT'];?></b></span> </td>
								<td style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:50%"> POLICY TYPE : <span class="blue_highlight"><b><?echo $sectionrow['PLCYTYP'];?></b></span></td>
							</tr>
										<tr>
								<td class="highlight_column1" style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:50%"> VALID UPTO : <span class="blue_highlight"><b><?echo $sectionrow['VALDUPT'];?></b></span></td>
								<td style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:50%"> CREATOR EC NO \ NAME : <span class="blue_highlight"><b><?echo $sectionrow['CRTECNO'];?> - <?echo $sectionrow['CRTUSNM'];?></b></span></td>
							</tr>
										<tr>
								<td class="highlight_column1" style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:50%"> APPROVED DATE : <span class="blue_highlight"><b><?echo $sectionrow['APPRDAT'];?></b></span></td>
								<td style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:50%"> CO-ORDINATOR EC NO \ NAME : <span class="blue_highlight"><b><?echo $sectionrow['CRDECNO'];?> - <?echo $sectionrow['CRDUSNM'];?></b></span></td>
							</tr>
							<tr>
								<td class="highlight_column1" style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:50%">  USER LIST : <span class="blue_highlight"><b><?echo $sectionrow['EMPCODE'];?> - <?echo $sectionrow['EMPNAME'];?> - <?echo $sectionrow['ESENAME'];?> - <?echo $sectionrow['DESNAME'];?> </b></span></td>
								<td style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:50%"> ASSIST BY EC NO \ NAME : <span class="blue_highlight"><b><?echo $sectionrow['ASTECNO'];?> - <?echo $sectionrow['ASTUSNM'];?></b></span></td>
							</tr>
							<?/*<tr>
							...
							<tr>
							<td colspan="2" class="highlight_column1" style="text-align:left; padding:-5px; font-weight: bold; border: 1px solid #e0e0e0;width:35%">  DESK PROCEDURE : <span class="blue_highlight"><b><?
						...
						<tr>
							<td colspan="2" class="highlight_column1" style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:35%">  POLICY DOCUMENTS : <span class="blue_highlight"><b><?echo $sectionrow['PLCATTC'];?></b></span></td>
						</tr>
						</tbody>
					</table>
				</td>
			...
			<tr colspan="3">
				...
				<table style="width:100%; max-width: 773px; min-height:60px; height:auto; border:1px solid #0088CC;padding:2px;">
					<tbody style="text-align:center">
					<tr>
					<td colspan="2" class="highlight_column1" style="text-align:left; font-weight: bold; border: 1px solid #e0e0e0;width:35%;padding:3px 5px"><br>  Details <span class="blue_highlight"><b>: </b></span>	
					 <?  //echo "..".$sql_approve_leads[0]['APPRFOR']."**".$sectionrow['PLCDATA']."**".$sql_approve_leads[0]['APPRDET']."**";
					 if($sql_approve_leads[0]['APPRFOR'] == '1') {
					$filepathname = $sectionrow['PLCDATA'];

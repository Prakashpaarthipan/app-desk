                        ...
                        <table id="customers2" class="table datatable">
                            ...
                            <?  $and = "";
                                ...
                                if($and != '') {
                                    ...
                                    $sql_brn = select_query_json("select BRNCODE,esecode from trandata.employee_office@tcscentr where empsrno = '".$_SESSION['tcs_userid']."'", "Centra", 'TCS');
                                    ...
                                    $sql_search = select_query_json("select to_char(ar.ADDDATE,'dd-MON-yyyy hh:mi:ss AM') ADDDATE, ar.FINDATE, ar.APRNUMB, ar.APPSTAT, ar.APPRSUB, ar.APPRFOR,
                                                                            ...
                                                                            order by ADDDATE desc, APPORDER asc, APRNUMB desc", "Centra", 'TCS');
                                }else{
                                    /* $rqusr = " and ar.ADDUSER = '".$_SESSION['tcs_empsrno']."' ";
                                    $sql_search = select_query_json("select ar.ADDDATE, ar.FINDATE, ar.APRNUMB, ar.APPSTAT, ar.APPRSUB, ar.APPRFOR, ar.APPFVAL, ar.APRTITL, ar.ARQSRNO,
                                                                        ...
                                                                        union
                                                                            ...
                                                                            from approval_request ar
                                                                            where ar.ARQSRNO = 1 ".$rqusr." ".$and." and ar.deleted = 'N'  and ar.arqyear not in ('2016-17')
                                                                            order by ADDDATE asc, APPORDER asc, APRNUMB desc", "Centra", 'TCS'); */
                                }
                                ...
                                for($search_i = 0; $search_i < count($sql_search); $search_i++) { $ij++;
                                    ...
                                    <tr class="odd gradeX" style='background-color:<?=$bgclr?>; color:<?=$clr?>;'>
                                        ...
                                        <td><a href='view_approval.php?action=view&reqid=<? echo $sql_search[$search_i]['ARQCODE']; ?>&year=<? echo $sql_search[$search_i]['ARQYEAR']; ?>&rsrid=1&creid=<? echo $sql_search[$search_i]['ATCCODE']; ?>&typeid=<? echo $sql_search[$search_i]['ATYCODE']; ?>' title='View' alt='View' style='color:<?=$clr?>; font-weight:normal; font-size:10px;'><? echo $sql_search[$search_i]['APRNUMB']; ?></a>
                                        ...
                                        <td class="center" style='text-align:center; white-space:nowrap;'>
                                            ...
                                            <a target="_blank" href='view_approval.php?action=view&reqid=<? echo $sql_search[$search_i]['ARQCODE']; ?>&year=<? echo $sql_search[$search_i]['ARQYEAR']; ?>&rsrid=<? echo $sql_search[$search_i]['ARQSRNO']; ?>&creid=<? echo $sql_search[$search_i]['ATCCODE']; ?>&typeid=<? echo $sql_search[$search_i]['ATYCODE']; ?>' title='View' alt='View' style='color:<?=$clr?>;'><i class="fa fa-eye"></i> View</a> <br>

    ...
    <div class="page-container page-navigation-toggled page-container-wide page-navigation-top-fixed">
		...
		<form role="form" id='frm_project' name='frm_project' action='' method='post' enctype="multipart/form-data">
			<div class="col-lg-12 col-md-12 tooltip-demo" style='border-right: 1px solid #d4d4d4;'>
				...
				<!-- Approval Management Users -->
                                                    <div class="form-group">
                                                        <label class="col-md-3 control-label"><span style='color:red'>Employee*</span></label>
                                                        <div class="col-md-9 col-xs-12">
                                                            <?  $sql_emp= select_query_json("select emp.EMPCODE,emp.EMPNAME from employee_office emp,tlu_employee_rights tim where tim.EMPCODE=emp.EMPCODE", "Centra", 'TCS');
                                                                    if($_REQUEST['action'] == 'view') {
                                                                    echo ": ".$sql_reqid[0]['EMPCODE']."-".$sql_reqid1[0]['EMPNAME'];
                                                                } else { ?>
                                                                    <input type='text' class="form-control" tabindex='6' style="text-transform: uppercase;" required name='txt_empcode' id='txt_empcode' data-toggle="tooltip" onblur="find_tags();" data-placement="top" data-original-title="Employee"value='<?=$sql_reqid[0]['EMPCODE']."-".$sql_reqid1[0]['EMPNAME']?>'>
                                                            <?  } ?>
                                                        </div>
                                                    </div>
					...
					<div class="col-lg-9 col-md-9">
						<? if($_REQUEST['action'] == 'view') { ?>
							: <?=$sql_reqid[0]['PRSCODE']?>
						<? } else { ?>
							<select class="form-control" tabindex='1' required name='txt_prscode' id='txt_prscode' autofocus required data-toggle="tooltip" data-placement="top" title="Process Code">
								<option>1</option>
								<option>2</option>
								<option>3</option>
								<option>4</option>
		...
		<? } else { ?>
			<div class="panel panel-default">
					<div class="panel-heading">
						<i class="fa fa-check-circle-o fa-fw"></i> TailYou Employee Management

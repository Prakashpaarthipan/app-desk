...
include('lib/process_connect.php');
extract($_REQUEST);
...
<html>
 ...
 <!--  <link rel="stylesheet" href="../dist/newlte/bower_components/font-awesome/css/font-awesome.min.css">
  <link rel="stylesheet" href="../dist/newlte/bower_components/font-awesome/fonts/fontawesome-webfont.woff"> -->
  <!-- Ionicons -->
...
<body oncopy="return false" oncut="return false" onpaste="return false"	ondragstart="return false" onselectstart="return false" oncontextmenu="return false" class="skin-black side sidebar-collapse">
   <div id='pageloader' class="loader"></div>
	...
	$('#supplier').select2({
			...
			ajax: {
	          url: 'ajax_complaint_entry.php?mode=supplier',
	          dataType: 'json',
			  delay: 250,
...
$('#newsave').click(function(){
	...
	function get_sup_details(supp){
		$.ajax({
		url:"ajax_complaint_entry.php?mode=supp_detail&supcode="+supp,
		}).done(function(data) 
		{	
			data1 = data.split('~');
			phn = data1[0].split(',');
...
function saveComplaint()
	{
		...
		$('.loader').show();
		$.ajax({
		url:"ajax_complaint_entry.php?mode=SAVE",
		type: "POST",
		data: form_data,
		processData: false,
		contentType: false 

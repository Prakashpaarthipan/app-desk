...
// $sql_approval_type_mode = select_query_json("select * from approval_master where ATMCODE = '".$cm."' and ATYCODE = '".$slt_submission."' and DELETED = 'N' order by APMNAME Asc", "Centra", 'TCS');
if($slt_submission == 1) {
...
<div class="form-group trbg">
	...
	<div class="col-lg-9 col-md-9"> */ ?>
		<select class="form-control custom-select chosn" tabindex='6' required name='slt_approval_listings' id='slt_approval_listings' data-toggle="tooltip" data-placement="top" title="Approval Subject" <? if($_REQUEST['action'] == 'edit') { ?> disabled <? } else { ?> onChange="get_advancedetails(); getapproval_listings(this.value)" onblur="call_days()" <? } ?>>
		<option value=''>Choose Approval Listings</option>

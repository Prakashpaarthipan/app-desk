...
<div class="form-group trbg" style='min-height:40px;'>
	...
	//echo("<pre>");
	$txt_user=explode(' - ',$txt_rate_finalized);
	//print_r($_REQUEST);
	// if($txtrequest_value > 0 and $txt_rate_finalized != '') {
	if($txt_user[0] != '' and ($budgetidtype == 1 or $budgetidtype == 6 or $budgetidtype == 7 )) {
		/* $sql_dptinchr1 = select_query_json("select EMPSRNO, EMPCODE APPHEAD, EMPNAME, 'DEPARTMENT INCHARGE' APPTITL, 1 allow_days from approval_branch_head 
														where deleted = 'N' and brncode = ".$slt_brnch." and depcode = ".$deptid." and tarnumb = ".$tarnum." 
														ORDER BY BRNHDCD, BRNHDSR", "Centra", 'TCS');  */
		...
		$flag==0;
		foreach ($sql_dptinchr1 as $key => $value) {
			if($value['APPHEAD']==$txt_user[0])
			{
				$flag=1;
			}		
		}
		...
		if($flag==0)
		{
			$_SESSION['sess_1'] .= $txt_user[1].' - '.$txt_user[0].' [ 1 Day(s) ] <br>';
			$_SESSION['sess_2'] += 1;
			$_SESSION['sess_3'] .= $txt_user[0]."~~";
			$_SESSION['chkexistuser'][] = $txt_user[0];
		}
	}
	//print_r($sql_dptinchr1);
	//exit();
	...
	if($budgetidtype == 10){
		...
		$sql_trno = select_query_json("select * from trandata.non_purchase_target@tcscentr tar, trandata.department_asset@tcscentr Dep 
											where tar.depcode=dep.depcode and dep.expsrno=".$slt_expense_approval_listings." and tar.brncode=".$slt_brnch." and tar.ptnumb > 9000 
												and trunc(sysdate) between trunc(PTFDATE) and trunc(PTTDATE)");
		
		if($type == 10 or $type == 11 ) { // fixed budget & reserved budget
			$sql_dptinchr1 = select_query_json("select EMPSRNO, EMPCODE APPHEAD, EMPNAME, 'DEPARTMENT INCHARGE' APPTITL, 1 allow_days, min(aprvalu) aprvalu, max(BRNHDSR) brnhdsr 	
														from approval_branch_head 
														where brncode = ".$sql_trno[0]['BRNCODE']." and depcode = ".$sql_trno[0]['DEPCODE']." and 
															tarnumb = ".$sql_trno[0]['PTNUMB']." and deleted = 'N' and APRVALU > 0  
														group by EMPSRNO, EMPCODE, EMPNAME, 'DEPARTMENT INCHARGE', 1, BRNHDCD
														ORDER BY BRNHDCD, BRNHDSR, APRVALU", "Centra", 'TCS');
		} 
	...
	if($budgetidtype == 8) { // Only AGREEMENT Type of Submission
		...
		foreach ($sql_dptinchr1 as $key => $dptincharge_value) {
			$sql_usractive = select_query_json("select * from employee_office where empcode = '".$dptincharge_value['APPHEAD']."'", "Centra", 'TCS');
			if(count($sql_usractive) > 0) {
	...
	elseif($budgetidtype == 4 or $budgetidtype == 9 ) { // Only INTERNAL REQUEST Type of Submission
		...
		find_hod_by_selected_dept($_SESSION['tcs_esecode'], $slt_core_department, $approval_listings_id); // Chosen Department Based HOD / DGM
		...
		if($slt_brnch == 114 and $approval_listings_id == 1368) { // Tailyou & Night Duty Approval must display SK sir
			$sql_dptinchr1 = select_query_json("select EMPSRNO, EMPCODE APPHEAD, EMPNAME, 'DEPARTMENT INCHARGE' APPTITL, 1 allow_days from approval_branch_head 
														where deleted = 'N' and apmcode = '".$approval_listings_id."' and brncode = ".$slt_brnch."
														ORDER BY BRNHDCD, BRNHDSR", "Centra", 'TCS'); 
		} else {
			$sql_dptinchr1 = select_query_json("select EMPSRNO, EMPCODE APPHEAD, EMPNAME, 'DEPARTMENT INCHARGE' APPTITL, 1 allow_days from approval_branch_head 
														where deleted = 'N' and apmcode = '".$approval_listings_id."' and brncode = 888
														ORDER BY BRNHDCD, BRNHDSR", "Centra", 'TCS'); 
		}
		...
		if($type == 7 and $slt_brnch != 120 and $slt_brnch != 114 and $slt_brnch != 121 and $slt_brnch != 33 and $vl > 0 and $slt_project != 38) { // new proposal must add KS Sir except tailyou, clean today, vr mall project / branches

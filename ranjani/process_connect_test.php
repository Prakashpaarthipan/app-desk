...
try{
	...
	for($keybrn = 0; $keybrn < count($slt_brnch); $keybrn++) { // branch for loop
       ...
       $project = select_query_json("select * from approval_policy_master where DELETED = 'N' order by aplcysr", "Centra", 'TCS');
		...
		// if(count($sql_reqexist) <= 0) {
			...
			{
				$m_table = "approval_salary_policy";
				$m_fld = array();
                $m_fld['APRYEAR'] = $current_year[0]['PORYEAR'];
				$m_fld['APRNUMB'] = $apprno;
				$m_fld['POLSUBJ'] = strtoupper($txtdynamic_subject);				
				$m_fld['EFFDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$datepicker_example11;
				$m_fld['POLTYPE'] = strtoupper($txtdynamic_policy_type);
				$m_fld['VALUPTO'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$datepicker_example12;
				$m_fld['CRENAME'] = strtoupper($txtdynamic_creator);
                $m_fld['APPDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$datepicker_example13;
				$m_fld['COODATE'] = strtoupper($txtdynamic_coordinator);
				$m_fld['USELIST'] = strtoupper($txt_dynamic_uselist);
				$m_fld['ASISTBY'] = strtoupper($txtdynamic_assistby);
				$m_fld['DESKPRO'] = strtoupper($txtdynamic_deskpro);
				$m_fld['POLIDOC'] = strtoupper($txtdynamic_policy_docs);
				$m_fld['FILEPOL'] = strtoupper($FCKeditor2);
				print_r($m_fld);

				echo "+++++++++";$m_insert = insert_test_dbquery($m_fld, $m_table);
			}

...
{	//print_r($_REQUEST);
	...
	$prevbrnhdcd=array();
	$prevbrnhdsr=array();
	$emp=array();$newempcode=array();$newbrnhdsr=array();
	//echo "select BRNHDSR,BRNHDCD,EMPCODE from approval_branch_head where apmcode = '".$_REQUEST['apmcode']."' and deleted='N' and BRNHDCD='1' order by BRNHDCD,BRNHDSR";
	$sql_cor = select_query_json("select (select MAX(BRNHDSR) from approval_branch_head where apmcode = '".$_REQUEST['apmcode']."' and BRNHDCD='1') num, BRNHDSR,BRNHDCD,EMPCODE,APRVALU from approval_branch_head where apmcode = '".$_REQUEST['apmcode']."' and BRNHDCD='1' and DELETED='N' order by BRNHDCD,BRNHDSR", "Centra", "TEST");
	print_r($sql_cor);
	$i=-1;
	// $emp_list=array();
	// foreach ($sql_cor1 as $key => $value) {
	// 	$emp_list[$value['EMPCODE']][$value['APRVALU']]=$value;
	// }
	// print_r($_REQUEST);
	// print_r($emp_list);

	// foreach ($_REQUEST['newflow'] as $key => $value) {
	// 	foreach ($emp_list[$value as $key => $value) {
	// 		# code...
	// 	}
	// }
	// exit();
	foreach($sql_cor as $key=>$value)
	{	$i++;
		$arr[$value['EMPCODE']]=$value;
		$prevbrnhdcd[$i]=$value['BRNHDCD'];
		$prevbrnhdsr[$i]=$value['BRNHDSR'];
		$aprvalu[$i]=$value['APRVALU'];
		$emp[$i]=$value['EMPCODE'];
	}
	$newflow=$_REQUEST['newflow'];
	for($j=0;$j<sizeof($newflow);$j++){
	 $key=array_search($newflow[$j],$emp);	
	 $newempcode[$j]=$newflow[$j];
	 $newbrnhdsr[$j]=$prevbrnhdsr[$key];
	}
 $number=$sql_cor[0]['NUM'];
	...
	for($x=0;$x<sizeof($newempcode);$x++){
				$g_table = "APPROVAL_BRANCH_HEAD";
				$g_fld = array();
				$g_fld['BRNHDSR'] =$number+$x+1;
				$where_appplan="EMPCODE = '".$newempcode[$x]."' and BRNHDCD='".$prevbrnhdcd[$x]."' and APRVALU='".$aprvalu[$x]."' and DELETED='N'";
				print_r($g_fld);
				print_r($where_appplan);

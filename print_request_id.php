...
if($_REQUEST['finstat'] == 'c') {
	$sql_reqid = select_query_json("select ar.*, to_char(APPRSFR,'hh:mi:ss AM') APPRSFR_crt_Time, to_char(APPRSFR,'dd-MON-yyyy hh:mi:ss AM') APPRSFR_Time,
											...
											(select EMPCODE||' - '||EMPNAME from employee_office where EMPCODE = ar.RTFNUSR) RATE_FINALIZEBY
...
$sql_tarbalance = select_query_json("select R.*, ast.EXPSRNO, ast.EXPNAME EXPHEAD, to_char(R.APPRSFR,'hh:mi:ss AM') APPRSFR_crt_Time, to_char(R.APPRSFR,'dd-MON-yyyy hh:mi:ss AM') APPRSFR_Time,
											...
											(select EMPCODE||' - '||EMPNAME from employee_office where EMPCODE = R.RTFNUSR) RATE_FINALIZEBY
...
/*
PRAKASH PROJECT CREATION TASKS CUSTOME CODE START HERE!!!
*/

 $sql_project_master = select_query_json("select apm.* ,to_char(apm.ADDDATE,'dd-MON-yyyy hh:mi:ss AM') PRJDATE,brn.BRNNAME,brn.NICADDR from approval_project_master apm,branch brn where apm.PRMSCOD = '".$id."' and apm.PRMSYER='".$c_year."' and apm.brncode=brn.brncode","Centra","TEST");
 //print_r($sql_project_master);
$len = sizeof($sql_project_master);
?>
...
<body>
	...
	<?php
		...
		$sql_approve_leads = select_query_json("select * from APPROVAL_REQUEST ar, approval_project pr, approval_process_type pt, APPROVAL_BUDGET_MODE bm, approval_priority ap
													where ar.APRCODE = pr.APRCODE and pt.PRJPRCS(+) = ar.PRJPRCS and bm.BUDCODE(+) = ar.BUDCODE and ap.PRICODE(+) = ar.PRICODE and ap.deleted(+) = 'N' and 
														bm.deleted(+) = 'N' and pt.DELETED(+) = 'N' and ar.DELETED = 'N' and pr.DELETED in ('N', 'W') and ar.ARQCODE = '".$_REQUEST['reqid']."' and 
														ar.ARQYEAR = '".$_REQUEST['year']."' and ar.ATCCODE = '".$_REQUEST['creid']."' and ar.ATYCODE = '".$_REQUEST['typeid']."'
		...
		$sql_hir = select_query_json("with t as (select apphead from approval_mdhierarchy
											where aprnumb in ('".$sql_approve_leads[0]['APRNUMB']."') and apphead in (1,2,3)) 
												select * from t pivot(count(APPHEAD)for(apphead) in (1 as KS,2 as PS,3 as AK))", 'Centra', 'TEST');
	?>
	<div class="page">
		<? if($agnpr == 1) { ?><div id="watermark"></div><? } ?>
	...
	<?php if($viewonly == 0) { ?>
		<tr>
			<td style='width:70%; height:20px; font-weight:bold; vertical-align: top;'>
				<label style='font-size:13px;'><?if($len != 0){echo $sql_project_master[0]['PRJNAME']." / ".$sql_project_master[0]['PRMSYER']." - ".$sql_project_master[0]['PRMSCOD'];}else{ echo $sql_reqid[0]['APRNUMB'];}?></label> <!-- Approval Number -->
			</td>
			<td style='width:30%; height:20px; text-align:right;'>
				...
				<? if($_REQUEST['rsrid'] != 1 and $_REQUEST['rsrid'] != ''){ ?>
					<label>Approve on : <?=$systemdate?><? /* .(<?=$_SESSION['tcs_user']?>) */ ?></label> <!-- Current Date & Time -->
				<? }else{ ?>
					<label style="font-size:14px">Print on : <?=date("d/m/Y h:i A")?>.(<?=$_SESSION['tcs_user']?>)</label> <!-- Current Date & Time -->
				<? } ?>
			</td>
		</tr>
	<?php }else{ ?>
		<tr>
			<td style='width:70%; height:20px; font-weight:bold;'>
				<label style='font-size:13px;'><?=$sql_project_master[0]['PRJTITL']?></label> <!-- Approval Number -->
			</td>
	...
	<?php } ?>
			...
			<table width='100%'>
				...
				<tr>
					<td colspan=3 style='width:100%; height:20px; text-align:right;'>
						<label style="font-size:14px">Date : <?=$sql_project_master[0]['PRJDATE']?><br>
							Process Priority : <?
									switch ($sql_project_master[0]['PRIMODE']) {
										case 1:
											$clrcod = 'badge-ap1';
											$clrcod1 = '#FF0000';
											break;
									...
									<span class="badge <?=$clrcod?>" style="font-size:20px; font-weight:bold;"><? echo "AP-".$sql_project_master[0]['PRIMODE']; ?></span>
								<?/* } else { ?>
									<span class="badge badge-success" style="font-size:20px; background-color:#299654; font-weight:bold;"><? echo "AP-".$sql_approve_leads[0]['PRICODE']; ?></span>
								<? }*/ ?>
						</label> <!-- Created Date -->
					</td>
				</tr>
		...
		<?	$kind_attn = 'Sri. KS Sir';
			...
			$sql_approve_comnts = select_query_json("select req.*, regexp_replace(SubStr(req.REQESEN,1,4),'[0-9]','')||SubStr(req.REQESEN,5,100) REQ_ESEN,
															...
															where DELETED = 'N' and ARQCODE = '".$_REQUEST['reqid']."' and ARQYEAR = '".$_REQUEST['year']."' and ATCCODE = '".$_REQUEST['creid']."' 
																and ATYCODE = '".$_REQUEST['typeid']."' and ARQSRNO not in (1) and aprnumb = '".$sql_reqid[0]['APRNUMB']."'
															order by ATCCODE, ARQCODE, ARQSRNO desc, ATYCODE", 'Centra', 'TEST');
			// print_r($sql_approve_comnts);
		...
		<td colspan=2 style='width:100%; vertical-align:top; text-align:left;'>
			<table border=0 style='width:100%; max-width: 773px; '>
				...
				<tr style='min-height:20px; line-height:20px;'>
					...
					<td style='width:80%; text-align:left;'>
						<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? echo $sql_project_master[0]['PRJNAME']; /* if(trim($sql_approve_leads[0]['DYNSUBJ']) != '-') { echo rtrim($sql_approve_leads[0]['DYNSUBJ'], " - "); } /* if(trim($sql_approve_leads[0]['TXTSUBJ']) != '-') { echo $sql_approve_leads[0]['TXTSUBJ']; } */ ?></span></label> <!-- Approval Listings -->
					</td>
				</tr>

				<? if($sql_approve_leads[0]['DYSBFDT'] != '') { ?>
					<tr style='min-height:20px; line-height:20px;'>
						<td style='font-size:16px; font-weight:bold; width:20%; text-align:left;'>
							<label>Period</label> <!-- AMC Period -->
						</td>
						<td style='width:80%; text-align:left;'>
							<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? if($sql_approve_leads[0]['DYSBFDT'] != '-') { echo strtoupper(date("d-M-Y", strtotime($sql_approve_leads[0]['DYSBFDT'])) ." TO ". date("d-M-Y", strtotime($sql_approve_leads[0]['DYSBTDT']))); } ?></span></label> <!-- AMC Period -->
						</td>
					</tr>
				<? } elseif(rtrim($sql_approve_leads[0]['DYNSUBJ'], " - ") != '') { ?>
					<tr style='min-height:20px; line-height:20px;'>
						<td style='font-size:16px; font-weight:bold; width:20%; text-align:left;'>
							<label>Period</label> <!-- Month Period -->
						</td>
						<td style='width:80%; text-align:left;'>
							<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? if(trim($sql_approve_leads[0]['DYNSUBJ']) != '-') { echo rtrim($sql_approve_leads[0]['DYNSUBJ'], " - "); } ?></span></label> <!-- Month Period -->
						</td>
					</tr>
				<? } ?>
				

				<? if(trim($sql_approve_leads[0]['TXTSUBJ']) != '') { ?>
					<tr style='min-height:20px; line-height:20px;'>
						<td style='font-size:16px; font-weight:bold; width:20%; text-align:left;'>
							<label>Details</label> <!-- Approval Listings -->
						</td>
						<td style='width:80%; text-align:left;'>
							<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? if(trim($sql_approve_leads[0]['TXTSUBJ']) != '-') { echo $sql_approve_leads[0]['TXTSUBJ']; } ?></span></label> <!-- Approval Listings -->
						</td>
					</tr>
				<? } ?>
			<tr style='min-height:25px; line-height:25px;'>
				<td style='width:20%; padding-top:5px; text-align:left;'>
						<label style=' font-size:16px; font-weight:bold'>Branch </label> <!-- Project Name -->
				</td>
				<td style='width:80%; padding-top:5px; text-align:left;'>
						<table style="width: 100%">
						<tr style='min-height:20px; line-height:20px;'>
							<td style='width:70%; text-align:left;'>
								<label>: <label style=' font-size:16px; font-weight:bold;'><?=$sql_project_master[0]['BRNCODE']."-".$sql_project_master[0]['NICADDR']?> </label>
				...
				<tr style='min-height:25px; line-height:25px;'>
					...
					<td style='width:80%; padding-top:5px; text-align:left;'>
						...
						<tr style='min-height:20px; line-height:20px;'>
							<td style='width:70%; text-align:left;'>
								<label>: <label style=' font-size:16px; font-weight:bold;'><?=$sql_project_master[0]['PRJNAME']."/".$sql_project_master[0]['PRMSCOD']?> / </label>
								<label style=' font-size:16px; font-weight:bold;border: 0px solid #00a1ff;padding: 3px;'><?=$sql_approve_leads[0]['APRCODE']." - ".$sql_approve_leads[0]['APRNAME']?></label> [ <?=$sql_approve_leads[0]['PRSTYPE'];?> ] <!-- Project Name -->
							</td>
				...
				<?  $sql_renewal = select_query_json("select * from APPROVAL_AGREEMENT_RENEWAL
                                                                         where aprnumb = '".$sql_reqid[0]['APRNUMB']."' and DELETED = 'N'", "Centra", 'TEST');
																							if(count($sql_renewal)!= 0){ ?>
					<tr style='min-height:20px; line-height:20px;'>
						<td style='font-size:16px; font-weight:bold; width:20%; text-align:left;'>
							<label>Renewal Period</label> <!-- RENEWAL Period -->
						</td>
						<td style='width:80%; text-align:left;'>
							<span style="font-size:12px; font-weight: normal; line-height:20px;">:</span> <span style=" font-size: 12px; font-weight: bold;" class="blue_highlight"><?=strtoupper(date('d-M-Y',strtotime($sql_renewal[0]['AGFRDAT'])));?>  -  <?=strtoupper(date('d-M-Y',strtotime($sql_renewal[0]['AGTODAT'])));?></span></label> <!-- RENEWAL Period -->
						</td>
					</tr>
					<?}?>
				<?if($len !=0){?>
				<tr style='min-height:25px; line-height:25px;'>
					<td style='width:20%; padding-top:5px; text-align:left;'>
						<label style=' font-size:16px; font-weight:bold'>Due Date</label> <!-- Project Due -->
					</td>
						<td style='width:80%; padding-top:5px; text-align:left;'>
							: <label style=' font-size:16px; font-weight:bold;'><?if($len !=0){echo date_format(date_create($sql_project_master[0]['DUEDATE']), 'd-m-Y');}?></label>
						</td>
				</tr>	
				<?}?>
...
?>
					...
					<tr style='min-height:30px; vertical-align:top; line-height:30px; font-size: 12px;'>
					<td style='width:60%; text-align:left;'>
										<label style='height:27px;'>DEPARTMENT :&nbsp;<span style='color:red'></span></label>
									<ol><b>																	
									<?foreach($sql_timer as $key => $value){
									?>
									<li><?=$value['TARNUMB']."-".$value['TARNAME']?>&nbsp; : <span style="color:red">Rs.<?=$value['PRJVALU']?></span></li> 
									<?}?>
									</b>
									</ol>
								</td>
								<td style='width:40%; text-align:left;'>
										...
										<label style='height:27px;'>PROJECT PERSONS:&nbsp;<span style='color:red'></span></label>
										<ol>
									<b>
									<?$sql_timer1 = select_query_json("select * from approval_project_head where PRMSYER = '".$c_year."' and PRMSCOD = '".$id."' and PRJTITL in (1,2,3) and DELETED = 'N' order by PRJTITL ", "Centra", "TEST");
									//print_r($sql_timer1);
									//print_r($sql_timer);
									foreach($sql_timer1 as $key => $value1){
									?>
									<li><?=$value1['EMPCODE']."-".$value1['EMPNAME']?>&nbsp;<span><?if($value1['PRJTITL'] == 1){echo "-Owner" ;}elseif($value1['PRJTITL'] == 2){echo "-Head";}elseif($value1['PRJTITL'] == 3){echo "-Member";}else{ echo "";}?> </span></li> 
									<?}?>
									</ol>
									</b>
								</td>
...
<?}?>
						<table border=0 width='100%' style='max-width: 773px; border: 1px solid #0088CC; min-height: 70px; height:auto; padding:3px; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px;'>
							...
							<tr style=''>
								<td style='width:100%; text-align:left; max-width: 760px !important; overflow-x: auto !important;'>
									<label class="print_table"><? // echo $sql_approve_leads[0]['APPRDET'];
							...
							<tr><td colspan='6'>
									...
									<tbody>
										...
										?>
											<tr class="active">
												...
												<td class="colheight" style="padding: 0px;width: 10%;text-align:left">
													<?
														...
														foreach($sql_descode as $sectionrow_emp) {
															if ($sectionrow_emp['EMPCODE'] == '0') {
																$sql_descode1=select_query_json("select * from trandata.APPROVAL_branch_head@tcscentr 
					                                                                                    ...
					                                                                                    order by brnhdsr", "Centra", "TCS");
																if(count($sql_descode1) > 0) {
								...
								<?
									...
									if($edtvl == 1) {
										...
										// $sql_plan = select_query_json("select * from approval_budget_planner where deleted = 'N' and aprnumb = '".$sql_reqid[0]['APRNUMB']."' order by APRSRNO", 'Centra', 'TCS');
										$ijk = 0;
												    										...
												    										/* union
												    											select qut.SUPCODE, qut.SUPNAME, cty.CTYNAME, sup.SUPMOBI, qut.DELPRID, qut.SUPRMRK
												    													...
												    													where qut.supcode = sup.supcode and cty.ctycode = sup.ctycode and qut.SLTSUPP = 1 and
												    														...
												    														qut.PBDLSNO = '".$prdlist['PBDLSNO']."'", 'Centra', 'TCS'); */ ?>
										   <tr><td><? echo $sql_prdquotlist[0]['GPSNAME']; ?> : <b style="font-size: 14px; font-weight: bold;"><?=$sql_prdquotlist[0]['SUPCODE']." - ".$sql_prdquotlist[0]['SUPNAME']; /* ." - ".$sql_prdquotlist[0]['CTYNAME']." - ".$sql_prdquotlist[0]['SUPMOBI'] */ ?></b>; <span style="font-size: 9px;color: #a0a0a0;">( Delivery Duration : <?=$sql_prdquotlist[0]['DELPRID']?> Days ) ( Due Date : <?=strtoupper(date("d-M-Y", strtotime($prdlist['PRDDEDT'])))?> - <?=strtoupper(date("d-M-Y", strtotime($prdlist['PRDEDDT'])))?> ) ( <?=$sql_prdquotlist[0]['SUPRMRK']?> )</span></td></tr>
			...
			$sql_ap = select_query_json("select distinct aprnumb from approval_request
												where ARQCODE = '".$_REQUEST['reqid']."' and ARQYEAR = '".$_REQUEST['year']."' and ATCCODE = '".$_REQUEST['creid']."' and
													ATYCODE = '".$_REQUEST['typeid']."' and  deleted='N'", 'Centra', 'TEST');
	...
	<?	$sql_night = select_query_json("select * from approval_night_duty where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_ap[0]['APRNUMB']."%' order by entsrno", 'Centra', 'TEST');
		if(count($sql_night)>0 && $sql_ap[0]['APRNUMB'] != ""){
			echo "<br>"; ?>
			<!--<div class='' id="contractor" name="tablecontractor" style="text-align: left;margin-top: 20px"><h4>CONTRATCTOR DETAILS</h4></div>-->
			<table style="width: 100%; line-height: 15px;">
				<tr class="row" style="margin-right: -5px; text-align: center; text-transform: uppercase; background-color: #f0f0f0; color:#000; border-top-left-radius:5px; border-top-right-radius: 5px; display: flex; font-size: 11px; font-weight: bold; width: 100%;">
					<td class="colheight" style="padding: 0px; border-top-left-radius:5px;width: 3%;">#</td>
		...
		<? } ?>
	<!-- STAFF night Duty START -->
	<!-- Contractor Night Duty Start Here -->
		<?	$sql_night_contract = select_query_json("select * from approval_contractor_night_duty where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_ap[0]['APRNUMB']."%' order by entsrno", 'Centra', 'TEST');
		if(count($sql_night_contract)>0 && $sql_ap[0]['APRNUMB'] != ""){
			echo "<br>"; ?>
			<div class='' id="contractor" name="tablecontractor" style="text-align: left;margin-top: 20px"><h4>CONTRATCTOR DETAILS</h4></div>
			<table style="width: 100%; line-height: 15px;">
				<tr class="row" style="margin-right: -5px; text-align: center; text-transform: uppercase; background-color: #f0f0f0; color:#000; border-top-left-radius:5px; border-top-right-radius: 5px; display: flex; font-size: 11px; font-weight: bold; width: 100%;">
					<td class="colheight" style="padding: 0px; border-top-left-radius:5px;width: 3%;">#</td>
					<td class="colheight" style="padding: 0px;width: 20%;">Company Name</td>
		            <td class="colheight" style="padding: 0px;width: 25%;">Employee Name</td>
		            <td class="colheight" style="padding: 0px;width: 15%;">Photo</td>
		            <!--<td class="colheight" style="padding: 0px;width: 15%;">Department</td>-->
		            <td class="colheight" style="padding: 0px;width: 15%;">Nature Of Work</td>
		            <td class="colheight" style="padding: 0px;width: 10%;">Working Hours</td>
		            <td class="colheight" style="padding: 0px;width: 15%;">Mobile No</td>
		        </tr>
				<? $g=0;
				foreach ($sql_night_contract as $gift1) { $g++;?>
				<tr class="row" style="margin-right: -5px; text-align: center; background-color: transparent; color:#000; display: flex; font-size: 11px; text-transform: uppercase;">
					<td class="colheight" style="padding: 0px 0px; width: 3%;"><?=$g?></td>
					<td class="colheight" style="padding: 0px;width: 20%;"><?=$gift1['CMPNAME']?></td>
					<td class="colheight" style="padding: 0px;width: 25%;"><?=$gift1['EMPNAME']?></td>
					<td class="colheight" style="padding: 0px;width: 15%;">
						<?
							$folder_path1 = "approval_desk/contractor_night_duty/".$_REQUEST['year']."/";?>							
							<img class="img" style="width:50px;height: 40px; " align="center" src="ftp_image_view.php?pic=<?=$gift1['PROLINK']?>&path=<?=$folder_path1?>"  title="<? echo $gift1['EMPNAME']; ?>">
						
					</td>
					
					<td class="colheight" style="padding: 0px;width: 15%;"><?=$gift1['EMPWORK']?></td>
					<td class="colheight" style="padding: 0px;width: 10%;"><?=$gift1['WORKHRS']?></td>
					<td class="colheight" style="padding: 0px;width: 15%;"><?=($gift1['MOBILE'])?$gift1['MOBILE']:"-"?></td>
				</tr>
				<? } ?>
			</table>
		<? } ?>
	
	<!-- Contractor Night Duty End Here -->
		...
		if(count($sql_gift)>0 && $sql_ap[0]['APRNUMB'] != ""){
			...
			<table style="width: 100%; line-height: 15px;">
				...
				<tr class="row" style="margin-right: -5px; text-align: center; background-color: transparent; color:#000; display: flex; font-size: 11px; text-transform: uppercase;">
		            ...
		            <? if($sql_reqid[0]['APRNUMB'] == 'S-TEAM / ATTENDANCE 1000068 / 26-04-2018 / 0068 / 02:23 PM') {
		            	$sql_des = select_query_json("select * from approval_staff_department where apryear='".$_REQUEST['year']."' and aprnumb like '%".$sql_ap[0]['APRNUMB']."%'", 'Centra', 'TCS'); ?>
			...
			<?php
			$sql_descode=select_query_json("Select p.APLCYNM, to_char(pf.EFCTDAT, 'dd-MM-yyyy') EFCTDAT, to_char(pf.VALDUPT, 'dd-MM-yyyy') VALDUPT, to_char(pf.APPRDAT, 'dd-MM-yyyy') APPRDAT,
													pf.APLCYCD, pf.PLCYTYP, pf.CRTECNO, pf.CRTUSNM, pf.CRDECNO, pf.CRDUSNM, pf.ASTECNO, pf.ASTUSNM, pf.USERLST, pf.APRVDBY, pf.APRVDUS,
													pf.DESKPRO, pf.PLCDATA, pf.PLCATTC
			...
			<tr style='height:20px;'><td></td></tr>
				...
				<tr style='min-height:25px; line-height:25px;'>
					...
					echo '<label>'.$file_attach[$filecount]['FILENAM'].'
					<span style="pull-left" ><i class="fa fa-file" aria-hidden="true"></i> <a href = "../approval-desk/ftp_image_view_pdf.php?pic='.$filename.'&path=approval_desk/approval_project_mgt/2018-19/" target="_blank" >View </a></span></label></br>';
					
									}}
									else{ echo '<label class=""> No Attachments
					<span style="pull-left" ><i class="fa fa-file" aria-hidden="true"></i></span></label>'; }
										?>
						...
						<? /* <td rowspan=4 style='width:20%; text-align:center;'>
							<? $sql_verify = select_query_json("select * from APPROVAL_MODE_HIERARCHY where deleted = 'N' and VRFYREQ > 0 and APMCODE = '".$sql_reqid[0]['APMCODE']."' order by APMCODE", 'Centra', 'TCS');
				...
				<? if($sql_reqid[0]['FNTARDT'] != '') { ?>
					<tr style='min-height:20px; line-height:20px;'>
						...
						<td style='width:80%; text-align:left;' colspan="3">
							<label>: <? echo $sql_reqid[0]['FNTARDT']; ?></label> <!-- Work Finish Target Date -->
						</td>
					</tr>
				<? } ?>
				...
				<tr style='min-height:25px; max-width: 773px; width: 100%; line-height:25px;'>
					...
					<td colspan=5 style='width:80%; text-align:left;'>
						: <label style=' font-size:32px; font-weight:bold;' class="blue_highlight">
						
						<? if($sql_approve_leads[0]['APPFVAL'] == 0)
							{
							if($app_val != "")
							{
								echo $tot[$app_val];
				...
				<!-- Expense Percentage -->
				<? // echo "**".$_SESSION['tcs_usrcode']."**".$_SESSION['tcs_empsrno']."**".$_SESSION['tcs_user']."**";
				...
				<td colspan=2 style='width:100%; font-size:10px; text-align:left;'>
					...
					$approveUser = select_query_json("select EMPSRNO  from approval_project_hierarchy where PRMSCOD = '".
								$id."' and deleted = 'N' and APPSTAT = 'Y' order by APPSRNO","Centra","TEST");
								$a = array();
						...
						<? foreach($approveUser as $key => $username){ 
						if($username['EMPSRNO']==125){?>
							<td style="height:50px;vertical-align:bottom;">KOLANCHI NATHAN.M.P
						</td><?}if($username['EMPSRNO']==452){?>
							<td style="height:50px;vertical-align:bottom;">KUMARAN K
						</td><?}elseif($username['EMPSRNO']==21344){?>
							<td style="height:50px;vertical-align:bottom;">S KAARTHI
						</td><?}elseif($username['EMPSRNO']==43400){?>
							<td style="height:50px;vertical-align:bottom;">PADHMA SIVLINGAM S
						</td><?}elseif($username['EMPSRNO']==20118){?>
							<td style="height:50px;vertical-align:bottom;">SIVALINGAM K
						</td><?}?>
						...
						<?$appname = select_query_json("select EMPNAME from employee_office where EMPSRNO = '".$arr[$k]."'","Centra","TCS");?>
						<td style="height:50px;vertical-align:bottom;"><?=$appname[0]['EMPNAME']?></td>
						<?//print_r($arr);?>
						<?}?>
						</tr>
						</table>
							<table border=0 cell-padding=1 cell-spacing=1 style='width:100%; padding-left:2px; padding-right:2px;'>
								<? 
									...
									<?
									//$approveUser= array_reverse($approveUser);
									//foreach($approveUser as $key => $name){
									//	echo "<tr><td><label>".$name['EMPNAME']."</label></td></tr>";
									//}
									for($i=0;$i<count($approveUser);$i++)
									{
										//echo("<tr >");
										for($j=1;$j<=4;$j++)
										{	
											//echo "<td style='padding:20px'><label>".$approveUser[$i]['EMPNAME']."</label></td>";$i++;
										}
									   //echo('</tr>');
									}
								...
								<tr>
									<? 	// print_r($hod_sign); echo "<br>--------";
										...
										// $hod_dsgn = array_values(array_unique($hod_dsgn));
										// print_r($hod_sign); echo "<br>--------";
										...
										for($hodi = 0; $hodi < count($hod_name); $hodi++) {
											...
											<td style='<?=$txtalign?>'>
												<label style='width:14%; font-weight:bold' class="green_highlight"><? if($nxt_name != '') { echo $nxt_name; } ?></label> <!-- Manager -->
											</td>
										<? } */ ?>
								...
								<tr>
									...
									<? } } /* elseif($nxt_name != '') { ?>
										<td style='<?=$txtalign?> width:14%; font-size:11px;'>
											<label style='color:#0088CC; font-weight:bold'><?=$comnts_userdept[0]?></label> <!-- DGM / HOD -->
										</td>
									<? } */ elseif($dept_n != '') { ?>
										<td style='<?=$txtalign?> width:14%; font-size:11px;'>
									...
									<? } /* else { ?>
										<td style='<?=$txtalign?> width:14%; font-size:11px;vertical-align: top;'>
											<label style='color:#0088CC; font-weight:bold'>Sr.GM</label> <!-- Sr.GM -->
										</td>
									<? } */ ?>

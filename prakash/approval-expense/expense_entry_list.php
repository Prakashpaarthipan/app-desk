...
<!-- START SCRIPTS -->
    ...
    <script type="text/javascript">
       ...
       function fix_state(sup) {
            ...
            $.ajax({
                ...
                success: function(data1) {
                  ...
                  var str1 = data[1].trim();
                    //fix_tax(iv, jv);
                   // sup1 = sup.id.toString();
                   // console.log(sup1);
                   // $('#'+sup1).val(str1);
                   // var state= $('#'+sup.id).val();
                   var id = sup.id.split("_");
                   console.log(id[3]);
                   $('#supstate_'+id[3]).val(str1);
        ...
        function fix_tax(tax) {
            $('#load_page').hide();
           var supcode = tax.value;
           var ostate = $("#"+tax).val();
           scode = supcode.split(" - ");
           console.log(scode[0].trim());
            suppcode= scode[0].trim();
            var prd = $("#slt_subproduct").val();
            var strURL="prakash/savesupplierquote.php?action=get_tax&prdcodetax="+prd+"&supcode="+suppcode;
            $.ajax({
                type: "POST",
                url: strURL,
                success: function(data1) {
        ...
        $('#list_content').append("<div id=content"+ntab+" class='row' style='margin-bottom: 10px;''>"+
                                        "<div class='form-group'>"+
                                          ...
                                          "<div class='col-sm-1 col-xs-1' style='width:100px'>"+
                                              "<input type='text' class='form-control required' onkeypress='javascript:return isNumber(event);' id='duration_"+ntab+"' name='duration[]' style='text-transform: uppercase;' maxlength='3' required onblur ='javascript:return valueNotZero(this);return fix_tax(this);' />"+
                                          "</div><!-- duration -->"+

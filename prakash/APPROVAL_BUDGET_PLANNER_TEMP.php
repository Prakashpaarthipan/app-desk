...
extract($_REQUEST);
		...
		$PRJCODE = select_query_json("select * from APPROVAL_BUDGET_PLANNER_TEMP where trunc(ADDDATE) = to_date('01/07/2018','dd/MM/yyyy')","Centra","TCS");
		...
		$g_table = "APPROVAL_BUDGET_PLANNER_TEMP";
...
foreach($PRJCODE  as $val)
{
 ...
 $file['ACCVRFY'] =	$val['ACCVRFY'] ; 
 $file['TMTARNO'] =	$val['TMTARNO']  ; 

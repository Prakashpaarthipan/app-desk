...
{
  $check_ap = select_query_json("select REQSTBY from approval_request where aprnumb='".trim($_REQUEST['aprnumb'])."'", "Centra", 'TEST');
  if($check_ap[0]['REQSTBY'] == $_SESSION['tcs_empsrno']){
    $sql_data = select_query_json("select * from approval_product_quotation_fix where aprnumb='".trim($_REQUEST['aprnumb'])."'", "Centra", 'TEST');
  }
  elseif ( $_SESSION['tcs_usrcode'] == 1103288 or $_SESSION['tcs_usrcode'] == 1444288 or $_SESSION['tcs_usrcode'] == 2280588  ) {
    $sql_data = select_query_json("select * from approval_product_quotation_fix where aprnumb='".trim($_REQUEST['aprnumb'])."'", "Centra", 'TEST');
  }
  else{
    echo 0;
 ...
 and dep.Deleted='N'","Centra","TCS");
		 ...
		 $result = select_query_json("select distinct sup.SUPCODE, sup.SUPNAME, sup.SUPMOBI, cty.ctyname from supplier_asset sup, city cty
            ...
            <td class="center"  style='text-align:left'>
			 ...
			 <td class="center"  style='text-align:left'>
            	 <a target="_blank" href='/approval-desk-test/print_request_quote.php?aprnum=<?=$_REQUEST['aprnumb']?>' title='View' alt='View' style='color:black; font-size:12px'><i class="fa fa-print"></i> Print</a>
            </td>
          </tr>
        <?
      } ?>

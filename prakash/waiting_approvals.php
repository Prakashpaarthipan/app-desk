	...
	case 48237: 	// SELVAGANAPATHI - 20446
		...
		$sql_expkn = select_query_json("select * from APPROVAL_REQUEST
												where ARQCODE = '".$_REQUEST['reqid']."' and ARQYEAR = '".$_REQUEST['year']."' and ATCCODE = '".$_REQUEST['creid']."' and
													ATYCODE = '".$_REQUEST['typeid']."' and deleted = 'N' and REQSTBY in (61579, 83815)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' and $_POST['function'] == 'save_more_suppliers') {
	...
	$exsupplier = select_query_json("Select * From APPROVAL_PRODUCT_QUOTATION
												WHERE PBDYEAR = '".$hid_pbdyear."' and PBDCODE = ".$hid_pbdcode." and PBDLSNO = '".$hid_pbdlsno."'
													and PRLSTYR = '".$hid_pbdyear."' and supcode = '".$hid_supcode."'", "Centra", 'TCS');
	for($qdi = 0; $qdi < count($txt_sltsupcode); $qdi++) { // echo "***";
		...
		$maxprlstsr = select_query_json("Select nvl(Max(PRLSTSR),0)+1 MAXPRLSTSR
												...
												WHERE PBDYEAR = '".$hid_pbdyear."' and PBDCODE = ".$hid_pbdcode." and PBDLSNO = '".$hid_pbdlsno."' and PRLSTYR = '".$hid_pbdyear."'", "Centra", 'TCS');
...
if($_SERVER['REQUEST_METHOD'] == "POST" and $_POST['function'] != 'save_more_suppliers')
{
	...
	$sql_live = select_query_json("select * from APPROVAL_REQUEST
									...
									union
										...
										where ARQCODE = '".$hid_reqid."' and APPSTAT = 'W' and ARQYEAR = '".$hid_year."' and ARQSRNO = '".$hid_rsrid."' and
											ATCCODE = '".$hid_creid."' and ATYCODE = '".$hid_typeid."'", 'Centra', 'TCS');
	...
	if($update_apprq == 1 and $cntlive > 0) {
		...
		for ($aai = 0; $aai < count($hidchk_reject_reason); $aai++) {
			...
			if(count($chk_reject_reason[$hidchk_reject_reason[$aai]]) <= 0) {
				$maxprlstno = select_query_json("Select * From APPROVAL_PRODUCTLIST
														WHERE PBDYEAR = '".$arqyear."' and PBDCODE = '".$slt_aprno."' and PBDLSNO = '".$hidchk_reject_reason[$aai]."'", "Centra", 'TCS');
				...
				$sql_rjvlu = select_query_json("Select * From APPROVAL_product_quotation
														WHERE PBDYEAR = '".$arqyear."' and PBDCODE = ".$slt_aprno." and PBDLSNO != '".$hidchk_reject_reason[$aai]."' and SLTSUPP = 1", "Centra", 'TCS');
		...
		if(($_SESSION['tcs_empsrno'] == 61579 or $_SESSION['tcs_empsrno'] == 83815) and $txt_extarno != $slt_targetno and $txtrequest_value > 0) {
			$sql_extr = select_query_json("select sum(nvl(APPRVAL, 0)) aprvlu from approval_budget_planner_temp
													where BRNCODE=".$slt_branch." and APRYEAR = '".$hid_year."' and EXPSRNO = ".$slt_core_department." and deleted = 'N'", 'Centra', 'TCS');
			if(count($sql_extr) > 0) {
				$sql_yrlyttl = select_query_json("select sum(distinct nvl(sm.BUDVALUE, 0)) BUDVALUE, (sum(distinct nvl(sm.APPVALUE, 0)) + sum(distinct nvl(tm.APPRVAL, 0))) APPVALUE,
														...
														where sm.BUDYEAR=tm.APRYEAR AND sm.BRNCODE=tm.BRNCODE AND sm.EXPSRNO=tm.EXPSRNO and tm.deleted = 'N' and sm.BRNCODE=".$slt_branch." and
															sm.BUDYEAR = '".$hid_year."' and sm.EXPSRNO = ".$slt_core_department."", 'Centra', 'TCS');
			} else {
				$sql_yrlyttl = select_query_json("select sum(nvl(BUDVALUE, 0)) BUDVALUE, sum(nvl(APPVALUE, 0)) APPVALUE, (sum(nvl(BUDVALUE, 0)) - sum(nvl(APPVALUE, 0))) pendingvalue
														from budget_planner_head_sum where BRNCODE=".$slt_branch." and BUDYEAR = '".$hid_year."' and EXPSRNO = ".$slt_core_department."", 'Centra', 'TCS');
			}
		...
		$maxarqcode = select_query_json("Select nvl(Max(ARQCODE),1) maxarqcode, nvl(Max(ARQSRNO),0)+1 maxarqsrno From APPROVAL_REQUEST
												WHERE ATCCODE = ".$slt_topcore." and APMCODE = '".$slt_approval_listings."' and ATMCODE = '".$slt_subtype."' and
													ATYCODE = '".$slt_submission."' and ARQYEAR = '".$hid_year."' and ARQCODE = '".$hid_reqid."'", 'Centra', 'TCS'); // Get the Last record + 1 from APPROVAL_REQUEST
		...
		if($apuser[0] == '' or $apuser[0] == 0)
		{
			if($_SESSION['tcs_user'] == $appdesk or $_SESSION['tcs_user'] == $steam or $_SESSION['tcs_user'] == $legalteam) {
				...
				if($_REQUEST['steam_legal'] == 1)
				{
					...
					$sql_cur_hier = select_query_json("select AMHSRNO from APPROVAL_MODE_HIERARCHY amh, employee_office emp
															...
															order by amh.APMCODE, amh.AMHSRNO", 'Centra', 'TCS');
					$ampsrno = "";
			...
			for($apusri = 0; $apusri < count($apuser)-1; $apusri++)
			{
				...
				$emapr = select_query_json("select APPROVAL_REQUEST.*, to_char(APPRSFR,'dd-MON-yyyy hh:mi:ss AM') APPRSFR_Time, to_char(APPRSTO,'dd-MON-yyyy hh:mi:ss AM') APPRSTO_Time,
												...
												order by ARQCODE", 'Centra', 'TCS'); // reqid=$hid_reqid&year=$hid_year&rsrid=$hid_rsrid&creid=$hid_creid&typeid=$hid_typeid
				...
				if($empg[0]['EMPSRNO'] != $_SESSION['tcs_empsrno'] and count($emapr) <= 1) {
					$sql_ex_intverify2 = select_query_json("select REQSTBY, appfrwd from APPROVAL_REQUEST
																...
																order by ARQSRNO asc", 'Centra', 'TCS');
					if(($hid_int_verification == 'F' or $hid_int_verification == 'S') and count($sql_ex_intverify2) > 0) {
						$usr_apr = select_query_json("select RQTODES from APPROVAL_REQUEST where aprnumb like '".$txt_approval_number."' and arqsrno = 1", 'Centra', 'TCS');
					...
					if($t == 0) {
						...
						if($_SESSION['tcs_descode'] == 132) {
							...
							$sql_apdesk_requiredornot = select_query_json("select VRFYREQ from APPROVAL_MODE_HIERARCHY
																				where APMCODE = '".$slt_approval_listings."' and DELETED = 'N' order by APMCODE, AMHSRNO desc", 'Centra', 'TCS');
		...
		if($_REQUEST['hid_action'] == 'sbmt_query') {
			...
			$sql_raiser = select_query_json("select REQSTBY from APPROVAL_REQUEST
													...
													order by ARQCODE, arqsrno desc", 'Centra', 'TCS');
			...
			if($sql_lastuser[0]['APPHEAD'] == '') {
				$sql_lastuser = select_query_json("select RQTODES from APPROVAL_REQUEST where aprnumb like '".$txt_approval_number."' and ARQSRNO = 1", 'Centra', 'TCS');
			...
			$emp_t = select_query_json("select * from employee_office emp, employee_salary sal where emp.empsrno = sal.empsrno and emp.EMPSRNO = ".$emp[0]['EMPSRNO'], 'Centra', 'TCS');
			$empdes = "designation"; $empsec = "empsection";
			...
			$sql_ex_intverify1 = select_query_json("select ARQSRNO from APPROVAL_REQUEST
														...
														order by ARQSRNO desc", 'Centra', 'TCS');
			...
			if(count($mnt_yr) > 0) {
				...
				if($slt_approval_listings != 807 and $slt_approval_listings != 777) {
					...
					$sql_ext_curr_month = select_query_json("select APRSRNO, APPRVAL from ".$tbl_appplan."
																	where aprnumb like '".$txt_approval_number."' and APRPRID = '".$currnt_mnth."' order by aprsrno", 'Centra', 'TCS');
					$sql_ext_curr_month_all = select_query_json("select * from ".$tbl_appplan."
																	where aprnumb like '".$txt_approval_number."' and aprsrno <= '".$sql_ext_curr_month[0]['APRSRNO']."' order by aprsrno", 'Centra', 'TCS');
				...
				for($cntmntyr = 0; $cntmntyr < count($mnt_yr); $cntmntyr++) {
					...
					if($mnt_yr_amt[$cntmntyr] != $mnt_yr_amt1[$cntmntyr] and $mnt_yr_amt[$cntmntyr] > 0 and $mnt_yr_amt[$cntmntyr] <= $txtrequest_value) {
						if($txt_tmporlive == 0) {
							$tbl_appplan = "approval_budget_planner_temp";
						} else {
							$tbl_appplan = "approval_budget_planner";
	...
	if($hid_samearqsrno == 0) {
		...
		if($end == 1) {
			...
			// Update in APPROVAL_REQUEST Table ARQSRNO = 1 APPFVAL

			if($slt_submission == 1 or $slt_submission == 6 or $slt_submission == 7) {

				/* $sql_double_validate = select_query_json("select SUM(APPRVAL) SUMAPPRVAL, req.APPFVAL, req.aprnumb from APPROVAL_REQUEST REQ, approval_budget_planner PLN 
																	where REQ.APRNUMB = PLN.APRNUMB AND REQ.DELETED = 'N' and arqsrno in (select max(arqsrno) from APPROVAL_REQUEST 
																		where ARQCODE = req.ARQCODE and aprnumb = req.aprnumb and ARQYEAR = req.ARQYEAR and ATYCODE = req.ATYCODE and 
																		ATMCODE = req.ATMCODE and APMCODE = req.APMCODE and ATCCODE = req.ATCCODE) and req.aprnumb = '".$txt_approval_number."' 
																	group by req.APPFVAL, req.aprnumb having SUM(APPRVAL) <> APPFVAL 
																	Order by req.aprnumb, req.APPFVAL", "Centra", "TCS");
				if(count($sql_double_validate) > 0) {
					// echo "$txtrequest_value";
					if($txt_tmporlive == 0) {
						$tbl_appplan = "approval_budget_planner_temp";
					} else {
					...
					$field_appplan = array();
					if($slt_submission == 1) {
						$field_appplan['APPRVAL'] = $sql_double_validate[0]['APPFVAL'];
					} elseif($slt_submission == 6) {
						$field_appplan['APPRVAL'] = $sql_double_validate[0]['APPFVAL'];
						$field_appplan['RESVALU'] = $sql_double_validate[0]['APPFVAL'];
					} elseif($slt_submission == 7) {
						$field_appplan['APPRVAL'] = $sql_double_validate[0]['APPFVAL'];
						$field_appplan['EXTVALU'] = $sql_double_validate[0]['APPFVAL'];
					}
					$where_appplan = " APRNUMB='".$txt_approval_number."' and BRNCODE=".$slt_branch." and APRYEAR = '".$hid_year."' and EXPSRNO = ".$group_wise[0]['EXPSRNO']." and TARNUMB = ".$slt_targetno." and deleted = 'N' and APPMODE = 'N' and APRSRNO = 1";
					$insert_appplan = update_dbquery($field_appplan, $tbl_appplan, $where_appplan);
				} */
		...
		if($_REQUEST['hid_action'] == 'sbmt_mdapprove') { // GM approve this instead of MD
			...
			if($sql_requsr[0]['REQSTFR'] == 61579 or $sql_requsr[0]['REQSTFR'] == 83815 or $_SESSION['tcs_esecode'] == 137) {
		...
		if($_REQUEST['hid_action'] == 'sbmt_reject') {
			...
			$mail_body1 = "<BR>Dear Sir,<BR>Sorry! <b>\"".$txt_approval_number."\"</b> request has been rejected. Reason - ".strtoupper($txt_remarks)."<BR>Kindly verify the request.<BR><BR>Thank you,<BR>Approval Desk Team<BR>".$site_title."";

			$sql_mailsend = store_procedure_query_json("PORTAL_APPROVED_AUTO_MAIL('".$txt_approval_number."', 'R', '".$subject1."', '".$mail_body1."', '".$_SESSION['tcs_usrcode']."')", 'Req', 'TCS'); // Auto Mail send using Store Procedure - 3136 - Saravanakumar
	...
	if(($_SESSION['tcs_empsrno'] == 61579 or $_SESSION['tcs_empsrno'] == 83815) and $txt_extarno == $slt_targetno) { // echo "STEP1";
		...
		$where_appplan = " APRNUMB='".$txt_approval_number."' and DELETED = 'N' ";
		$insert_appplan = update_dbquery($field_appplan, $tbl_appplan, $where_appplan);
	} elseif(($_SESSION['tcs_empsrno'] == 61579 or $_SESSION['tcs_empsrno'] == 83815) and $txt_extarno != $slt_targetno and $tarnochang_allowornot == 1) { // echo "STEP2";
	...
	if($insert_appreq == 1) { // exit();
		if($fnupdt == 1) {
			...
			$sql_brndet = select_query_json("select * from approval_branch_detail where APRNUMB = '".$txt_approval_number."' order by BRDTSRN, BRNCODE", "Centra", "TCS");
			...
			$sql_subjectadd = select_query_json("select * from approval_subject_add where APRNUMB = '".$txt_approval_number."' order by ENTSRNO, BRNHDSR, BRNCODE", "Centra", "TCS");
			...
			if($txtrequest_value > 0 and $slt_budgetmode == 1) { // PO BASED
				$sql_ntpo_generate = store_procedure_query_json("NT_AUTO_PO_CONVERTION('".$hid_year."', '".$slt_aprno."')", 'Req', 'TCS'); // Non Textile PO Generate for PO BASED approvals using Store Procedure - 3136 - Saravanakumar
			} elseif($txtrequest_value > 0 and $slt_budgetmode == 2) { // SELF CHEQUE
				$sql_ntpo_generate = store_procedure_query_json("NT_AUTO_SELF_CHQ_CONVERTION('".$hid_year."', '".$slt_aprno."')", 'Req', 'TCS'); // Non Textile PO Generate for SELF CHEQUE approvals using Store Procedure - 3136 - Saravanakumar
			} elseif($txtrequest_value > 0 and ($slt_budgetmode == 3 or $slt_budgetmode == 4)) { // Salary Advance / Travel Expense
				$sql_ntpo_generate = store_procedure_query_json("NT_AUTO_SLRY_TRAVEL_CONVERTION('".$hid_year."', '".$slt_aprno."')", 'Req', 'TCS'); // Non Textile PO Generate for Salary Advance / Travel Expense approvals using Store Procedure - 3136 - Saravanakumar
			}
			...
			if($_SESSION['tcs_empsrno'] == 21344) {
				$mail_body1 = substr("Dear Sir,<BR>Congrats! <b>\"".$slt_dynamic_subject.$sql_aplist[0]['APMNAME']." - ".$txt_approval_number."\"</b> request has been approved.<BR>Kindly contact to the approval desk team regarding this request.<BR><BR>Approval Link : https://portal.thechennaisilks.com/approval-desk/print_request_view.php?action=print&reqid=".$hid_reqid."&year=".$hid_year."&rsrid=".$rsrid."&creid=".$hid_creid."&typeid=".$hid_typeid."<BR><BR>Thank you,<BR>Approval Desk Team.<BR>".$site_title."", 0, 2000);
			} else {
			...
			$sql_mailsend = store_procedure_query_json("PORTAL_APPROVED_AUTO_MAIL('".$txt_approval_number."', 'A', '".$subject1."', '".$mail_body1."', '".$_SESSION['tcs_usrcode']."')", 'Req', 'TCS'); // Auto Mail send using Store Procedure - 3136 - Saravanakumar
...
$sql_reqid = select_query_json("select req.*, prj.APRNAME, top.ATCNAME, typ.ATYNAME, apm.APMNAME, regexp_replace(SubStr(nicname,1,4),'[0-9]','')||SubStr(nicname,5,10) branch, emp.EMPCODE,
									...
									order by req.ARQCODE, req.ARQSRNO, req.ATYCODE", 'Centra', 'TCS'); // or req.REQSTBY = '".$usrid."'
...
$sql_tmporlive = select_query_json("select ast.EXPNAME exphead from approval_budget_planner but, department_asset ast
											where ast.EXPSRNO = but.EXPSRNO and but.aprnumb like '".$sql_reqid[0]['APRNUMB']."' and but.aprsrno = 1", 'Centra', 'TCS');

$sql_prdlist = select_query_json("select * from APPROVAL_PRODUCTLIST where PBDCODE = '".$sql_reqid[0]['IMUSRIP']."' and PBDYEAR = '".$sql_reqid[0]['ARQYEAR']."' and REJUSER is null", 'Centra', 'TCS');
...
$sql_tarbalance = select_query_json("select R.*, ast.EXPSRNO, ast.EXPNAME EXPHEAD, to_char(R.APPRSFR,'hh:mi:ss AM') APPRSFR_crt_Time, to_char(R.APPRSFR,'dd-MON-yyyy hh:mi:ss AM') APPRSFR_Time,
											...
											order by R.ARQCODE, R.ARQSRNO, R.ATYCODE", 'Centra', 'TCS'); //  and R.APPSTAT = 'A'
...
<body>
	...
	<?php
		$sql_docs = select_query_json("select * from APPROVAL_REQUEST_DOCS where aprnumb = '".$sql_reqid[0]['APRNUMB']."' order by apdcsrn", 'Centra', 'TCS');
		...
		$sql_approve_leads = select_query_json("select * from APPROVAL_REQUEST ar, approval_project pr, approval_process_type pt, APPROVAL_BUDGET_MODE bm, approval_priority ap
													...
													order by ar.ATCCODE, ar.ARQCODE, ar.ARQSRNO desc, ar.ATYCODE", 'Centra', 'TCS');
		$sql_hir = select_query_json("with t as (select apphead from approval_mdhierarchy
											...
											pivot(count(APPHEAD)for(apphead) in (1 as KS, 2 as PS, 3 as AK))", 'Centra', 'TCS'); ?>
	<div class="page">
	...
	<?php } ?>
									...
									if($sql_reqid[0]['APPSTAT'] == 'A' or $sql_reqid[0]['APPSTAT'] == 'R') {
										$sql_vl = select_query_json("select ADDDATE from APPROVAL_REQUEST
					                                                        where aprnumb = '".$sql_reqid[0]['APRNUMB']."' and ARQSRNO = (select max(ARQSRNO)
					                                                            from APPROVAL_REQUEST where aprnumb = '".$sql_reqid[0]['APRNUMB']."')", "Centra", 'TCS');
		...
		<?	$kind_attn = 'Sri. KS Sir';
			...
			$sql_approve_comnts = select_query_json("select req.*, regexp_replace(SubStr(req.REQESEN,1,4),'[0-9]','')||SubStr(req.REQESEN,5,100) REQ_ESEN,
															...
															order by ATCCODE, ARQCODE, ARQSRNO desc, ATYCODE", 'Centra', 'TCS');
			...
			<table border=0 style='width:100%; max-width: 773px; '>
				...
				<tr style='min-height:20px; line-height:20px;'>
					...
					<td style='width:80%; text-align:left;'>
						<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? echo $sql_reqid[0]['APMASTER']; /* if(trim($sql_reqid[0]['DYNSUBJ']) != '-') { echo rtrim($sql_reqid[0]['DYNSUBJ'], " - "); } if(trim($sql_reqid[0]['TXTSUBJ']) != '-') { echo $sql_reqid[0]['TXTSUBJ']; } */ ?></span></label> <!-- Approval Listings -->
					</td>
				</tr>
				...
				<? if($sql_reqid[0]['DYSBFDT'] != '') { ?>
					<tr style='min-height:20px; line-height:20px;'>
						...
						<td style='width:80%; text-align:left;'>
							<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? if($sql_reqid[0]['DYSBFDT'] != '-') { echo strtoupper(date("d-M-Y", strtotime($sql_reqid[0]['DYSBFDT'])) ." TO ". date("d-M-Y", strtotime($sql_reqid[0]['DYSBTDT']))); } ?></span></label> <!-- AMC Period -->
						</td>
					</tr>
				<? } elseif(rtrim($sql_reqid[0]['DYNSUBJ'], " - ") != '') { ?>
					<tr style='min-height:20px; line-height:20px;'>
						...
						<td style='width:80%; text-align:left;'>
							<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><? if(trim($sql_reqid[0]['DYNSUBJ']) != '-') { echo rtrim($sql_reqid[0]['DYNSUBJ'], " - "); } ?></span></label> <!-- Month Period -->
						</td>
					</tr>
				<? } ?>
								...
								<label>: <span style="font-size: 14px;<? if($aptype_display == "NEW PROPOSAL" or $aptype_display == "EXTRA BUDGET"){?> font-weight:bolder;<?}?>" <? if($aptype_display == "NEW PROPOSAL" or $aptype_display == "EXTRA BUDGET"){?> class="red_highlight" <?}?> ><? echo $aptype_display; ?></span> / <span style="font-size: 12px; font-weight: bold; " class="blue_highlight"><? echo $sql_approve_leads[0]['BUDNAME']; if($sql_reqid[0]['APRQVAL'] > 0) { ?></span> <span style="font-size: 10px;">[ Target NO - <b><? echo $sql_tarbalance[0]['TARNUMB']." - ".$sql_tarbalance[0]['TARDESC']; ?></b> ]</span><? } if($sql_reqid[0]['ATYCODE'] == 7) { ?><br><b>Reserved Budget Balance against Expense Head [ <span class="clrblue"><?=$sql_tarbalance[0]['EXPHEAD']?></span> ]</b> &#8377;
									<? 	$target_balance = select_query_json("select sum(distinct nvl(sm.BUDVALUE, 0)) BUDVALUE, (sum(distinct nvl(sm.APPVALUE, 0)) +
																					...
																					where sm.BUDYEAR=tm.APRYEAR AND sm.BRNCODE=tm.BRNCODE AND sm.EXPSRNO=tm.EXPSRNO and tm.deleted = 'N'
																						...
																						sm.EXPSRNO = ".$sql_reqid[0]['EXPSRNO']."", 'Centra', 'TCS');
				...
				<? /* if($sql_reqid[0]['APMCODE'] == 802) {
					$sql_proj = select_query_json("select * from approval_project where deleted = 'W' order by APRCODE desc", 'Centra', 'TCS'); ?>
					<tr style='min-height:20px; line-height:20px;'>
						<td style='width:20%; text-align:left;'>
					...
					<td colspan=2>
						<table border=0 width='100%' style='max-width: 773px; border: 1px solid #0088CC; min-height: 70px; height:auto; padding:3px; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px;'>
							...
							<tr>
								<td colspan="2">
									<table style="width:100%">
										<? $sql_descode=select_query_json("select distinct ATYCODE , TARNUMB , APMNAME , TOPCORE , SUBCORE , ENTSRNO from approval_subject_add where APRNUMB = '".$sql_reqid[0]['APRNUMB']."' order by ENTSRNO", "Centra", "TCS");
								...
								<?
										...
										if($tmporlive == 0) {
											if($_SESSION['tcs_user'] == 17108) {
												$sql_plan = select_query_json("select * from approval_budget_planner_temp
																					where aprnumb = '".$sql_reqid[0]['APRNUMB']."' order by APRSRNO", 'Centra', 'TCS');
											} else {
												$sql_plan = select_query_json("select * from approval_budget_planner_temp
																					where deleted = 'N' and aprnumb = '".$sql_reqid[0]['APRNUMB']."' order by APRSRNO", 'Centra', 'TCS');
											}
										} else {
										...
										foreach($sql_prdlist as $prdlist) { $inc++; $prdcnt++;
											$sql_prdquotlist = select_query_json("select qut.SUPCODE, qut.SUPNAME, cty.CTYNAME, sup.SUPMOBI, qut.DELPRID, qut.SUPRMRK, sup.SUPMODE, GTE.GPSNAME 
									    													...
									    													where qut.supcode = sup.supcode and cty.ctycode = sup.ctycode and GTE.GPSCODE = sup.SUPMODE and qut.SLTSUPP = 1 
									    														...
									    														qut.PBDLSNO = '".$prdlist['PBDLSNO']."'", 'Centra', 'TCS'); 
									    														/* union
									...
									<? 	$sql_prdquotlist1 = select_query_json("select * from APPROVAL_PRODUCT_QUOTATION 
								    													where PBDCODE = '".$prdlist['PBDCODE']."' and SLTSUPP = 1 and 
								    														PBDYEAR = '".$prdlist['PBDYEAR']."' and PBDLSNO = '".$prdlist['PBDLSNO']."'", 'Centra', 'TCS'); ?>
											<tr class="row" style="margin-right: -5px; text-align: center; background-color: transparent; color:#000; display: flex; font-size: 11px; text-transform: uppercase;">
									            ...
									            <td class="colheight" style="padding: 1px 0px; width: 17%;">
									    			...
									    			$sql_percentage = select_query_json("select nvl(CGSTPER, 0) CGSTPER, nvl(SGSTPER, 0) SGSTPER, nvl(IGSTPER, 0) IGSTPER 
									    														...
									    														where prdcode = '".$prdlist['PRDCODE']."' and subcode = '".$prdlist['SUBCODE']."' and rownum <= 1", "Centra", "TCS");
									            ...
									            <td class="colheight" style="padding: 1px 0px; width: 12%; margin-right: 0.6%;"><?=moneyFormatIndia(round($sql_prdquotlist1[0]['NETAMNT']))?><br>
									            	<? if($sql_requsr[0]['REQSTFR'] == 61579 or $sql_requsr[0]['REQSTFR'] == 83815) { /* ?>
	...
	$sql_ap = select_query_json("select distinct aprnumb from approval_request
											where ARQCODE = '".$_REQUEST['reqid']."' and ARQYEAR = '".$_REQUEST['year']."' and ATCCODE = '".$_REQUEST['creid']."'
												and ATYCODE = '".$_REQUEST['typeid']."' and deleted='N'", 'Centra', 'TCS');

	$sql_gen_det = select_query_json("select * from approval_general_detail where aprnumb='".$sql_ap[0]['APRNUMB']."' order by rowsrno,colsrno", 'Centra', 'TCS');
	$sql_gen_master = select_query_json("select * from approval_general_master where tempid=".$sql_gen_det[0]['TEMPID']." order by colsrno", 'Centra', 'TCS');
	$app_val = "";
									...
									if($col['APMCODE'] == 856 and $col['COLSRNO'] == 2) {
										...
										for($ij = 0; $ij < count($expl); $ij++) {
											$sql_tablemast = select_query_json("select * from master_table_detail
																						where deleted = 'N' and MASTERID = '".$expl[$ij]."' order by TABNAME asc", 'Centra', 'TCS');
	...
	<?	$sql_night = select_query_json("select * from approval_night_duty where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_ap[0]['APRNUMB']."%' order by entsrno", 'Centra', 'TCS');
	...
	<?	$sql_project_branch = select_query_json("SELECT * from approval_branch_detail bd, approval_branch_list bl
														where bd.BRNCODE = bl.BRNCODE and bd.APRNUMB = '".$sql_reqid[0]['APRNUMB']."'", "Centra", "TCS");
	...
	<?
		$sql_gift = select_query_json("select * from approval_staff_marriage where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_ap[0]['APRNUMB']."%'", 'Centra', 'TCS');
		if(count($sql_gift)>0 && $sql_ap[0]['APRNUMB'] != ""){
			...
			$sql_branch = select_query_json("select * from approval_staff_branch_change where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_ap[0]['APRNUMB']."%'", 'Centra', 'TCS');
			...
			$sql_desg = select_query_json("select * from approval_staff_designation where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_ap[0]['APRNUMB']."%'", 'Centra', 'TCS');
			...
			// STAFF DEPT CHANGE
			$sql_dept = select_query_json("select * from approval_staff_department where apryear='".$_REQUEST['year']."' and aprnumb like  '%".$sql_ap[0]['APRNUMB']."%'", 'Centra', 'TCS');
			...
			$sql_salary = select_query_json("select app.* from approval_staff_salary_change app, employee_office emp
													where emp.empsrno= app.empsrno and app.apryear='".$_REQUEST['year']."' and app.aprnumb like  '%".$sql_ap[0]['APRNUMB']."%'", 'Centra', 'TCS');
			...
			where pf.APLCYCD = p.APLCYCD and pf.APRNUMB = '".$sql_reqid[0]['APRNUMB']."'","Centra","TCS");
				foreach($sql_descode as $sectionrow) {
			?>
			<tr>
				<td colspan="3">
				...
				if($sql_reqid[0]['REQSTBY'] == $_SESSION['tcs_empsrno']) {
					$sql_inteam = select_query_json("select distinct trim(emp.EMPNAME) EMPNAME, emp.EMPCODE, substr(brn.NICNAME,3,10) brnname, des.DESNAME, SUBSTR(sec.esename, 4, 100) esename,
														...
														union
															...
															order by ordersrno, EMPNAME Asc", 'Centra', 'TCS');
				} else {
					...
					<table border=1 style='width:100%; max-width: 773px; min-height:100px; height:auto; border:1px solid #0088CC; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px; padding:5px 0px; margin-top: 5px;'>
						...
						<? $sql_othcmd = select_query_json("select emp.empcode,emp.empname,emp.esecode
																	...
																	where hir.apphead=emp.empcode and aprnumb like '".$sql_reqid[0]['APRNUMB']."'and apphead not in (select empcode
																		...
																		and arqsrno not in 1 and appfrwd='F')) order by amhsrno asc", 'Centra', 'TCS');
						// print_r($sql_othcmd);
						...
						for($usri = 0; $usri < count($comnts_user) && $comnts_user[$usri] != ''; $usri++) { ?>
							...
							<? 	if($usri == (count($comnts_user) - 1) and count($ks_cmnts) <= 0) {
									$sql_md = select_query_json("select INTSUGG, INTPESN, RQBYDES, ADDDATE, REQSTBY from APPROVAL_REQUEST req
																	...
																	order by req.ARQSRNO desc", 'Centra', 'TCS');
					...
					<? 	$sql_desname = select_query_json("select DESNAME from designation 
						...
						<tr><td colspan=4>
							...
							<? } ?>
								<tr>
							<?
								$sql_apphir = select_query_json("select * from approval_mdhierarchy where aprnumb in ('".$sql_reqid[0]['APRNUMB']."' ) and apphead=1 order by amhsrno ", 'Centra', 'TCS');
								$sql_appak = select_query_json("select * from approval_mdhierarchy where aprnumb in ('".$sql_reqid[0]['APRNUMB']."' ) and apphead=3 order by amhsrno ", 'Centra', 'TCS');
								$sql_appps = select_query_json("select * from approval_mdhierarchy where aprnumb in ('".$sql_reqid[0]['APRNUMB']."' ) and apphead=2 order by amhsrno ", 'Centra', 'TCS');
								$sql_all_md = select_query_json("select count(*) VAL from approval_mdhierarchy where  aprnumb in ('".$sql_reqid[0]['APRNUMB']."' ) and  apphead in (1,2)", 'Centra', 'TCS');
		...
		<? if(count($sql_prdlist) > 0 and $edtvl == 0) {
			$sql_prdlist1 = select_query_json("select * from APPROVAL_PRODUCTLIST PRD, APPROVAL_PRODUCT_QUOTATION QUT
			    												...
			    												order by PRD.PBDYEAR, PRD.PBDCODE, PRD.PBDLSNO", 'Centra', 'TCS');
		...
		$sql_quthist = select_query_json("select his.PBDYEAR, his.PBDCODE, his.PBDLSNO, his.PRLSTSR, his.SUPCODE, his.SUPNAME, his.SLTSUPP, prd.PRDCODE, prd.PRDNAME, prd.SUBCODE, prd.SUBNAME, 
		                                        ...
		                                        order by prd.PRDCODE, prd.SUBCODE, his.PBDCODE, his.PBDYEAR, his.PBDLSNO, his.SLTSUPP desc", "Centra", "TCS");
		if(count($sql_quthist) > 0) { ?>
		...
		<tr>
			<td colspan="2" style="width: 100%; vertical-align: top;">
				<textarea class="form-control" tabindex="26" name='txt_remarks' id='txt_remarks' maxlength="200" onKeyPress="return isQuotes(event)" style='width:98.5%; text-transform:uppercase; height:75px; padding: 5px;' required><? if($_SESSION['tcs_empsrno'] == '125' and $sql_approve_leads[0]['APPFVAL'] > 0) { /* ?>BUDGET HEADS ARE OK<? */ ?>APPROVED<? } else { ?>APPROVED<? } ?></textarea>
			</td>
		</tr>
		...
		<tr>
			<td style="width: 49%; vertical-align: top;">
				<input class="form-control" placeholder="Supporting Documents" tabindex='10' maxlength='150' type='file' name='txt_submission_fieldimpl[]' id='txt_submission_fieldimpl' multiple accept="image/jpg,image/jpeg,image/gif,image/png,image/jpg,.pdf" value='' data-toggle="tooltip" data-placement="top" title="Supporting Documents"><br><span style='color:#FF0000; font-size:10px;'>NOTE : ALLOWED ONLY PDF / IMAGES</span>
			</td>
		...
		<? // Assign Finish User & Finish Option - 18-09-2017
			...
			$sql_dptinchr1 = select_query_json("select distinct EMPSRNO, EMPCODE APPHEAD, EMPNAME, 'DEPARTMENT INCHARGE' APPTITL, 1 allow_days, min(aprvalu) APRVALU
														...
														ORDER BY EMPSRNO, EMPCODE, EMPNAME", "Centra", 'TCS');
				...
				: <? // if(($_REQUEST['action'] == 'edit' or $_REQUEST['action'] == 'view') and ($ccfinish_here == 0)) {
					if($_REQUEST['action'] == 'edit' or $_REQUEST['action'] == 'view') {
						...
						$sql_app_hierarchy1 = select_query_json("select * from APPROVAL_MDHIERARCHY amh, employee_office emp
																		...
																		order by amh.APMCODE, amh.AMHSRNO desc", 'Centra', 'TCS');
						if(count($sql_app_hierarchy1) > 0) { $flo = 1; $newentry = 1;
							...
							if($sql_reqid[0]['APPSTAT'] == 'W') {
								$sql_rsruser = select_query_json("select * from APPROVAL_request where aprnumb = '".$sql_reqid[0]['APRNUMB']."' and arqsrno = '".$rsrid."' order by arqsrno desc", 'Centra', 'TCS');
			...
			<div class="col-lg-12 col-md-12" style=' text-align:center; padding-right:0px;'>
				...
				<?  if(count($sql_reqid) > 0) {
					$sql_search = select_query_json("select ar.APRNUMB, ar.APPSTAT, ar.APPRSUB, ar.APRQVAL, ar.APRTITL, ar.ARQSRNO, ar.ARQCODE, ar.ATYCODE, ar.ATCCODE, ar.ARQYEAR,
														...
														order by APPORDER Asc, ar.APRQVAL Desc", 'Centra', 'TCS');
				...
				$sql_iv = select_query_json("select nvl(appfrwd, '') appfrwdrow from APPROVAL_REQUEST R
												where aprnumb like '".$sql_reqid[0]['APRNUMB']."' and arqsrno in (SELECT max(ARQSRNO) FROM APPROVAL_REQUEST where appfrwd='I' and
													...
													ATMCODE = R.ATMCODE and APMCODE = R.APMCODE and deleted = 'N')", 'Centra', 'TCS');
				...
				<? if($_REQUEST['action'] == 'view') {
					...
					if(($sql_requsr[0]['REQSTFR'] == 61579 or $sql_requsr[0]['REQSTFR'] == 83815) && $sql_reqid[0]['APPSTAT'] =='N' )
					{
						if($prdcnt>0 && $sql_reqid[0]['PURHEAD'] == "") { ?>
						 ...
						 if($final_approval == 1 and $mdfin == 0 and $sql_reqid[0]['APPFRWD'] != 'I') { // Final Approval.. ?>
							<button type="submit" name='sbmt_approve' id='sbmt_approve' tabindex='28' value='Final Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Final Approve"><i class="fa fa-check-square-o"></i> Final Approve</button>&nbsp;
						...
						<? }
						 ...
						 if($final_approval == 0) { $isstage = 1; // Not Final Approval..?>
							<button type="submit" style="float: right;" name='sbmt_forward' id='sbmt_forward' tabindex='28' value='Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve"><i class="fa fa-fast-forward"></i> Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve</button>&nbsp;
						<? } // Not Final Approval..
					} elseif(($sql_requsr[0]['REQSTFR'] == 61579 or $sql_requsr[0]['REQSTFR'] == 83815) && $sql_reqid[0]['APPSTAT'] =='Z' ) {
						if($prdcnt>0) { ?>
						...
						<? }
					} elseif($sql_requsr[0]['REQSTFR'] != 61579 and $sql_requsr[0]['REQSTFR'] != 83815) {
						if($final_approval == 1 and $mdfin == 0 and $sql_reqid[0]['APPFRWD'] != 'I') { $isstage = 1; // Final Approval.. ?>
							<button type="submit" name='sbmt_approve' id='sbmt_approve' tabindex='28' value='Final Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Final Approve"><i class="fa fa-check-square-o"></i> Final Approve</button>&nbsp;
							<button type="submit" style="display: none; float: right;" name='sbmt_forward' id='sbmt_forward' tabindex='28' value='Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve"><i class="fa fa-fast-forward"></i> Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve</button>&nbsp;
						<? }
...
<table border=0 cell-padding=1 cell-spacing=1 align='center' class="fixed" style='border:5px solid #303030; border-style:double; width:796.8px; max-height:1123.2px; padding:7px'>
	...
	<tr>
		...
		<? 	for($ij = 0; $ij < count($pagearry['doc']); $ij++){
				...
				echo $fieldindi = "<a href=\"ftp_image_view_pdf.php?pic=".$filename."&path=approval_desk/request_entry/".$dataurl."/\" target=\"_blank\" class=\"style_box\">".$filename."</a><br><br>";
...
<?
	...
	$sql_list = select_query_json("select * from approval_request where arqsrno = 1 and aprnumb like '".$sql_reqid[0]['APRNUMB']."'", 'Centra', 'TCS');

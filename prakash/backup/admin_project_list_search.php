				...
				<div class="tab-content">
				  ...
				  <div class="page-content-wrap">
					<div class="panel panel-default">
						...
						<form role="form" id='frm_project_search' name='frm_project_search' action='' method='post' enctype="multipart/form-data">
                            <div class="col-lg-2 col-sm-2" style='text-align:center; padding:5px 5px 0 5px;'>
								<input type='text' class="form-control" tabindex='1' name='search_pjname' id='search_pjname' value='<?=$_REQUEST['search_pjname']?>' maxlength="100" data-toggle="tooltip" data-placement="top" placeholder='PROJECT NAME' title="PROJECT NAME" style='text-transform: uppercase;'>
							</div>

							<div class="col-lg-2 col-sm-2" style='text-align:center; padding:5px;'>
								<input type='text' class="form-control" tabindex='2' name='project_id' id='project_id' value='<?=$_REQUEST['project_id']?>' data-toggle="tooltip" data-placement="top" maxlength="20" placeholder='PROJECT ID' title="PROJECT ID" style='text-transform: uppercase;'>
							</div>
							<div class="col-lg-2 col-sm-2" style='text-align:center; padding:5px;'>
								 <select class="form-control" tabindex='4' name='branch_type' id='branch_type' data-toggle="tooltip" data-placement="top" title="BRANCH TYPE" >
                                     <option value='' selected>ALL TYPE</option>
                                     <option value="B" <? if($_REQUEST['branch_type'] == 'B') { ?> selected <? } ?>>BRANCH</option>
                                     <option value="P" <? if($_REQUEST['branch_type'] == 'P') { ?> selected <? } ?>>NEW PROJECT</option>
                                    
                                </select>
							</div>
							<div class="col-lg-2 col-sm-2" style='text-align:center; padding:5px;display:none'>
							...
							<div class="col-lg-1 col-sm-1" style='text-align:center; padding:5px;display:none'>
								<select class="form-control" tabindex='4' name='request_type' id='request_type' data-toggle="tooltip" data-placement="top" title="REQUEST TYPE" >
                                    ...
                                    <? $complaintMaster = select_query_json("Select comcode, comname from APP_COMPLAINT_MASTER where deleted='N' order by COMCODE ASC", "Centra", "TCS");
                                        foreach($complaintMaster as $res) { ?>
                                            <option value="<?=$res['COMCODE']?>" <? if($res['COMCODE'] == $request_type) { ?> selected <? } ?>> <?=$res['COMNAME']?> </option>
                                    <? } ?>
								</select>
							</div>
                            <div class="col-lg-1 col-sm-1" style='text-align:center; padding:5px;display:none'>
                                <select class="form-control" tabindex='4' name='status_type' id='status_type' data-toggle="tooltip" data-placement="top" title="STATUS TYPE" >
                                     ...
                                     <option value="N" <? if($_REQUEST['status_type'] == 'N') { ?> selected <? } ?>>NOT ASSIGNED</option>
							...
							<div class="col-lg-2 col-sm-2" style='padding:5px;'>
								<input type='submit' name='search_frm' id='search_frm' tabindex='7' class='btn btn-primary' value='SEARCH' title='SEARCH' >
					...
					<form role="form" id='frm_project_list_edit' name='frm_project_list_edit' method='post' enctype="multipart/form-data">
						...
						<thead >
							...
							<tbody>
								...
								$and=" ";
								if($search_pjname != '') {
								$and .= " And (ap.PRJNAME) like '%".strtoupper($search_pjname)."%' ";
								}
							
								if($project_id != '') {
								$and .= " And (ap.PRMSCOD) like '%".strtoupper($project_id)."%' ";
								}
								if($branch_type != '') {
								$and .= " And ap.BRN_PRJ = '".strtoupper($branch_type)."' ";
								}
								...
								$and .= " And trunc(ap.ADDDATE) BETWEEN TO_DATE('".$frm_date."', 'DD-MON-YY') AND TO_DATE('".$to_date."', 'DD-MON-YY') ";
								}
							
								$project_search =  select_query_json("select to_char(ap.ADDDATE,'dd-MM-yyyy HH:mi:ss AM') ADDDATE,ap.* from approval_project_master ap where  ap.DELETED = 'N' and ap.BRN_PRJ IN ('B','P') ".$and." ORDER BY ap.PRMSCOD,ap.ADDDATE ASC ","Centra","TEST");
								//echo "select to_char(ap.ADDDATE,'dd-MM-yyyy HH:mi:ss AM') ADDDATE,ap.* from approval_project_master ap where  ap.DELETED = 'N' and ap.BRN_PRJ IN ('B','P') ".$and." ORDER BY ap.PRMSCOD,ap.ADDDATE ASC ";
								for($project_i = 0; $project_i < count($project_search); $project_i++) {
										
									?>
									<tr class="center">
									<td class="center" style="text-align:center"><? echo ($project_i+1);?></td>
									<td class="center" style="text-align:center"><? echo $project_search[$project_i]['PRMSCOD'];?></td>
									<td class="center" style="text-align:center"><? echo $project_search[$project_i]['PRJNAME'];?></td>
									<td class="center" style="text-align:center"><? echo $project_search[$project_i]['BRNNAME'];?></td>
									<td class="center" style="text-align:center"><? if ($project_search[$project_i]['BRN_PRJ'] == 'P') {
								  echo "NEW PROJECT";
								}else {  echo "BRANCH"; }?>
								</td>
								<td class="center" style="text-align:center"><? echo $project_search[$project_i]['ADDDATE'];?></td>
								<?
								$view = select_query_json("select * from approval_project_heirarchy where prmscod = '".$project_search[$project_i]['PRMSCOD']."' and Appstat ='Y'","Centra","TEST");
								//var_dump($view);
								//if(count($view) == 1){
								if($view[0]['APPSTAT'] == 'Y'){ ?> 
								<td class="center" style="text-align:center">
								<button onclick=" javascript:var pjid = <? echo $project_search[$project_i]['PRMSCOD'];?>; 
								var pjyr = '<? echo $project_search[$project_i]['PRMSYER'];?>';getProjectDetails(pjid,pjyr)" data-toggle="modal" type = "button" data-target="#modal_large" data-id="<? echo $project_search[$project_i]['PRMSCOD']; ?>" id="getProject" class="btn btn-sm btn-info" style="background-color: #397ce0;!important; border:none;"><i class="glyphicon glyphicon-retweet"></i> Processing</button>
								
							 </td> 
														 <?} else {?>
							 <td class="center" style="text-align:center">
							<a class='btn btn-info btn-sm'
						 href="/approval-desk-test/admin_project_modify_prakash.php?c_year=<?echo $project_search[$project_i]['PRMSYER'];?>&id=<?echo $project_search[$project_i]['PRMSCOD'];?>">
							...
							<?} ?>
							 ...
							 $sql_project1 = select_query_json("select distinct pm.PRMSYER, pm.PRMSCOD, pm.PRJNAME, pm.BRNNAME, ph.TOPCORE, ph.SUBCRNM, ph.TARNUMB, ph.TARNAME, pm.BRN_PRJ, ph.PRMSYER, ph.PRMSCOD,ah.EMPSRNO from approval_project_master pm ,  approval_project_head ph ,approval_project_heirarchy ah where pm.PRMSYER = ph.PRMSYER and pm.PRMSCOD = ph.PRMSCOD and PRJTITL = '4' and ph.DELETED = 'N' and ph.PRMSCOD = ah.PRMSCOD and ah.EMPSRNO = '188' and ah.APPSTAT = 'N' ORDER BY pm.PRMSCOD","Centra","TEST");*/
							 $sql_project1 = select_query_json("select to_char(ap.ADDDATE,'dd-MM-yyyy HH:mi:ss AM') ADDDATE,ap.* from approval_project_master ap  where ap.DELETED = 'N' and ap.PRJSTAT = 'N' and ap.CAPPUSR = '0' ORDER BY ap.PRMSCOD","Centra","TEST");
								...
								<? if ($_SESSION['tcs_empsrno'] == 83815){
									$sql_project1 = select_query_json("select to_char(ap.ADDDATE,'dd-MM-yyyy HH:mi:ss AM') ADDDATE,ap.* from approval_project_master ap where ap.CAPPUSR = 188 and ap.DELETED = 'N' and ap.PRJSTAT = 'N' ORDER BY ap.PRMSCOD","Centra","TEST");
									/* $sql_project1 = select_query_json("select * from approval_project_master pm, approval_project_heirarchy ah where pm.PRMSCOD = ah.PRMSCOD and pm.DELETED = 'N' and ((ah.EMPSRNO = '188' and ah.APPSTAT = 'Y') and (ah.EMPSRNO = '61579' and ah.APPSTAT = 'N')) ORDER BY pm.PRMSCOD","Centra","TEST");*/
							...
							<? if ($_SESSION['tcs_empsrno'] == 452){
									$sql_project1 = select_query_json("select to_char(ap.ADDDATE,'dd-MM-yyyy HH:mi:ss AM') ADDDATE,ap.* from approval_project_master ap where ap.CAPPUSR = 83815 and ap.DELETED = 'N' and ap.PRJSTAT = 'N' ORDER BY ap.PRMSCOD","Centra","TEST");
									for($project_j = 0; $project_j < count($sql_project1); $project_j++) {									
									?>
									<tr>
								<td class="center" style="text-align:center"><?echo ($project_j+1);?></td>
								...
								<? if ($_SESSION['tcs_empsrno'] == 21344){
									$sql_project1 = select_query_json("select to_char(ap.ADDDATE,'dd-MM-yyyy HH:mi:ss AM') ADDDATE,ap.* from approval_project_master ap where ap.CAPPUSR = 452 and ap.DELETED = 'N' and ap.PRJSTAT = 'N' ORDER BY ap.PRMSCOD","Centra","TEST");
									for($project_j = 0; $project_j < count($sql_project1); $project_j++) {									
									?>
									<tr>
								<td class="center" style="text-align:center"><?echo ($project_j+1);?></td>
					...
					<div class="non-printable" style='clear:both; border-bottom:1px solid #000; margin-bottom:10px; margin-top: 10px;'></div>
						<table id="tbl_approved_list" class="table datatable" style="font-size: 12px;"  >
							...
							<tbody>
								...
								{ //condition to check first stage of approval process
							$sql_project = select_query_json("select to_char(ah.APPDATE,'dd-MM-yyyy HH:mi:ss AM') APPDATE,am.* from approval_project_master am ,approval_project_heirarchy ah  where ah.prmscod = am.prmscod and ah.EMPSRNO = 188 and ah.APPSTAT = 'Y' ORDER BY ah.PRMSCOD","Centra","TEST");
								...
								if ($_SESSION['tcs_empsrno'] == 83815)
								{ //condition to check second stage of approval process
							$sql_project = select_query_json("select to_char(ah.APPDATE,'dd-MM-yyyy HH:mi:ss AM') APPDATE,am.*  from approval_project_master am ,approval_project_heirarchy ah  where ah.prmscod = am.prmscod and ah.EMPSRNO = 83815 and ah.APPSTAT = 'Y' ORDER BY ah.PRMSCOD","Centra","TEST");
								...
								if ($_SESSION['tcs_empsrno'] == 452)
								{ //condition to check third stage of approval process
							$sql_project = select_query_json("select to_char(ah.APPDATE,'dd-MM-yyyy HH:mi:ss AM') APPDATE,am.*  from approval_project_master am ,approval_project_heirarchy ah  where ah.prmscod = am.prmscod and ah.EMPSRNO = 452 and ah.APPSTAT = 'Y' ORDER BY ah.PRMSCOD","Centra","TEST");
								...
								if ($_SESSION['tcs_empsrno'] == 21344)
								{ //condition to check final stage of approval process
							$sql_project = select_query_json("select to_char(ah.APPDATE,'dd-MM-yyyy HH:mi:ss AM') APPDATE,am.*  from approval_project_master am ,approval_project_heirarchy ah  where ah.prmscod = am.prmscod and ah.EMPSRNO = 21344 and ah.APPSTAT = 'Y' ORDER BY ah.PRMSCOD","Centra","TEST");

			...
			<div class="nav-tabs-custom ">
				...
				<div class="tab-content">
				  ...
				  <div class="page-content-wrap">
					...
					<form role="form" id='frm_project_list_edit' name='frm_project_list_edit' action='' method='post' enctype="multipart/form-data">
                        <div class="non-printable" style='clear:both; border-bottom:1px solid #000; margin-bottom:10px; margin-top: 10px;'></div>
						
						<table id="tbl_project_list" class="table datatable" style="font-size: 10px;"  >
						<thead >
							<tr >
							  <th >S No</th>
								<th >Project ID</th>
 ...
 approval_project_head ph ,approval_project_heirarchy ah where pm.PRMSYER = ph.PRMSYER and pm.PRMSCOD = ph.PRMSCOD and PRJTITL = '4'
  and ph.DELETED = 'N' and ph.PRMSCOD = ah.PRMSCOD and ah.EMPSRNO = '188' and ah.APPSTAT = 'N' ORDER BY pm.PRMSCOD","Centra","TEST");
								...
								<td><?if ($sql_project[$project_i]['BRN_PRJ'] == 'P') {
								 ...
								 <td><?echo $sql_project[$project_i]['BRNNAME']." / ".$sql_project[$project_i]['TOPCORE']." / ".$sql_project[$project_i]['SUBCRNM']." / ".$sql_project[$project_i]['TARNUMB']." - ".$sql_project[$project_i]['TARNAME']." / ".$sql_project[$project_i]['PRJNAME'];?></td>
			...
			<div class="tab-pane" id="tab_2">
			 <div style ="padding-top:10px; background: #3f444c;"></div>
			   <div class="page-content-wrap">
					...
					<div class="non-printable" style='clear:both; border-bottom:1px solid #000; margin-bottom:10px; margin-top: 10px;'></div>
						<table id="tbl_approved_list" class="table datatable" style="font-size: 10px;"  >
							<thead>
								<tr>
								  <th>S No</th>
								  <th>Project ID</th>
							...
							<tbody>
								...
								if ($_SESSION['tcs_empsrno'] == 188)
								{ //condition to check first stage of approval process
							$sql_project = select_query_json("select * from approval_project_master pm ,approval_project_heirarchy ah where pm.PRMSCOD =ah.PRMSCOD and ah.EMPSRNO = 188 and ah.APPSTAT = 'Y' ORDER BY pm.PRMSCOD","Centra","TEST");
								...
								if ($_SESSION['tcs_empsrno'] == 61579)
								{ //condition to check first stage of approval process
							$sql_project = select_query_json("select * from approval_project_master pm ,approval_project_heirarchy ah where pm.PRMSCOD =ah.PRMSCOD and ah.EMPSRNO = 61579 and ah.APPSTAT = 'Y' ORDER BY pm.PRMSCOD","Centra","TEST");

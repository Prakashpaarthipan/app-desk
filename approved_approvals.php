                    ...
                    <div class="panel-body">
                        ...
                        <table id="customers2" class="table datatable">
                            ...
                            <?  $and = "";
                                    ...
                                    // echo $appfrwd;
                                    if($and != '') {
                                        $sql_search = select_query_json("select distinct ar.APRNUMB, ar.APPSTAT, ar.APPFRWD, ar.APPRSUB, ar.APPFVAL, ar.APRTITL,ar.ARQCODE, ar.ATYCODE, ar.ATCCODE, ar.ARQYEAR, 
                                                                                ar.RQESTTO, decode(ar.APPSTAT, 'N', 'NEW', 'F', 'FORWARD', 'A', 'APPROVED', 'P', 'PENDING', 'Q', 'QUERY', 'S', 'RESPONSE', 
                                                                                'C', 'COMPLETED', 'R', 'REJECTED', 'NEW') APPSTATUS, decode(ar.APPSTAT, 'N','1','F', '2', 'A', '3', 'P', '4', 'Q', '5', 'S', 
                                                                                '6', 'C', '7', 'R', '8', '9') APPORDER, (select APPSTAT from APPROVAL_REQUEST where ARQSRNO = 1 and DELETED = 'N' and 
                                                                                ARQCODE = ar.ARQCODE and ARQYEAR = ar.ARQYEAR and ATCCODE = ar.ATCCODE and ATYCODE = ar.ATYCODE and ATMCODE = ar.ATMCODE and 
                                                                                APMCODE = ar.APMCODE and APPSTAT = 'A') as APSTAT, (select EMPNAME from employee_office where empsrno = ar.RQESTTO) as reqto, 
                                                                                ar.ARQSRNO, ar.APPRDET, ar.PRICODE, (select EMPNAME from employee_office where empsrno in (select REQSTBY from APPROVAL_REQUEST 
                                                                                where ARQCODE = ar.ARQCODE and ARQYEAR = ar.ARQYEAR and ARQSRNO = 1 and ATCCODE = ar.ATCCODE and ATYCODE = ar.ATYCODE and 
                                                                                ATMCODE = ar.ATMCODE and APMCODE = ar.APMCODE and deleted = 'N')) as reqby, (select EMPNAME from employee_office 
                                                                                where empsrno = ar.REQSTFR) as pndingby, ar.APPRFOR, pr.PRICODE||' - '||pr.PRINAME priority 
                                                                            from APPROVAL_REQUEST ar, approval_priority pr 
                                                                            where ar.PRICODE = pr.PRICODE and pr.deleted = 'N' and ar.DELETED = 'N' ".$appfrwd." ".$and."
                                                                            order by ar.PRICODE Asc, ARQYEAR desc, APPORDER Asc, APRNUMB desc", "Centra", 'TEST');
                                        ...
                                        if($sql_search[0]['REQBY'] == '') {
                                            $sql_search = select_query_json("select distinct ar.APRNUMB, ar.APPSTAT, ar.APPFRWD, ar.APPRSUB, ar.APPFVAL, ar.APRTITL, ar.ARQCODE, ar.ATYCODE, ar.ATCCODE, 
                                                                                    ...
                                                                                    'S', 'RESPONSE', 'C', 'COMPLETED', 'R', 'REJECTED', 'NEW') APPSTATUS, decode(ar.APPSTAT, 'N','1','F', '2', 'A', '3', 'P', 
                                                                                    '4', 'Q', '5', 'S', '6', 'C', '7', 'R', '8', '9') APPORDER, (select APPSTAT from APPROVAL_REQUEST where ARQSRNO = 1 and 
                                                                                    ...
                                                                                    where empsrno = ar.RQESTTO) as reqto, ar.ARQSRNO, ar.APPRDET, ar.PRICODE, (select EMPNAME from employee_office 
                                                                                    where empsrno in (select REQSTBY from APPROVAL_REQUEST where ARQCODE = ar.ARQCODE and ARQSRNO = 1 and ATCCODE = ar.ATCCODE 
                                                                                    and ATYCODE = ar.ATYCODE and ATMCODE = ar.ATMCODE and APMCODE = ar.APMCODE and deleted = 'N' and rownum = 1)) as reqby, 
                                                                                    (select EMPNAME from employee_office where empsrno = ar.REQSTFR) as pndingby, ar.APPRFOR, pr.PRICODE||' - '||pr.PRINAME priority
                                                                                from APPROVAL_REQUEST ar, approval_priority pr 
                                                                                where ar.PRICODE = pr.PRICODE and pr.deleted = 'N' and ar.DELETED = 'N' ".$appfrwd." ".$and." 
                                                                                order by ar.PRICODE Asc, ARQYEAR desc, APPORDER Asc, APRNUMB desc", "Centra", 'TEST');
                                        }
                                ...
                                for($search_i = 0; $search_i < count($sql_search); $search_i++) { $ij++;
                                    ...
                                    <tr class="odd gradeX" style='background-color:<?=$bgclr?>; color:<?=$clr?>;'>
                                        <td><? $priority = select_query_json("select * from approval_priority 
                                                                                    where DELETED = 'N' and PRICODE = '".$sql_search[$search_i]['PRICODE']."' order by PRINAME", "Centra", 'TEST'); 
                                                echo $priority[0]['PRICODE']." - ".$priority[0]['PRINAME']; ?></td>

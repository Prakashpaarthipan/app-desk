...
try{
	// echo "**".$txtdynamic_subject."**".$txt_policy_subject."**"; exit;
	...
	for($keybrn = 0; $keybrn < count($slt_brnch); $keybrn++) { // branch for loop
		...
		if($_FILES['txt_dynamic_deskprocedure']['name'][0] != '') {
			$assign5=$_FILES['txt_dynamic_deskprocedure']['name'];
			$noofattachment += count($_FILES['txt_dynamic_deskprocedure']['name']);
		}
        ...
        if($_FILES['txt_dynnamic_procedure']['name'][0] != '') {
			$assign6=$_FILES['txt_dynamic_procedure']['name'];
			$noofattachment += count($_FILES['txt_dynemic_procedure']['name']);
		}
		...
		/* $sql_reqexist = select_query_json("select * from APPROVAL_request
													where ARQYEAR = '".$current_year[0]['PORYEAR']."' and ARQSRNO = '1' and ATYCODE = '".$slt_submission."' and ATMCODE = '".$slt_subtype."'
														...
														and trunc(ADDDATE) = TO_DATE('".strtoupper(date('d/M/Y'))."','dd/mon/yyyy')", "Centra", 'TEST'); */
		// if(count($sql_reqexist) <= 0) {
			...
			if($insert_appreq == 1 and ($slt_approval_listings==955 or $slt_approval_listings==954 ))
			{
				$maxtagsrno = select_query_json("Select nvl(Max(APLCYCD),0)+1 maxarqcode From approval_policy_form","Centra","TEST");
				echo "approval_policy_form";
				
				// store pdf file
				// Product Image
				if($_FILES['txtdynamic_deskprocedure']['type'] == "image/jpeg" or $_FILES['txtdynamic_deskprocedure']['type'] == "image/gif" or $_FILES['txtdynamic_deskprocedure']['type'] == "image/png" or $_FILES['txtdynamic_deskprocedure']['type'] == "application/pdf" ) {
						$fldimli = find_indicator( $_FILES['txtdynamic_deskprocedure']['type'] );
						$imgfile1 = $_FILES['txtdynamic_deskprocedure']['tmp_name'];
                        ...
                        switch($_FILES['txtdynamic_procedure']['type']) {
								case 'image/jpeg':
								           $extn1 = 'jpeg';
								case 'image/jpg':
								           $extn1 = 'jpg';
						...
						$expl = explode(".", $_FILES['txtdynamic_deskprocedure']['name']);
						$upload_img1 = $current_year[0]['PORYEAR']."_".$maxarqcode[0]['MAXARQCODE']."_1.pdf";
						$source = $imgfile1;
						...
						$original_complogos1 = "../uploads/text_approval_policy/".$uploadimg."/".$complogos1;
						// echo '!!!'.$complogos1.'<br>';
						move_uploaded_file($source, $original_complogos1);

						/* Upload into FTP */
						$local_file = "../uploads/text_approval_policy/".$uploadimg."/".$complogos1;
						$server_file = 'approval_desk/text_approval_policy/'.$uploadimg.'/'.$complogos1;
						if ((!$conn_id) || (!$login_result)) {
							$upload = ftp_put($ftp_conn, $server_file, $local_file, FTP_BINARY);
							// echo "lar Succes";
							unlink($local_file);
				...
				// store txt file
				$description = $_REQUEST['FCKeditor2'];
				$lpdyear = $current_year[0]['PORYEAR'];
				$txt_srcfilename = "apd_".$lpdyear."_".$srno."_1.txt";

				$local_file = "../uploads/text_approval_policy/".$lpdyear."/".$txt_srcfilename;
				$myfile = fopen($local_file, "w") or die("Unable to open file!");
				fwrite($myfile, $description);
				fclose($myfile);

				$server_file = 'approval_desk/text_approval_policy/'.$lpdyear.'/'.$txt_srcfilename;
				if ((!$conn_id) || (!$login_result)) {
					$upload = ftp_put($ftp_conn, $server_file, $local_file, FTP_BINARY);
					unlink($local_file);
				}
			$f_table = "APPROVAL_POLICY_FORM";
			$f_fld = array();
			   
			$sql_plcymaster = select_query_json("select * from approval_policy_master where APLCYCD = '".$txt_policy_subject."'");
			$sql_creator = select_query_json("select * from employee_office where EMPCODE = '".$txtdynamic_creator."'");
			$sql_coordinator = select_query_json("select * from employee_office where EMPCODE = '".$txtdynamic_coordinator."'");
			$sql_asistby = select_query_json("select * from employee_office where EMPCODE = '".$txtdynamic_assistby."'");
			$f_fld['APLCYCD'] = $sql_plcymaster[0]['APLCYCD'];
			$f_fld['APRNUMB'] = $apprno;
			$f_fld['PLCSUBJ'] = $sql_plcymaster[0]['APLCYNM'];
			$f_fld['EFCTDAT'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$txtdynamic_efct_date; 
			$f_fld['VALDUPT'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$txtdynamic_valid_upto;
			$f_fld['APPRDAT'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$txtdynamic_approval_date;
			$f_fld['PLCYTYP'] =  $txtdynamic_policy_type;
			$f_fld['CRTECNO'] =  $sql_creator[0]['EMPCODE'];
			$f_fld['CRTUSNM'] =  $sql_creator[0]['EMPNAME'];
			$f_fld['CRDECNO'] =  $sql_coordinator[0]['EMPCODE'];
			$f_fld['CRDUSNM'] =  $sql_coordinator[0]['EMPNAME'];
			$f_fld['ASTECNO'] =  $sql_asistby[0]['EMPCODE'];
			$f_fld['ASTUSNM'] =  $sql_asistby[0]['EMPNAME'];
			$f_fld['USERLST'] =  $txtdynamic_userlist;
			$f_fld['DESKPRO'] =  $current_year[0]['PORYEAR']."_".$maxarqcode[0]['MAXARQCODE']."_1.pdf";
			$f_fld['PLCATTC'] =  $txtdynamic_policy_docs;
			$f_fld['PLCDATA'] = str_replace("'", "", $lpdyear.'/'.$txt_srcfilename);
			$f_fld['ADDUSER'] = $_SESSION['tcs_usrcode'];
			$f_fld['ADDDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;
			$f_fld['DELETED'] = 'N';// Y - Yes; N - No;
			$f_fld['APRVDBY'] = '0';
			$f_fld['APRVDUS'] = '0';
			
			print_r($f_fld);
			$f_insert = insert_test_dbquery($f_fld,$f_table);
			}
			
			// $maxarqcode[0]['MAXARQCODE'].'_'.$current_year[0]['PORYEAR'].'_'.$k.'.'.strtolower($path_parts['extension']);
	/*$uploadimg1 = $current_year[0]['PORYEAR'];
				// Product Image
				if($_FILES['txtdynamic_deskprocedure']['type'][$pdi] == "image/jpeg" or $_FILES['txtdynamic_deskprocedure']['type'][$pdi] == "image/gif" or $_FILES['txtdynamic_deskprocedure']['type'][$pdi] == "image/png" or $_FILES['txtdynamic_deskprocedure']['type'][$pdi] == "application/pdf") {
						$fldimli = find_indicator( $_FILES['txtdynamic_deskprocedure']['type'][$pdi] );
						$imgfile1 = $_FILES['txtdynamic_deskprocedure']['tmp_name'][$pdi];
						if($fldimli == 'i')
						{
						...
						switch($_FILES['txtdynamic_deskprocedure']['type'][$pdi]) {
							case 'image/jpeg':
							case 'image/jpg':
							case 'image/gif':
							case 'image/png':
						...
						$expl = explode(".", $_FILES['txtdynamic_deskprocedure']['name'][$pdi]);
						$upload_img1 = $current_year[0]['PORYEAR']."_".$srno."_".$maxprlstno[0]['MAXPRLSTNO'].".".$extn1;
						$source = $imgfile1;
						$complogos1 = preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $upload_img1); //str_replace(" ", "_", $upload_img1));
						$complogos1 = str_replace(" ", "-", $upload_img1);
						$complogos1 = strtolower($complogos10);
						...
						if($fldimli == 'i')
						{
							...
							$dest_thumbfile = "approval_desk/policy_images/".$uploadimg1."/thumb_images/".$complogos1_tmp;
							...
							$server_file = 'approval_desk/policy_images/'.$uploadimg1.'/thumb_images/'.$complogos1;
						...
						$original_complogos1 = "../uploads/policy_images/".$uploadimg1."/".$complogos1;
						// echo '!!!'.$complogos1.'<br>';
						move_uploaded_file($source, $original_complogos10);

						/* Upload into FTP */
						/*$local_file = "../uploads/policy_images/".$uploadimg1."/".$complogos1;
						$server_file = 'approval_desk/policy_images/'.$uploadimg1.'/'.$complogos1;
						if ((!$conn_id) || (!$login_result)) {
							$upload = ftp_put($ftp_conn, $server_file, $local_file, FTP_BINARY);
							// echo "lar Succes";
							unlink($local_file);
		...
		//	echo "insert_appreq = ".$insert_appreq;
						...
						if($insert_appreq == 1 and $slt_approval_listings == 1491)
						{
							...
							for($emp = 0; $emp < sizeof($types_of_submission); $emp++)
							{
								if ($types_of_submission[$emp] == 1) {
									...
									$g_fld_subject['BRNHDSR'] = $BRNHDSR[0]['BRNHDSR'];
									$g_fld_subject['BRNCODE'] = $brn_code;
									$g_fld_subject['TARNUMB'] = $target_number[$emp];
									$g_fld_subject['ATYCODE'] = $types_of_submission[$emp];
									$g_fld_subject['SUBCORE'] = $top_sub[0]['SUBCORE'];
									$g_fld_subject['TOPCORE'] = $top_sub[0]['TOPCORE'];
									//$g_fld['EMPSRNO'] = $empsrno[$emp];
									...
									for($emp1 = 0; $emp1 < sizeof($add_emp_txt[$emp]); $emp1++)
									{
										...
										$g_fld_subject['BRNHDSR'] = $BRNHDSR[0]['MAXARQCODE'];
										$g_fld_subject['BRNCODE'] = $brn_code;
				...
				for($pdi = 0; $pdi < count($txt_prdcode); $pdi++) {
					...
					if($txt_prdcode[$pdi] != '') {
						...
						$prdhsn = select_query_json("Select distinct PRDCODE, PRDNAME, HSNCODE From product_asset
															WHERE depcode = '".$slt_department_asset."' and deleted = 'N' and PRDCODE = '".$prd_cd[0]."'", "Centra", 'TEST');
						...
						$maxprlstno = select_query_json("Select nvl(Max(PRLSTNO),0)+1 MAXPRLSTNO
																...
																WHERE PBDYEAR = '".$current_year[0]['PORYEAR']."' and PBDCODE = ".$srno." and PRLSTYR = '".$current_year[0]['PORYEAR']."' and
																	PRLSTNO = '".$maxprlstno[0]['MAXPRLSTNO']."'", "Centra", 'TEST');
			...
			if($insert_appreq == 1) {
				...
				//uploading the txtdynamic_deskprocedure 
			...
			// artwork
			//policy
			/*for($i=0; $i<count($assign6); $i++)
				{
					if($assign6[$i] != '')
					{
						$poli = '-'; $complogos1 = '-';
						if($_FILES['txt_dynamic_procedure']['type'][$i] == "image/jpeg" or $_FILES['txt_dynamic_procedure']['type'][$i] == "image/gif" or $_FILES['txt_dynamic_procedure']['type'][$i] == "image/png" or $_FILES['txt_dynamic_procedure']['type'][$i] == "application/pdf") {
							$poli = find_indicator( $_FILES['txt_dynamic_procedure']['type'][$i] );

							$imgfile1 = $_FILES['txt_dynamic_procedure']['tmp_name'][$i];
							if($poli == 'i')
							{
								$info = getimagesize($imgfile1);
								$image1 = imagecreatefromjpeg($imgfile1);
							...
							switch($_FILES['txt_dynamic_procedure']['type'][$i]) {
								case 'image/jpeg':
								case 'image/jpg':
								case 'image/gif':
								case 'image/png':
							...
							// $upload_img1 = $_FILES['txt_dynamic_procedure']['name'];
							$expl = explode(".", $_FILES['txt_dynamic_procedure']['name'][$i]);
							$upload_img1 = $maxarqcode[0]['MAXARQCODE']."_".$slt_submission."_".$slt_topcore."_".$current_year[0]['PORYEAR']."_lastapproval_".$poli."_".$i.".".$extn1;
							$source = $imgfile1;
							$complogos1 = preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $upload_img1); //str_replace(" ", "_", $upload_img1));
							$complogos1 = str_replace(" ", "-", $upload_img1);
							$complogos1 = strtolower($complogos1);
							...
							if($poli == 'i')
							{
								$upload_img1_tmp = $maxarqcode[0]['MAXARQCODE']."_".$slt_submission."_".$slt_topcore."_".$current_year[0]['PORYEAR']."_lastapproval_".$poli."_".$i.".jpg";
								$source_tmp = $imgfile1;
								$complogos1_tmp = preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', $upload_img1_tmp); //str_replace(" ", "_", $upload_img1));
								$complogos1_tmp = str_replace(" ", "-", $upload_img1_tmp);
								$complogos1_tmp = strtolower($complogos1_tmp);
								...
								$resized_file = "../uploaded_files/". $complogos1_tmp;
								$dest_thumbfile = "approval_desk/request_entry/lastapproval/thumb_images/".$complogos1_tmp;
								imagejpeg($tmp1, $resized_file, 50);
								imagedestroy($image1);
								imagedestroy($tmp1);
								// echo "^^^".$complogos1_tmp."^^^".$source_tmp."^^^".$dest_thumbfile."^^^".$complogos1.'<br>';
								move_uploaded_file($source_tmp, $dest_thumbfile);
								$local_file = "../uploaded_files/".$complogos1_tmp;
								$server_file = 'approval_desk/request_entry/lastapproval/thumb_images/'.$complogos1;
								// exit;
							...
							$original_complogos1 = "../uploads/request_entry/lastapproval/".$complogos1;
							move_uploaded_file($source, $original_complogos1);

							/* Upload into FTP */
							//$local_file = "../uploads/request_entry/lastapproval/".$complogos1;
							//$server_file = 'approval_desk/request_entry/lastapproval/'.$complogos1;

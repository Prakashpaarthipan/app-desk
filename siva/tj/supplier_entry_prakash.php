			...
			if(count($slt_supcode) !=0){
				...
				for($m =0;$m<count($slt_supcode) ; $m++){
					...
					if ($twgtsrno=='0')
						...
						else /* if (count($weight_range[$i+1])=="1") */
						{
				...
				for($n=0;$n<count($slt_supcode);$n++)
				{
                 $ii = $i + 1;
					...
					$prd_code=explode(" - ",$prdcode[$n]);
							...
							$field_appreqdt_mc['FR_PRDWGHT'] = $fr_wgt[$n][$ii][$j];
							$field_appreqdt_mc['TO_PRDWGHT'] = $to_wgt[$n][$ii][$j];
							$field_appreqdt_mc['OPTMKCH'] = $slt_mc_opt[$n][$ii][$j];
							$field_appreqdt_mc['PRDMKCH'] = $mak_charge[$n][$ii][$j];
	...
	/*if(isset($sbmt_supplier)) 
	{
		...
		if(empty($sqlExistQuery)) 
		{
			...
			{ ?>
					...
					<script>window.location='supplier_entry_prakash.php';</script>
			<?
					//exit();
			} 
			else 
										...
										<div class="fair_border">
											...
											<div class="col-md-12" style='text-transform:uppercase;'>
												...
												<div class="col-sm-6">
													...
													<select input type='text'  class="form-control custom select" style="text-transform: uppercase;"  name="weight_range" id="weight_range_1" value="<?php echo $weight_range_1;?>" data-toggle="tooltip" data-placement="top" data-original-title="weight range" >

...
<?php exit();
	...
	if(isset($sbmt_supplier))
	{
		...
		$sql_year=select_ktmquery("select PORYEAR from trandata.codeinc@ktmcentr");
			$sql_entnum=select_ktmtestquery("SELECT Nvl(Max(ENTNUMB),0)+1  ENTNUMB FROM Supplier_Touch_Add_Request WHERE ENTYEAR='".$sql_year[0]['PORYEAR']."'");
			$sql_entsrno=select_ktmtestquery("SELECT Nvl(Max(ENTSRNO),0)+1  ENTSRNO FROM SUPPLIER_TOUCH_ADD_REQUEST WHERE ENTNUMB='".$sql_entnum[0]['ENTNUMB']."' and ENTYEAR='".$sql_year[0]['PORYEAR']."'");
			...
			if(count($slt_supcode) !=0){
				...
				for($m =0;$m<count($slt_supcode) ; $m++){
					...
					$sql_Prdtouch = select_ktmtestquery("Select Prdtouch from Product where prdcode='".$prd_code[0]."'");
					$field_appreqdt_m['ENTYEAR'] = $sql_year[0]['PORYEAR'];
					$field_appreqdt_m['ENTNUMB'] = $sql_entnum[0]['ENTNUMB'];
					$field_appreqdt_m['ENTSRNO'] = $sql_entsrno[0]['ENTSRNO'];
					...
					//
					$field_appreqdt_m['PAYMODE'] = $pay_mode_save[$m];
					...
					$insert_appreqdt = insert_ktmtestquery($field_appreqdt_m, $tbl_appreqdt_m);
				}
				...
				{			
					...
					$prd_code=explode(" - ",$prdcode[$n]);
							$sql_entnum=select_ktmtestquery("SELECT Nvl(Max(ENTNUMB),0)+1  ENTNUMB FROM Supplier_Touch_mc WHERE ENTYEAR='".$sql_year[0]['PORYEAR']."'");
							...
							$insert_appreqdt_mc = insert_ktmtestquery($field_appreqdt_mc, $tbl_appreqdt_mc);
				}
	...
	/*if(isset($sbmt_supplier)) 
	{
		...
		if(empty($sqlExistQuery)) 
		{
			...
			exit();	
			//exit;*/
			if($insert_appreqdt_m == 1 || $insert_appreqdt_mc == 1) 
			{ ?>
					<script>alert("Touch Request Entry Added Succesfully");</script>
					<script>window.location='entry1.php';</script>
			<?
					//exit();
			} 
			else 
				...
				<form name="counter_master" id="counter_master" method="post" action="">
					...
					<div class="">
										...
										<div class="fair_border">
											...
											<div class="col-md-12" style='text-transform:uppercase;'>
												...
												<div class="col-sm-6">
													...
													<select input type='text'  class="form-control ui-autocomplete-input number" style="text-transform: uppercase; width:100%; height:12%;"  name="weight_range" id="weight_range_1" value="<?php echo $weight_range_1;?>" data-toggle="tooltip" data-placement="top" data-original-title="weight range" >
											...
											<div class="col-sm-6">	
											       ...
											       <select   class="form-control ui-autocomplete-input number" name="pay_mode[]" id="pay_mode_1" placeholder="SELECT PAYMENT MODE" title="Regular" style="width:100%;">
						...
						<div id = "supplier_dynamic_table" name ="supplier_dynamic_table"> <h4>Supplier Touch Value Details</h4></div>
								...
								<table id="example0" class="table table-bordered table-striped table-hover controller" style="border-collapse:none; overflow: hidden;">
								<thead>
				...
				<div class="tab-pane" id="tab_5" style="display:block; padding:5px; border: 1px solid #e0e0e0;min-height: 350px;margin:0 0 px;">
					...
					<div onscroll='scroller("scrollme", "scroller")' style="overflow:scroll; height:650px" id=scrollme> 
						<table id="example0" class="table table-bordered table-striped table-hover controller" style="border-collapse:collapse; overflow: hidden;">
							...
							<tbody> 
						  <?php
							$sql_touch = select_ktmtestquery("Select S.Supcode,S.Supname,C.Ctyname,P.Prdcode,P.Prdname,S.Seccode,G.SECGRNO,S.Secname,G.Grpname,R.entyear,R.entnumb,R.ENTSRNO,R.HODREASON,R.PMREASON,R.GMREASON,R.HOD_STATUS,R.PM_STATUS,R.GM_STATUS,Decode(Allrange,'Y','ALL WGT RANGE',WGTRANGE)  Wgtrange,Ntotcalc,
							...
							{ $i++; 
								$sql_compariosion = select_ktmtestquery("Select S.Supcode,S.Supname,C.Ctyname,P.Prdcode,P.Prdname,R.Totcalc,Decode(R.Paymode,'R','REGULAR',Decode(R.paymode,'S','SPOT','ADVANCE')) Paymode  From Supp_purchase_profile R,Supplier S,City C,Product P
				...
				<div class="tab-pane" id="tab_2" style="display:block; padding:5px; border: 1px solid #e0e0e0;min-height: 350px;margin:0 0 px;">
					...
					<div onscroll='scroller("scrollme", "scroller")' style="overflow:scroll; height:650px" id=scrollme> 
						<table id="supplier_touch_request" class="table table-bordered table-striped table-hover controller" style="border-collapse:collapse; overflow: hidden;">
							...
							<tbody> 
						  <?php
							$sql_touch = select_ktmtestquery("Select S.Supcode,S.Supname,C.Ctyname,P.Prdcode,P.Prdname,S.Seccode,G.SECGRNO,S.Secname,G.Grpname,R.entyear,R.entnumb,R.ENTSRNO,R.WGTRANGE EUCOM,R.HOD_STATUS,R.HOD_TOTCALC,R.HODREASON,R.PMREASON,Decode(Allrange,'Y','ALL WGT RANGE',WGTRANGE)  Wgtrange,Ntotcalc,
							Decode(R.Paymode,'R','REGULAR',Decode(R.paymode,'S','SPOT','ADVANCE')) Paymode  From Supplier_Touch_Add_Request R,Supplier S,City C,Product P, Section_Group G, Section S
				...
				<div class="tab-pane" id="tab_3" style="display:block; padding:5px; border: 1px solid #e0e0e0;min-height: 350px;margin:0 0 px;">
					...
					<div onscroll='scroller("scrollme", "scroller")' style="overflow:scroll; height:650px" id=scrollme> 
						<table id="supplier_touch_request" class="table table-bordered table-striped table-hover controller" style="border-collapse:collapse; overflow: hidden;">
							...
							<tbody>--> 
						  <?php
							$sql_touch = select_ktmtestquery("Select S.Supcode,S.Supname,C.Ctyname,P.Prdcode,P.Prdname,S.Seccode,G.SECGRNO,S.Secname,G.Grpname,R.entyear,R.entnumb,R.ENTSRNO,R.GM_STATUS,R.HODREASON,R.PMREASON,R.GMREASON,R.PM_STATUS,R.HOD_STATUS,R.PM_TOTCALC,R.GM_TOTCALC,Decode(Allrange,'Y','ALL WGT RANGE',WGTRANGE)  Wgtrange,Ntotcalc,
							Decode(R.Paymode,'R','REGULAR',Decode(R.paymode,'S','SPOT','ADVANCE')) Paymode  From Supplier_Touch_Add_Request R,Supplier S,City C,Product P, Section_Group G, Section S
				...
				<div class="tab-pane" id="tab_4" style="display:block; padding:5px; border: 1px solid #e0e0e0;min-height: 350px;margin:0 0 px;">
					...
					<div onscroll='scroller("scrollme", "scroller")' style="overflow:scroll; height:650px" id=scrollme> 
						<table id="example0" class="table table-bordered table-striped table-hover controller" style="border-collapse:collapse; overflow: hidden;">
							...
							<tbody> 
						  <?php
							$sql_touch = select_ktmtestquery("Select S.Supcode,S.Supname,C.Ctyname,P.Prdcode,P.Prdname,S.Seccode,G.SECGRNO,S.Secname,G.Grpname,R.entyear,R.entnumb,R.ENTSRNO,R.HODREASON,R.PMREASON,R.GMREASON,R.HOD_STATUS,R.PM_STATUS,R.HOD_TOTCALC,Decode(Allrange,'Y','ALL WGT RANGE',WGTRANGE)  Wgtrange,Ntotcalc,
							Decode(R.Paymode,'R','REGULAR',Decode(R.paymode,'S','SPOT','ADVANCE')) Paymode  From Supplier_Touch_Add_Request R,Supplier S,City C,Product P, Section_Group G, Section S

...
if($inner_menuaccess[0]['VEWVALU'] == 'Y') {  // Menu Permission is allowed 
                                ...
                                <tr class="odd gradeX" style='background-color:<?=$bgclr?>; color:<?=$clr?>;'>
                                    ...
                                    <td>
                                        ...
                                        <?  $userid = $_SESSION['tcs_empsrno'];
                                            ...
                                            <? } ?>
                                            <br><span class="label label-info" id ="timetoskip_<?=$ij?>" style="font-size: 12px">1 Hrs</span>
                                            <input type="hidden" name="" id="timetoskip1_<?=$ij?>" value="1" />
                                            <input type="hidden" name="" id="stopnextalter_<?=$ij?>" value="0" />
                                            <br><span><select name="alterTime"  id="alterTime_<?=$ij?>"  style="width:30%" onchange="addnewtime(this.value,'<?=$ij?>')">
    ...
    <!-- START SCRIPTS -->
        ...
        <script type="text/javascript">
            ...
            function addnewtime(time,id){
              var stop = $("#stopnextalter_"+id).val();
              if (stop !=1){
                var actualtime = $("#timetoskip1_"+id).val();
               var newtime =  parseInt(time)+parseInt(actualtime);
               alert(newtime);
               $("#stopnextalter_"+id).val(1);
               $("#timetoskip1_"+id).attr('readonly',true);
              }
               else{
                alert("Alreadytime changed");

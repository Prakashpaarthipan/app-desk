	...
	case 125: 		// PKN - 1118
		...
		$sql_expkn = select_query_json("select * from APPROVAL_REQUEST 
												where ARQCODE = '".$_REQUEST['reqid']."' and ARQYEAR = '".$_REQUEST['year']."' and ATCCODE = '".$_REQUEST['creid']."' and 
													ATYCODE = '".$_REQUEST['typeid']."' and deleted = 'N' and REQSTBY = 125 
...
$next_url = "http://$_SERVER[HTTP_HOST]/approval_desk/".$rturl."?status=success";
// echo "<pre>"; 
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	if($update_apprq == 1 and $cntlive > 0) {
		...
		for ($aai = 0; $aai < count($hidchk_reject_reason); $aai++) { 
			...
			if(count($chk_reject_reason[$hidchk_reject_reason[$aai]]) <= 0) {
				...
				echo "**".$where_prlist = "PBDYEAR = '".$arqyear."' and PBDCODE = '".$slt_aprno."' and PBDLSNO = '".$hidchk_reject_reason[$aai]."'";
				// $update_prlist = update_dbquery($field_apprq, $tbl_apprq, $where_apprq);
				echo "!!!".$update_prlist."@@@";
				// Update in APPROVAL_PRODUCTLIST Table
		...
		if($_SESSION['tcs_empsrno'] == 125 and $txt_extarno != $slt_targetno and $txtrequest_value > 0) {
			$sql_extr = select_query_json("select sum(nvl(APPRVAL, 0)) aprvlu from approval_budget_planner_temp 
													where BRNCODE=".$slt_branch." and APRYEAR = '".$hidapryear."' and EXPSRNO = ".$slt_core_department." and deleted = 'N'", 'Centra', 'TEST');
			if(count($sql_extr) > 0) { 
				$sql_yrlyttl = select_query_json("select sum(distinct nvl(sm.BUDVALUE, 0)) BUDVALUE, (sum(distinct nvl(sm.APPVALUE, 0)) + sum(distinct nvl(tm.APPRVAL, 0))) APPVALUE, 
														...
														where sm.BUDYEAR=tm.APRYEAR AND sm.BRNCODE=tm.BRNCODE AND sm.EXPSRNO=tm.EXPSRNO and tm.deleted = 'N' and sm.BRNCODE=".$slt_branch." and 
															sm.BUDYEAR = '".$hidapryear."' and sm.EXPSRNO = ".$slt_core_department."", 'Centra', 'TEST'); 
			} else { 
				$sql_yrlyttl = select_query_json("select sum(nvl(BUDVALUE, 0)) BUDVALUE, sum(nvl(APPVALUE, 0)) APPVALUE, (sum(nvl(BUDVALUE, 0)) - sum(nvl(APPVALUE, 0))) pendingvalue
														from budget_planner_head_sum where BRNCODE=".$slt_branch." and BUDYEAR = '".$hidapryear."' and EXPSRNO = ".$slt_core_department."", 'Centra', 'TEST'); 
			}
		...
		$ot = find_datacount($hid_reqid."_".$hid_slt_submission."_".$hid_slt_topcore."_".$hidapryear."_fieldimpl_", 'i', 'fieldimpl');
		// fieldimpl
		// echo "CAME"; exit;
		for($i=0; $i<count($assign0); $i++)
		{
			if($assign0[$i] != '')
			{
				...
				if($_FILES['txt_submission_fieldimpl']['type'][$i] != '') {
					...
					$upload_img1 = $maxarqcode[0]['MAXARQCODE']."_".$slt_submission."_".$slt_topcore."_".$hidapryear."_fieldimpl_".$fldimli."_".($ot + $i).".".$extn1;
					$source = $imgfile1;
					...
					if($fldimli == 'i') 
					{
						$upload_img1_tmp = $maxarqcode[0]['MAXARQCODE']."_".$slt_submission."_".$slt_topcore."_".$hidapryear."_fieldimpl_".$fldimli."_".($ot + $i).".jpg";
						$source_tmp = $imgfile1;
			...
			if(count($mnt_yr) > 0) {
				...
				if($slt_approval_listings != 807 and $slt_approval_listings != 777) {
					...
					for($vi = 0; $vi < count($sql_ext_curr_month_all); $vi++) {
						...
						// print_r($field_appplan);
						$where_appplan = " APRNUMB='".$txt_approval_number."' and BRNCODE=".$slt_branch." and APRYEAR = '".$hidapryear."' and EXPSRNO = ".$group_wise[0]['EXPSRNO']." and deleted = 'N' and TARNUMB = ".$slt_targetno." and APRSRNO = '".$sql_ext_curr_month_all[$vi]['APRSRNO']."'";
						$insert_appplan = update_dbquery($field_appplan, $tbl_appplan, $where_appplan); 
					}
					...
					// print_r($field_appplan);
					$where_appplan = " APRNUMB='".$txt_approval_number."' and BRNCODE=".$slt_branch." and APRYEAR = '".$hidapryear."' and EXPSRNO = ".$group_wise[0]['EXPSRNO']." and deleted = 'N' and TARNUMB = ".$slt_targetno." and APRSRNO = '".$sql_ext_curr_month[0]['APRSRNO']."'";
					$insert_appplan = update_dbquery($field_appplan, $tbl_appplan, $where_appplan); 
					// exit;
					// Remove existing month value and move that value to current month 
				}
				...
				for($cntmntyr = 0; $cntmntyr < count($mnt_yr); $cntmntyr++) {
					...
					if($mnt_yr_amt[$cntmntyr] != $mnt_yr_amt1[$cntmntyr] and $mnt_yr_amt[$cntmntyr] > 0) {
						...
						// print_r($field_appplan);
						$where_appplan = " APRNUMB='".$txt_approval_number."' and BRNCODE=".$slt_branch." and APRYEAR = '".$hidapryear."' and EXPSRNO = ".$group_wise[0]['EXPSRNO']." and deleted = 'N' and TARNUMB = ".$slt_targetno." and APRSRNO = '".$mnt_yraprsrno[$cntmntyr]."'";
						$insert_appplan = update_dbquery($field_appplan, $tbl_appplan, $where_appplan); 
					}
					// This is for "IF ANY CHANGES OCCURS IN APPROVAL VALUE", it will update
				}
	...
	if($hid_samearqsrno == 0) {
		...
		if($end == 1) {
			...
			if($slt_submission == 1 or $slt_submission == 6 or $slt_submission == 7) {
				...
				$where_appplan = " APRNUMB='".$txt_approval_number."' and BRNCODE=".$slt_branch." and APRYEAR = '".$hidapryear."' and EXPSRNO = ".$group_wise[0]['EXPSRNO']." and TARNUMB = ".$slt_targetno." and deleted = 'N' and APPMODE = 'N'";
				$insert_appplan = update_dbquery($field_appplan, $tbl_appplan, $where_appplan); 
			}
		}
		...
		if($_REQUEST['hid_action'] == 'sbmt_mdapprove') { // GM approve this instead of MD
			...
			if($_SESSION['tcs_empsrno'] == 61579 or $_SESSION['tcs_empsrno'] == 59006 or $_SESSION['tcs_empsrno'] == 63624) {
		...
		if($_REQUEST['hid_action'] == 'sbmt_reject') {
			...
			if($slt_submission == 1 or $slt_submission == 6 or $slt_submission == 7) {
				...
				$where_appplan = " APRNUMB='".$txt_approval_number."' and BRNCODE=".$slt_branch." and APRYEAR = '".$hidapryear."' and EXPSRNO = ".$group_wise[0]['EXPSRNO']." and deleted = 'N' and TARNUMB = ".$slt_targetno."";
				$insert_appplan = update_dbquery($field_appplan, $tbl_appplan, $where_appplan); 
			}
		...
		/* echo $where_appreq = " APRNUMB='".$txt_approval_number."' and BRNCODE=".$slt_branch." and APRYEAR = '".$hidapryear."' and EXPSRNO = ".$group_wise[0]['EXPSRNO']." and deleted = 'N' and TARNUMB = ".$slt_targetno.""; */
		...
		$field_appreq['IMDUEDT'] = $impldue_date1;
	...
	$addiv_return = 8;
	if(($hid_int_verification == 'F' or $hid_int_verification == 'S') and $_REQUEST['hid_action'] == 'sbmt_forward' and $insert_appreq == 1 and $frwrdemp[0]['EMPSRNO'] == 21344 and $_SESSION['tcs_empsrno'] != 188) {
		$addiv_return = add_ivuser(21344, 188, $txt_approval_number, $maxarqcode[0]['MAXARQSRNO']); // MR. SK Sir User - 21344 & MR. Ashok Sir User - 188
	}
	...
	// Move the Temp Table to Live Table - PKN Login
	if($_SESSION['tcs_empsrno'] == 125 and $txt_extarno == $slt_targetno) { // echo "STEP1";
		// Step 1 : Move the Temp Table records to LIVE Table
		$ivqry = delete_dbquery("INSERT INTO approval_budget_planner select APRNUMB,APRSRNO,APRPRID,APRMNTH,APPRVAL,APPMNTH,APPYEAR,TARNUMB,RESVALU,EXTVALU,BUDMODE,APRYEAR,ADDUSER,ADDDATE,'','','N','','',BRNCODE,APPMODE,EXPSRNO,'0','' from approval_budget_planner_temp where APRNUMB = '".$txt_approval_number."' and DELETED = 'N'");
		...
		$where_appplan = " APRNUMB='".$txt_approval_number."' and DELETED = 'N' ";
		$insert_appplan = update_dbquery($field_appplan, $tbl_appplan, $where_appplan); 
	} elseif($_SESSION['tcs_empsrno'] == 125 and $txt_extarno != $slt_targetno and $tarnochang_allowornot == 1) { // echo "STEP2";
	...
	if($insert_appreq == 1) { // exit(); 
		...
		$sql_all_mail = select_query_json("select distinct emp.empsrno,mail.emailid,hir.amhsrno from approval_request req, approval_mdhierarchy hir, approval_email_master mail, employee_office emp
			where req.aprnumb = hir.aprnumb and emp.empcode=hir.apphead and emp.empsrno=mail.empsrno and hir.aprnumb = '".$txt_approval_number."' and req.appstat = 'A' order by amhsrno", 'Centra', 'TCS');
		...
		$addiv_return1 = 8;
		if($hid_int_verification == 'F' and $_REQUEST['hid_action'] == 'sbmt_forward' and $insert_appreq == 1 and $frwrdemp[0]['EMPSRNO'] == 21344 and $_SESSION['tcs_empsrno'] != 188 and $addiv_return == 0) {
			// echo "HAI";
			$addiv_return1 = add_ivuser(21344, 188, $txt_approval_number, $maxarqcode[0]['MAXARQSRNO']); // MR. SK Sir User - 21344 & MR. Ashok Sir User - 188
		} 
		// echo "RET";
...
$sql_reqid = select_query_json("select req.*, prj.APRNAME, top.ATCNAME, typ.ATYNAME, apm.APMNAME, regexp_replace(SubStr(nicname,1,4),'[0-9]','')||SubStr(nicname,5,10) branch, emp.EMPCODE, 
									...
									where req.ATYCODE = typ.ATYCODE and req.APRCODE = prj.APRCODE and req.ATCCODE = top.ATCCODE and req.APMCODE = apm.APMCODE and req.BRNCODE = brn.BRNCODE 
										and req.DEPCODE = ast.DEPCODE and req.ARQCODE = '".$_REQUEST['reqid']."' and req.ARQYEAR = '".$_REQUEST['year']."' and 
										...
										req.INTPEMP = '".$usrid."') and req.appstat in ('W', 'N', 'Z') ".$usr." ".$mr_ak_date."
...
if(count($sql_reqid) <= 0) { 
	$sql_reqid = select_query_json("select req.*, prj.APRNAME, top.ATCNAME, typ.ATYNAME, apm.APMNAME, regexp_replace(SubStr(nicname,1,4),'[0-9]','')||SubStr(nicname,5,10) branch, emp.EMPCODE, 
											...
											from branch where DELETED = 'N' and BRNCODE = req.BRNCODE) branch 
...
if(($_SESSION['tcs_usrcode'] == 9938358 or $_SESSION['tcs_usrcode'] == 9193333 or $_SESSION['tcs_usrcode'] == 3000000) and (count($sql_reqid) <= 0)) { 
	$sql_reqid = select_query_json("select req.*, prj.APRNAME, top.ATCNAME, typ.ATYNAME, apm.APMNAME, regexp_replace(SubStr(nicname,1,4),'[0-9]','')||SubStr(nicname,5,10) branch, emp.EMPCODE, 
									...
									where req.ATYCODE = typ.ATYCODE and req.APRCODE = prj.APRCODE and req.ATCCODE = top.ATCCODE and req.APMCODE = apm.APMCODE and req.BRNCODE = brn.BRNCODE 
										and req.DEPCODE = ast.DEPCODE and req.ARQCODE = '".$_REQUEST['reqid']."' and req.ARQYEAR = '".$_REQUEST['year']."' and 
										req.ATCCODE = '".$_REQUEST['creid']."' and req.ATYCODE = '".$_REQUEST['typeid']."' and req.deleted = 'N' and brn.DELETED = 'N' and 
										ast.DELETED = 'N' and emp.empsrno = req.ADDUSER and brn.BRNMODE in ('B', 'K', 'T') and req.appstat in ('W', 'N', 'Z') ".$usr." ".$mr_ak_date."
									order by req.ARQCODE, req.ARQSRNO, req.ATYCODE", 'Centra', 'TEST');
	$viewonly = 1;
}
...
<body>
	...
	<table border=0 cell-padding=1 cell-spacing=1 align='center' class="fixed" style='border:5px solid #303030; background-color: #ffffff; border-style:double; width:796.8px; max-height:1123.2px; padding:7px' ng-controller="customersCtrl">
				...
				<tr>
					<td colspan=3 style='width:100%; height:20px; text-align:right;'>
						<label>Date : <?=$sql_reqid[0]['ADDDATE_DATE']?><br>
							...
							if($_SESSION['tcs_descode'] == 132 or $_SESSION['tcs_descode'] == 189 or $_SESSION['tcs_descode'] == 19 or $_SESSION['tcs_descode'] == 165 or $_SESSION['tcs_descode'] == 78 or $_SESSION['tcs_descode'] == 9) { 
								// HDO / DGM / GM / Sr.GM / MD's can change the Process Priority
								$allow_priority = 1;
							}
							...
							if($sql_approve_leads[0]['PRICODE'] == '') { 
								...
								switch ($sql_process_priority[0]['PRICODE']) {
									...
									if($allow_priority == 1) { ?>
									<input type="radio" name="slt_priority" id="slt_priority" value="1" title="DO RIGHT AWAY - URGENT AND IMPORTANT" <? if($sql_process_priority[0]['PRICODE'] == 1) { ?> checked <? } ?>>&nbsp;<span class="badge badge-danger" title="DO RIGHT AWAY - URGENT AND IMPORTANT" style="font-size:20px; background-color:#FF0000; font-weight:bold;">1</span>&nbsp;
									<input type="radio" name="slt_priority" id="slt_priority" value="2" title="PLAN TO DO ASAP - NOT URGENT BUT IMPORTANT" <? if($sql_process_priority[0]['PRICODE'] == 2) { ?> checked <? } ?>>&nbsp;<span class="badge badge-warning" title="PLAN TO DO ASAP - NOT URGENT BUT IMPORTANT" style="font-size:20px; background-color:#D58B0A; font-weight:bold;">2</span>&nbsp;
									<input type="radio" name="slt_priority" id="slt_priority" value="3" title="DELEGATE - URGENT BUT NOT IMPORTANT" <? if($sql_process_priority[0]['PRICODE'] == 3) { ?> checked <? } elseif($sql_approve_leads[0]['PRICODE'] == '') { ?> checked <? } ?>>&nbsp;<span class="badge badge-success" title="DELEGATE - URGENT BUT NOT IMPORTANT" style="font-size:20px; background-color:#299654; font-weight:bold;">3</span>&nbsp; 
									<? /* ?>
							...
							<? 	} 
								...
								<input type="radio" name="slt_priority" id="slt_priority" value="1" title="DO RIGHT AWAY - URGENT AND IMPORTANT" <? if($sql_approve_leads[0]['PRICODE'] == 1) { ?> checked <? } ?>>&nbsp;<span class="badge badge-danger" title="DO RIGHT AWAY - URGENT AND IMPORTANT" style="font-size:20px; background-color:#FF0000; font-weight:bold;">1</span>&nbsp;
								<input type="radio" name="slt_priority" id="slt_priority" value="2" title="PLAN TO DO ASAP - NOT URGENT BUT IMPORTANT" <? if($sql_approve_leads[0]['PRICODE'] == 2) { ?> checked <? } ?>>&nbsp;<span class="badge badge-warning" title="PLAN TO DO ASAP - NOT URGENT BUT IMPORTANT" style="font-size:20px; background-color:#D58B0A; font-weight:bold;">2</span>&nbsp;
								<input type="radio" name="slt_priority" id="slt_priority" value="3" title="DELEGATE - URGENT BUT NOT IMPORTANT" <? if($sql_approve_leads[0]['PRICODE'] == 3) { ?> checked <? } elseif($sql_approve_leads[0]['PRICODE'] == '') { ?> checked <? } ?>>&nbsp;<span class="badge badge-success" title="DELEGATE - URGENT BUT NOT IMPORTANT" style="font-size:20px; background-color:#299654; font-weight:bold;">3</span>&nbsp;
				...
				switch($find_lead)
				{
					case 3: // Manager
							if($sql_approve_comnts[$alaprcomments]['REQESEC'] == 137) {
								if($sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'I' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'P' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'Q') {
									...
									// $cc_sign = "ftp://$ftp_user_name_apdsk:$ftp_user_pass_apdsk@$ftp_server_apdsk.$ftp_srvport_apdsk/approval_desk/digital_signature/".$cc_empsrno.".png";
								...
								if($sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'I' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'P' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'Q') { // echo "@@";
									...
									/* Read Cost Control sigature from FTP */
									// $srexc_mgr_sign_audit = "ftp://$ftp_user_name_apdsk:$ftp_user_pass_apdsk@$ftp_server_apdsk.$ftp_srvport_apdsk/approval_desk/digital_signature/".$srexc_mgr_empsrno_audit.".png";
					...
					case 67: // TJ / KTM BM
							if($sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'I' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'P' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'Q') {
								...
								// $hod_sign[] = "ftp://$ftp_user_name_apdsk:$ftp_user_pass_apdsk@$ftp_server_apdsk.$ftp_srvport_apdsk/approval_desk/digital_signature/".$sql_approve_comnts[$alaprcomments]['REQSTBY'].".png";
								if(file_exists("ftp://$ftp_user_name_apdsk:$ftp_user_pass_apdsk@$ftp_server_apdsk.$ftp_srvport_apdsk/approval_desk/digital_signature/".$sql_approve_comnts[$alaprcomments]['REQSTBY'].".png")) {
					...
					case 69: // KTM / TJ HOD
							...
							if($sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'I' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'P' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'Q') {
								...
								// $hod_adddate[] = date("d-M-Y h:i:s A", strtotime($sql_approve_comnts[$alaprcomments]['RQADDDATE']));
					...
					case 19:
							if($sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'I' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'P' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'Q') {
								...
								$gm_adddate = date("d-M-Y h:i:s A", strtotime($sql_approve_comnts[$alaprcomments]['RQADDDATE']));
								...
								// $gm_sign[] = "ftp://$ftp_user_name_apdsk:$ftp_user_pass_apdsk@$ftp_server_apdsk.$ftp_srvport_apdsk/approval_desk/digital_signature/".$gm_empsrno.".png";
					...
					case 165:
							if($sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'I' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'P' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'Q') {
								...
								// $srgm_sign = "ftp://$ftp_user_name_apdsk:$ftp_user_pass_apdsk@$ftp_server_apdsk.$ftp_srvport_apdsk/approval_desk/digital_signature/".$srgm_empsrno.".png";
					...
					case 78:
							...
							if($sql_approve_comnts[$alaprcomments]['REQSTBY'] == 21344 && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'I' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'P' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'Q')
							{
								...
								// $ak_sign = "ftp://$ftp_user_name_apdsk:$ftp_user_pass_apdsk@$ftp_server_apdsk.$ftp_srvport_apdsk/approval_desk/digital_signature/".$ak_empsrno.".png";
					...
					case 9:
							...
							if($sql_approve_comnts[$alaprcomments]['REQSTBY'] == 43400 && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'I' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'P' && $sql_approve_comnts[$alaprcomments]['APPFRWD'] != 'Q')
							{
								...
								// $ps_sign = "ftp://$ftp_user_name_apdsk:$ftp_user_pass_apdsk@$ftp_server_apdsk.$ftp_srvport_apdsk/approval_desk/digital_signature/".$ps_empsrno.".png";
			...
			<table border=0 style='width:100%; max-width: 773px; '>
				...
				<tr style='min-height:20px; line-height:20px;'>
					...
					<td style='width:80%; text-align:left;'>
						<label>: <span style=" font-size: 18px; font-weight: bold;" class="blue_highlight"><?=$sql_reqid[0]['DYNSUBJ'].$sql_reqid[0]['APMASTER']?></span></label> <!-- Approval Listings -->
					</td>
				</tr>
				...
				<? } else { ?>
					<tr style='min-height:20px; line-height:20px;'>
						...
						<td style='width:80%; text-align:left;'>
							<label>: <?=$sql_reqid[0]['APTYPE']?></label> <!-- Specification -->
						</td>
					</tr>
				<? } ?>
							...
							<tr><td colspan='2'>
								...
								<? 	
									...
									<table style="width: 100%; line-height: 15px;">
									    ...
									    <tr><td>Supplier : <b style="font-size: 14px; font-weight: bold;"><?=$sql_prdquotlist[0]['SUPCODE']." - ".$sql_prdquotlist[0]['SUPNAME']; ?></b>; <span style="font-size: 9px;color: #a0a0a0;">( Delivery Duration : <?=$sql_prdquotlist[0]['DELPRID']?> Days )</span></td></tr>
										<tr class="row" style="margin-right: -5px; text-align: center; text-transform: uppercase; background-color: #f0f0f0; color:#000; border-top-left-radius:5px; border-top-right-radius: 5px; display: flex; font-size: 11px; font-weight: bold; width: 100%;">
								            ...
								            <td class="colheight" style="padding: 0px;width: 10%;">Per Piece Rate</td>
								            <td class="colheight" style="padding: 0px;width: 17%;">Tax</td>
		 			...
		 			{ $ncol = $col['COLSRNO']-1;
		 				...
		 				<td <?if($sql_gen_master[$ncol]['COLTYPE'] == "Y"){?> class="colnum"<?}else{?> class="coltext" <?}?>>
		 					<?	if($sql_gen_master[$ncol]['CALRES'] != $col['COLSRNO'])
		 						{ echo $col['COLDET'];}else{ echo round($col['COLDET']);}
		 					?>
		 				</td>
					<?$row = $col['ROWSRNO'];
		...
		if(count($sql_gift)>0 && $sql_ap[0]['APRNUMB'] != ""){
			...
			<table style="width: 100%; line-height: 15px;">
				...
				<tr class="row" style="margin-right: -5px; text-align: center; background-color: transparent; color:#000; display: flex; font-size: 11px; text-transform: uppercase;">
					...
					<td class="colheight" style="padding: 0px;width: 10%;"><?=$gift['DATEJOIN']?></td>
			...
			<table style="width: 100%; line-height: 15px;">
				...
				<tr class="row" style="margin-right: -5px; text-align: center; background-color: transparent; color:#000; display: flex; font-size: 11px; text-transform: uppercase;">
					...
					<td class="colheight" style="padding: 0px;width: 10%;"><?=$branch['DATEJOIN']?></td>
			...
			<table style="width: 100%; line-height: 15px;">
				...
				<tr class="row" style="margin-right: -5px; text-align: center; background-color: transparent; color:#000; display: flex; font-size: 11px; text-transform: uppercase;">
					...
					<td class="colheight" style="padding: 0px;width: 10%;"><?=$desg['DATEJOIN']?></td>
			...
			<table style="width: 100%; line-height: 15px;">
				<tr class="row" style="margin-right: -5px; text-align: center; text-transform: uppercase; background-color: #f0f0f0; color:#000; border-top-left-radius:5px; border-top-right-radius: 5px; display: flex; font-size: 11px; font-weight: bold; width: 100%;">
		            ...
		            <td class="colheight" style="padding: 0px;width: 10%;">Current Department</td>
		            <td class="colheight" style="padding: 0px;width: 12%;">Current Designation</td>
		            <td class="colheight" style="padding: 0px;width: 18%;">New Department</td>
		        </tr>
				<? $g=0;
				...
				<tr class="row" style="margin-right: -5px; text-align: center; background-color: transparent; color:#000; display: flex; font-size: 11px; text-transform: uppercase;">
					...
					<td class="colheight" style="padding: 0px;width: 10%;"><?=$dept['DATEJOIN']?></td>
					...
					<td class="colheight" style="padding: 0px;width: 18%;color: green;font-weight: bold;"><?=$dept['NEWDEP']?></td>
				</tr>
				<?}?>					 
			</table>
			<? }
			...
			<table style="width: 100%; line-height: 15px;">
				...
				<tr class="row" style="margin-right: -5px; text-align: center; background-color: transparent; color:#000; display: flex; font-size: 11px; text-transform: uppercase;">
					...
					<td class="colheight" style="padding: 0px;width: 10%;"><?=$salary['DATEJOIN']?></td>
			...
			<tr style='height:20px;'><td></td></tr>
				...
				<? $sql_inteam = select_query_json("select distinct trim(emp.EMPNAME) EMPNAME, emp.EMPCODE, substr(brn.NICNAME,3,10) brnname, des.DESNAME, 
																SUBSTR(sec.esename, 4, 100) esename, decode(emp.empsrno, ".$adduser.", 0, 1) ordersrno 
															from employee_office emp, branch brn, designation des, empsection sec 
															where emp.brncode = brn.brncode and emp.descode = des.descode and emp.ESECODE = sec.ESECODE and emp.EMPCODE not in (1, 2, 3) 
																and (emp.EMPCODE > 1000 or emp.EMPCODE in (557, 530)) and (((des.dessrno <= 16 and des.DESSRNO > 0) or (des.dessrno = 99 and 
																(des.descode = 179 or des.descode = 187))) or (emp.empsrno = ".$adduser." or emp.empsrno = 61579 or emp.empsrno = 59006 
																or emp.empsrno = 1689 or emp.empsrno = 59003 or emp.empsrno = 49277 or emp.empsrno = 26255 or emp.empsrno = 55641 or 
																emp.empsrno = 14180 or emp.empsrno = 2158 or emp.empsrno = 14180 or emp.empsrno = 2158 or emp.empsrno = 14180 or emp.empsrno = 82237 or emp.empsrno = 53864 or emp.empsrno = 86464)) 
																and emp.brncode in(1,10, 14,23,888,102,104,107,112,113,114,116,100)
													union 
														select distinct trim(emp.EMPNAME) EMPNAME, emp.EMPCODE, substr(brn.NICNAME,3,10) brnname, des.DESNAME, 
															...
															where emp.brncode = brn.brncode and emp.descode = des.descode and emp.ESECODE = sec.ESECODE and 
																emp.EMPCODE not in (1, 2, 3) and (emp.EMPCODE > 1000 or emp.EMPCODE in (557, 530)) and (( des.dessrno <= 7 and 
																des.DESSRNO > 0) or (emp.empsrno = ".$adduser." or emp.empsrno = 61579 OR emp.empsrno = 59006 or emp.empsrno = 1689 or 
																emp.empsrno = 59003 or emp.empsrno = 49277 or emp.empsrno = 26255 or emp.empsrno = 55641 or emp.empsrno = 14180 or 
																emp.empsrno = 2158 or emp.empsrno = 14180 or emp.empsrno = 2158 or emp.empsrno = 14180 or emp.empsrno = 82237 or emp.empsrno = 53864 or emp.empsrno = 86464)) and emp.brncode in(201,203,204,206,207)
				...
				<tr style='min-height:25px; max-width: 773px; line-height:25px; display: none;' id="comments_history">
					...
					<table border=1 style='width:100%; max-width: 773px; min-height:100px; height:auto; border:1px solid #0088CC; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px; padding:5px 0px; margin-top: 5px;'>
						...
						<? $sql_othcmd = select_query_json("select emp.empcode,emp.empname,emp.esecode 
																	...
																	where hir.apphead=emp.empcode and aprnumb like '".$sql_reqid[0]['APRNUMB']."'and apphead not in (select empcode from employee_office 
																		where empsrno in (select reqstby from approval_request  where aprnumb like '".$sql_reqid[0]['APRNUMB']."' and arqsrno not in 1 and 
																		appfrwd='F'))order by amhsrno asc", 'Centra', 'TEST');
						...
						foreach ($sql_othcmd as $cmd) {
							...
							<?}else{
								$sql_check = select_query_json("select * from approval_request 
																		where aprnumb in ('".$sql_reqid[0]['APRNUMB']."') and REQESEC=".$cmd['ESECODE']." and appstat='F' order by arqsrno", 'Centra', 'TEST');
		...
		<tr><td colspan="4" style='margin-top:5px; width:100%; text-align:center;'>
			<div class="col-lg-12 col-md-12" style=' text-align:center; padding-right:0px;'>
				...
				<? if($_REQUEST['action'] == 'view') { 
					...
					if(($sql_reqid[0]['APPFRWD'] != 'Q' and $sql_reqid[0]['APPFRWD'] != 'P' and $sql_reqid[0]['APPFRWD'] != 'I') or $_SESSION['tcs_usrcode'] == '1657888' or $_SESSION['tcs_usrcode'] == '1367002' or $_SESSION['tcs_usrcode'] == '1558888') {  
					...
					<? $intverif = 0;
					if($_SESSION['tcs_descode'] == 19 or $_SESSION['tcs_descode'] == 92 or $_SESSION['tcs_descode'] == 189 or $_SESSION['tcs_descode'] == 165 or $_SESSION['tcs_descode'] == 169 or $_SESSION['tcs_descode'] == 132 or $_SESSION['tcs_descode'] == 9 or $_SESSION['tcs_descode'] == 78 or $_SESSION['tcs_empsrno'] == 61579 or $_SESSION['tcs_empsrno'] == 59006 or $_SESSION['tcs_empsrno'] == 188 or $_SESSION['tcs_empsrno'] == 62762 or $_SESSION['tcs_empsrno'] == 1746 or $_SESSION['tcs_empsrno'] == 34593) { $intverif++; // HOD / BM / DGM / GM / SR.GM / MD Verification.. // HW Saravana Kumar ?>
				...
				if($_SESSION['tcs_empsrno'] == 61579 && $sql_reqid[0]['APPSTAT'] =='N' )
				{
					if($prdcnt>0 && $sql_reqid[0]['PURHEAD'] == "") { ?>
						<button type="button" name='sbmt_bid' id='sbmt_bid' tabindex='28' class="btn" title="BID" style="background-color:#d42c65;color: white;" onclick="get_bid_new('<?=$sql_reqid[0]['ARQYEAR']?>','<?=$sql_reqid[0]['IMUSRIP']?>','<?=$sql_reqid[0]['ATCCODE']?>','CREATE');"> <i class="fa fa-gavel"></i> BID</button>
					 <? }
					...
					<? }
					 ...
					 if($final_approval == 0) { $isstage = 1; // Not Final Approval..?>
						<button type="submit" name='sbmt_forward' id='sbmt_forward' tabindex='28' value='Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve"><i class="fa fa-fast-forward"></i> Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve</button>&nbsp;
					<? } // Not Final Approval.. 
				} elseif($_SESSION['tcs_empsrno'] == 61579 && $sql_reqid[0]['APPSTAT'] =='Z' ) {
					if($prdcnt>0) { ?>
				...
				<? if($previous_url != 'http://www.tcsportal.com/approval_desk/'.$rturl) { ?>
				...
				<? if($next_url != 'http://www.tcsportal.com/approval_desk/'.$rturl) { ?>
				...
				} elseif($_REQUEST['tcs_empsrno'] != 61579) {
					if($finish_here == 1) { ?>
						<button type="submit" name='sbmt_mdapprove' id='sbmt_mdapprove' tabindex='28' value='Final Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Use this Final Approve if value is low and no need for MD Approval." style='margin-left:25px;'><i class="fa fa-check-square-o"></i> Final Approve</button>&nbsp;
					<? } elseif($isstage == 0 ) { ?>
						<button type="submit" name='sbmt_forward' id='sbmt_forward' tabindex='28' value='Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve' class="btn btn-success delete_confirm" data-toggle="tooltip" data-placement="top" title="Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve"><i class="fa fa-fast-forward"></i> Stage <?=$sql_reqid[0]['ARQSRNO']?> Approve</button>&nbsp;
...
<table border=0 cell-padding=1 cell-spacing=1 align='center' class="fixed" style='border:5px solid #303030; border-style:double; width:796.8px; max-height:1123.2px; padding:7px'>
	<tr>
		...
		<td style='width:30%; height:20px; text-align:right;'>
			<label style="font-weight:bold;">Page on : <?php echo $ij+2; ?>/<?php echo $pagecount+1; ?></label>
		</td>
	</tr>
	<tr>
		<td style='width:70%; height:20px; font-weight:bold;'>
...
<table border=0 cell-padding=1 cell-spacing=1 align='center' class="fixed" style='border:5px solid #303030; border-style:double; width:796.8px; max-height:1123.2px; padding:7px'>
	<tr>
		...
		<td style='width:30%; height:20px; text-align:right;'>
			<label style="font-weight:bold;">Page on : <?php echo count($pagearry['img'])+2; ?>/<?php echo $pagecount+1; ?></label>
		</td>
	</tr>
	<tr>
		<td style='width:70%; height:20px; font-weight:bold;'>
...
<? } ?>
	...
	function get_bid_new(year,code,head,stat)
	{
		$(".loader").show();
		$.ajax({
		url:"ajax_bid_new.php?action="+stat+"&year="+year+"&code="+code+"&head="+head,
		success:function(data)
		{
			if(stat == "CREATE")
			{
				...
				if(data == 1)
				{
					...
					document.getElementById('mybody').innerHTML="Session Finished , Login again...!";
				    $("#modal-default1").modal('show');
				}else if(data == 0){
					document.getElementById('mybody').innerHTML="Bid Send Failed..!";
				    $("#modal-default1").modal('show');
				}else if(data == 2)
				{
					document.getElementById('mybody').innerHTML="No Product Quotation Found..!";
				    $("#modal-default1").modal('show');
				}else if(data == "nobid") {
					document.getElementById('mybody').innerHTML="No reverse bid found..!";
				    $("#modal-default1").modal('show');
				}
			}else{
				$(".loader").fadeOut("slow");
				...
				if(data[0] == "failed")
				{
					document.getElementById('mybody').innerHTML="Failed in reverse Bid...!";
					$("#modal-default1").modal('show');
				}else if(data[0] != 0)
				{
				    document.getElementById('mybody').innerHTML="Reverse Bid Done ...!";
				    $("#modal-default1").modal('show');
					$('#sbmt_bid').prop('disabled', true);
				}else if(data[0] == 0 && data[1] != 0)
				{
					document.getElementById('mybody').innerHTML="Tender Closed...!";
				    $("#modal-default1").modal('show');
					$('#sbmt_bid').prop('disabled', true);
				}
			}
